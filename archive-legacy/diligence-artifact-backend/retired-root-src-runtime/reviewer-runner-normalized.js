...
import { runPhase7Layer5SemanticBatchQualityGatePhase } from "./phase7-layer5-semantic-batch-quality-gate-runner.js";
...
const NORMALIZED_RUNNER_PHASES = Object.freeze(["M8_FEATURE_CANDIDATE_INVENTORY", "DATA_PROVENANCE_PROFILE_LAYER4", "DATA_PROVENANCE_PROFILE_LAYER5", "NORMALIZED_COMPILER", "RENDERER"]);
...
export async function advanceReviewerRun({ run_id }) {
  ...
  try {
    ...
    else if (phase === "DATA_PROVENANCE_PROFILE_LAYER5") await runPhase7Layer5SemanticBatchQualityGatePhase({ run: { ...run, current_phase: phase }, phase, contract });
