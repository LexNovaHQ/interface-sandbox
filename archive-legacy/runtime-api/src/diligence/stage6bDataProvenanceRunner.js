...
function buildCanonicalDataProfile(stage6Review, input, normalized = null, legalGovernancePrefill = null) { return buildDataProvenanceProfile({ targetFeatureProfile: input.target_feature_profile, stage6bReview: stage6Review, legalDocumentCartography: input.legal_document_cartography || input.stage6a_review?.legal_document_cartography || {}, normalizedClassification: normalized, legalGovernancePrefill }); }
...
export async function runStage6BDataProvenanceClassification({ input = {}, promptText = "", env = process.env, options = {} } = {}) {
  ...
  const dataProvenanceProfile = buildCanonicalDataProfile(stage6Review, input, normalized.classification, finalized.legal_governance_prefill);
  const finalValidation = validateFinalStage6B(stage6Review, input, true);
  if (!finalValidation.ok) return { ok: false, ...finalValidation, stage6_review: stage6Review, data_provenance_profile: dataProvenanceProfile, legal_governance_prefill_debug: finalized.legal_governance_prefill, stage6_guardrail: finalValidation.guardrail || null, stage6_summary: stage6Summary(stage6Review, finalValidation.guardrail), packet_summary: packetSummary, normalized_semantic_classification: normalized.classification, semantic_classification_repairs: normalized.repairs, model_metadata: publicModelMetadata(runResult) };
  return { ok: true, semantic_packet_version: packet.semantic_packet_version, raw_semantic_classification_version: rawClassification?.semantic_classification_version || null, normalized_semantic_classification: normalized.classification, semantic_classification_repairs: normalized.repairs, stage6_review: stage6Review, data_provenance_profile: dataProvenanceProfile, legal_governance_prefill_debug: finalized.legal_governance_prefill, stage6_guardrail: finalValidation.guardrail, packet_summary: packetSummary, stage6_summary: stage6Summary(stage6Review, finalValidation.guardrail), validation: finalValidation.validation, model_metadata: publicModelMetadata(runResult) };
}
...
export async function runStage6BDataProvenance({ source_bundle, target_profile, company_profile, target_feature_profile, evidence_junction, legal_document_cartography, stage6a_review, runtime_options = {}, promptText = "", env = process.env } = {}) {
  ...
  if (deterministicOnlyAllowed && options.disableSemanticClassification === true) {
    ...
    const finalized = finalizeStage6BDataProvenance(built, input);
    const stage6Review = finalized.stage6_review;
    const dataProvenanceProfile = buildCanonicalDataProfile(stage6Review, input, null, finalized.legal_governance_prefill);
    const finalValidation = validateFinalStage6B(stage6Review, input, false);
    if (!finalValidation.ok) return { ok: false, ...finalValidation, stage6_review: stage6Review, data_provenance_profile: dataProvenanceProfile, legal_governance_prefill_debug: finalized.legal_governance_prefill, stage6_guardrail: finalValidation.guardrail || null, stage6_summary: stage6Summary(stage6Review, finalValidation.guardrail) };
    return { ok: true, semantic_model_attempted: false, semantic_model_disabled: true, normalized_semantic_classification: null, semantic_classification_repairs: [], stage6_review: stage6Review, data_provenance_profile: dataProvenanceProfile, legal_governance_prefill_debug: finalized.legal_governance_prefill, stage6_guardrail: finalValidation.guardrail, packet_summary: { data_flow_seed_count: stage6Review.data_provenance_profile?.data_flow_profile?.length || 0, feature_ref_count: target_feature_profile?.feature_inventory?.length || 0, provenance_ref_count: dataProvenanceCount(input), source_ref_count: 0 }, stage6_summary: stage6Summary(stage6Review, finalValidation.guardrail), validation: finalValidation.validation, model_metadata: null };
  }
