...
function legalEvidenceSummary(evidenceInput = {}) {
  ...
  return records
    ...
    .map((record) => ({
      ...
      source_bucket: record.source_bucket || record.discovery_source_bucket || null,
      title: record.structure?.title || "",
      word_count: record.text?.word_count || 0,
      clean_text_length: record.text?.clean_text_length || String(record.text?.clean_text_lossless || "").length,
      clean_text_sha256: record.text?.clean_text_sha256 || null,
      raw_html_sha256: record.hashes?.raw_html_sha256 || null,
      headings_preview: Array.isArray(record.structure?.headings)
        ? record.structure.headings.slice(0, 12).map((heading) => ({ level: heading.level, text: heading.text }))
        : [],
      embedded_markers: {
        dpa: /data processing addendum|\bDPA\b|annexure\s+c/i.test(record.text?.clean_text_lossless || ""),
        sla: /service level agreement|\bSLA\b|annexure\s+a/i.test(record.text?.clean_text_lossless || ""),
        subprocessor: /sub-?processor|subprocessing/i.test(record.text?.clean_text_lossless || ""),
        aup: /acceptable use|usage policy|prohibited use/i.test(record.text?.clean_text_lossless || "")
      }
    }));
}
...
console.log(JSON.stringify({ ok: true, step: "start", phase: "stage_4_legal_evidence_context_inspector", target: targetInput, runtime_url: base }, null, 2));
...
if (!sources.length) fail("Source discovery returned no capturable sources", {
...
const evidenceInput = buildEvidenceRefinerInput({ targetInput, discoveryResponse, captureResponse, runId: `inspect_${Date.now()}` });
const sourceCounts = evidenceInput.scrape_meta?.coverage_summary?.source_counts || {};
const documents = legalEvidenceSummary(evidenceInput);

console.log(JSON.stringify({
  ok: true,
  service: "lexnova-runtime-api",
  phase: "stage_4_legal_evidence_context_inspector",
  target: targetInput,
  source_counts: sourceCounts,
  coverage_gaps: evidenceInput.scrape_meta?.coverage_summary?.coverage_gaps || [],
  legal_evidence_context: {
    full_text_not_printed: true,
    legal_governance_document_count: documents.length,
    total_words: documents.reduce((sum, doc) => sum + (doc.word_count || 0), 0),
    documents
  }
}, null, 2));
