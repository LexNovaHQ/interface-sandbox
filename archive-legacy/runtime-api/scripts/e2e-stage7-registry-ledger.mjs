...
function readStage6Cache(filePath) { const parsed = readJsonFile(filePath, "Stage 6 cache"); if (parsed?.cache_version !== "stage6_canonical_e2e_cache_v1") fail("Bad Stage 6 cache version", { cache_version: parsed?.cache_version }); if (!parsed.source_bundle || !parsed.evidence_junction || !parsed.target_feature_profile || !parsed.stage6_review || !parsed.stage6_to_stage7_adapter) fail("Stage 6 canonical cache incomplete", { keys: Object.keys(parsed || {}) }); return parsed; }
...
const adapterResult = buildRegistryLedgerInput({ sourceBundle: cache.source_bundle, evidenceJunction: cache.evidence_junction, targetProfile: cache.company_profile || cache.target_profile_v2 || cache.target_profile, targetFeatureProfile: cache.target_feature_profile, stage6Review: cache.stage6_review, stage6ToStage7Adapter: cache.stage6_to_stage7_adapter, registryBatch: batch, registryKey, runId: batch.run_id, budget: { max_input_chars: Number(process.env.STAGE7_MAX_INPUT_CHARS || 120000), max_estimated_tokens: Number(process.env.STAGE7_MAX_ESTIMATED_TOKENS || 60000), max_single_source_chars: Number(process.env.STAGE7_MAX_SINGLE_SOURCE_CHARS || 60000), prompt_overhead_tokens: Number(process.env.STAGE7_PROMPT_OVERHEAD_TOKENS || 25000) } });
