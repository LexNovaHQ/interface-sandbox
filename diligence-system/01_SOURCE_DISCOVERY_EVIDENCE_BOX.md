...
`source_discovery_handoff.phase_packages` is mandatory. It must contain exactly the six canonical package keys above, and each package key must exist even when it has no sources.

No alternate package names are allowed:
- `target_feature_package` is forbidden; use `feature_profile_package`.
- `data_provenance_packages` is forbidden; use `data_provenance_package`.
- `legal_governance_evidence_package` is forbidden; use `legal_cartography_package`.
- `legal_governance_lossless_evidence_package` is forbidden; use `registry_support_package`.
...
```json
{
  "package_status": "POPULATED | EMPTY",
  "admitted_source_ids": [],
  "evidence_box_ids": [],
  "source_family_hints": [],
  "coverage_notes": [],
...
Each package is a mechanical routing packet only. Do not add target profiling, feature profiling, legal cartography, data provenance analysis, registry evaluation, or final report findings inside any package.
...
8. Emit `source_discovery_handoff.candidate_accounting` and `source_discovery_handoff.phase_packages` exactly as shown in the hard skeleton below.

### Final Output Hard Skeleton
```json
{
  "source_discovery_forensic_ledger": {},
  "source_discovery_trace": {},
  "source_discovery_handoff": {
    "candidate_accounting": {
      "s0_candidate_count": 0,
      "admitted_count": 0,
      "rejected_count": 0,
      "quarantined_count": 0,
      "limitation_routed_count": 0,
      "package_count": 6
    },
    "phase_packages": {
      "target_profile_package": {
...
Every routed package must be a mechanical packet containing only:
```text
package_status
admitted_source_ids
evidence_box_ids
...
CHECK 13 — source_discovery_handoff.candidate_accounting exists and package_count is 6.
CHECK 14 — source_discovery_handoff.phase_packages contains all six canonical package keys and no alternate package names.
```
