...
export async function runM12Phase2GChallenge({ run, internalJobId = "M12", contract, readArtifacts } = {}) {
  ...
  return Object.freeze({
    ok: true,
    output,
    phase_lock_status: challenge.status || "LOCKED_WITH_LIMITATIONS",
...
function assertCallback(fn, label) {
  if (typeof fn !== "function") throw new Error(`M12_PHASE2G_RUNNER_MISSING_CALLBACK:${label}`);
}
