...
export function enhanceLegalCartographyIndex({ index = {}, map = {}, semantic = {} } = {}) {
  ...
  index.control_language_locator = asArray(index.control_language_locator).map((row) => ({ ...row, limitation: row.limitation || "Technical locator only; not a material field by itself.", display_in_main_report: false, technical_annexure_only: true }));
  ...
  delete index.qualified_review_legal_signals;
  index.downstream_rules = {
    ...
    legal_signal_derivation_profile_is_separate_job_b_artifact: true,
    qualified_review_legal_signals_retired_from_m9_index: true,
...
function buildQualifiedReviewLocator(index) {
  const missing = asArray(index.missing_limited_legal_governance_items)
    .filter((row) => row.display_in_main_report !== false)
    .map((row, index) => stripEmpty({ locator_id: `M9-QRL-MISS-${String(index + 1).padStart(3, "0")}`, locator_type: "MISSING_OR_LIMITED_SOURCE", artifact_or_unit: row.missing_or_limited_item, expected_location: row.expected_location, status: row.status, blocking: row.blocking !== false, reviewer_action: "Verify whether the missing or limited source exists before reliance." }));
  const priority = asArray(index.priority_semantic_locator)
    .slice(0, 25)
    .map((row, index) => stripEmpty({ locator_id: `M9-QRL-SEM-${String(index + 1).padStart(3, "0")}`, locator_type: "PRIORITY_SEMANTIC_NAVIGATION", document_id: row.document_id, unit_id: row.unit_id, heading_label: row.heading_label, subcats: row.subcats, control_families: row.control_families, navigation_pointer: row.navigation_pointer, reviewer_action: "Use as locator; M9 is index-only." }));
  const locatorRows = ["legal_notice_locator", "dispute_resolution_locator", "governing_law_venue_locator", "contact_grievance_locator"]
    .flatMap((key) => asArray(index[key]))
    .slice(0, 20)
    .map((row, index) => stripEmpty({ locator_id: `M9-QRL-LOC-${String(index + 1).padStart(3, "0")}`, locator_type: row.locator_type, document_id: row.document_id, unit_id: row.unit_id, heading_label: row.heading_label, navigation_pointer: row.navigation_pointer, reviewer_action: "Use as locator; M9 is index-only." }));
