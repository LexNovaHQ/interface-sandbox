...
export const INTERNAL_JOB_WRITE_PERMISSIONS = Object.freeze({
  ...
  M12: [ART.exposureBatchValidationPattern, ART.challengeGate],
...
export function isKnownArtifactName(artifactName) {
  ...
  return ARTIFACT_NAMES.includes(name) || ARCHIVED_LEGACY_ARTIFACT_NAMES.includes(name) || LOSSLESS_FAMILY_ARTIFACT_PATTERN.test(name) || M11_BATCH_ARTIFACT_PATTERN.test(name) || M11_BATCH_VALIDATION_ARTIFACT_PATTERN.test(name);
}
...
export function assertKnownAgent(agentId) {
  if (!AGENTS.includes(agentId)) throw new Error(`INVALID_AGENT:${agentId || "missing"}`);
}
...
export function assertCanReadArtifact(agentId, artifactName) {
  ...
  if (!allowed.some((permission) => artifactMatchesPermission(artifactName, permission))) throw new Error(`READ_FORBIDDEN:${agentId}:${artifactName}`);
}
...
export function assertCanWriteArtifact(agentId, artifactName) {
  ...
  if (!allowed.some((permission) => artifactMatchesPermission(artifactName, permission))) throw new Error(`WRITE_FORBIDDEN:${agentId}:${artifactName}`);
}

export function assertInternalJobCanWriteArtifact(internalJobId, artifactName) {
  assertKnownArtifactName(artifactName);
  const allowed = INTERNAL_JOB_WRITE_PERMISSIONS[internalJobId] || [];
  if (!allowed.some((permission) => artifactMatchesPermission(artifactName, permission))) throw new Error(`INTERNAL_JOB_WRITE_FORBIDDEN:${internalJobId}:${artifactName}`);
}

export function publicPermissionMatrix() {
  return { read: READ_PERMISSIONS, write: WRITE_PERMISSIONS, internal_job_write: INTERNAL_JOB_WRITE_PERMISSIONS };
}

export const ARTIFACT_PERMISSION_CONTRACT_STATUS = Object.freeze({
...
function escapeRegExp(value) {
  const replacements = { "\\": "\\\\", "^": "\\^", "$": "\\$", "*": "\\*", "+": "\\+", "?": "\\?", ".": "\\.", "(": "\\(", ")": "\\)", "|": "\\|", "{": "\\{", "}": "\\}", "[": "\\[", "]": "\\]" };
  return String(value).split("").map((ch) => replacements[ch] || ch).join("");
}
