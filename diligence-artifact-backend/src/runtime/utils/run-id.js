...
export function createRunId(target = "TARGET", now = new Date()) {
  const stamp = [now.getUTCFullYear(), pad(now.getUTCMonth() + 1), pad(now.getUTCDate())].join("");
  const time = [now.getUTCHours(), now.getUTCMinutes(), now.getUTCSeconds()].map(pad).join("");
...
export function assertRunId(runId) {
  if (typeof runId !== "string" || !/^LN-\d{8}-\d{6}-[A-Z0-9-]+-\d{6}$/.test(runId)) {
