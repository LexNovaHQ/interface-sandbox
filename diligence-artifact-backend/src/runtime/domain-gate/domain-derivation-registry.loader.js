...
// Discover every domain key in references/registry/, keyed by its own domain_package.
// Adding a new domain = drop its <Name>_Registry_Key.yml here. No code change.
...
// Name preserved for consumer compatibility (validator + runner import this).
...
function assembleRules({ keysByPackage, grammar }) {
  ...
  for (const r of rules) { if (!r.rule_id || seen.has(r.rule_id)) continue; seen.add(r.rule_id); out.push(r); }
  return out;
}
...
function conditionRefs(expr) { return [...new Set(String(expr || "").match(/\bC\d+\b/g) || [])]; }
