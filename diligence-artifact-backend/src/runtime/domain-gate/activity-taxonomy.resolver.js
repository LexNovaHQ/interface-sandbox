...
export const ACTIVITY_TAXONOMY_RESOLVER_STATUS = Object.freeze({
  ...
  resolver_version: "v2_behavior_class_canonical",
...
export async function resolveActivityTaxonomy({ primaryPackageId, capabilityOverlayIds = [] } = {}) {
  ...
  for (const overlayId of overlayIds) {
    ...
    if (!record) { limitations.push(`OVERLAY_HAS_NO_TAXONOMY_KEY:${overlayId}`); continue; }
...
function normalizeKeyRecord({ packageId, file, key }) {
  ...
  return Object.freeze({
    ...
    behavior_class_vocabulary: Object.freeze(normalizeBehaviorClass(key?.behavior_class, declaredPackageId)),
...
function normalizeBehaviorClass(value, packageId) {
  return normalizeArray(value).map((row) => {
    if (typeof row === "string") return { code: row, normalized_name: row, trigger_if: "", exclude_if: [], package_id: packageId };
    return {
...
function normalizeSurfaceAxes(value, packageId) {
  if (Array.isArray(value)) return value.map((axis, index) => normalizeSurfaceAxis(axis, index, packageId)).filter(Boolean).map(Object.freeze);
  if (isPlainObject(value)) return Object.entries(value).map(([axisId, axisValue], index) => normalizeSurfaceAxis({ axis_id: axisId, tokens: normalizeArray(axisValue) }, index, packageId)).filter(Boolean).map(Object.freeze);
  return [];
}

function normalizeSurfaceAxis(axis, index, packageId) {
  if (typeof axis === "string") return { axis_id: `surface_axis_${index + 1}`, tokens: Object.freeze([Object.freeze({ token: axis, normalized_name: axis, trigger_if: "", package_id: packageId })]) };
  if (!isPlainObject(axis)) return null;
  const axisId = normalizeId(axis.axis_id || axis.id || axis.name || `surface_axis_${index + 1}`);
  const tokenRows = normalizeArray(axis.tokens || axis.values || axis.options || axis.surface_tokens || axis.token).map((row) => normalizeSurfaceToken(row, packageId)).filter(Boolean).map(Object.freeze);
...
function normalizeSurfaceToken(value, packageId) {
  ...
  return { token, normalized_name: normalizeId(value.normalized_name || value.name || value.label || token), trigger_if: normalizeTrigger(value.trigger_if), package_id: packageId };
}
function normalizeOverlayDeclaration(value) { return uniqueStrings(value); }
function normalizeTrigger(value) { if (typeof value === "string") return value.trim(); if (Array.isArray(value)) return value.map((item) => typeof item === "string" ? item : JSON.stringify(item)).join(" AND "); if (isPlainObject(value)) return JSON.stringify(value); return ""; }
