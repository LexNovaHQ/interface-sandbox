...
const NON_ANNEXURE_UI_ARTIFACTS = new Set([
  "renderer_payload",
  "qualified_review_handoff",
  "qualified_review_renderer_payload"
]);
const DOMAIN_SELECTION_STAGE = "PHASE_3B_DOMAIN_DERIVATION";
const PUBLIC_DOMAIN_SELECTION_VERSION = "public_domain_selection.v1";
...
publicRouter.use((req, res, next) => {
  if (!config.publicReviewerEnabled) {
    return res.status(404).json({
      ok: false,
      error: "PUBLIC_DILIGENCE_SYSTEM_DISABLED",
      message: "Public diligence-system routes are disabled."
    });
  }
  return next();
});

publicRouter.all(["/reviewer", "/reviewer/*"], (_req, res) => res.status(410).json({
  ok: false,
  error: "PUBLIC_REVIEWER_ALIAS_RETIRED",
  message: "Public reviewer routes are retired. Use /public/diligence-system routes."
}));
...
publicRouter.post("/diligence-system/jobs", async (req, res) => {
  try {
    ...
    return res.status(201).json({
      ...publicRunResponse(run),
      domain_selection: pendingPublicDomainSelection()
    });
  } catch (error) {
...
publicRouter.post("/diligence-system/jobs-with-documents", (_req, res) => res.status(409).json({
  ok: false,
...
publicRouter.get("/diligence-system/jobs/:run_id", async (req, res) => {
  try {
    ...
    const domainSelection = await buildPublicDomainSelection(artifacts);
    return res.json({
      ok: true,
      run: publicRunResponse(run, { artifact_count: artifacts.length }),
      artifacts: publicArtifactList(artifacts),
...
publicRouter.post("/diligence-system/jobs/:run_id/advance", async (req, res) => {
  try {
    ...
    if (body.sync) {
      throw new Error("SYNC_ADVANCE_RETIRED:Use async /advance and poll /public/diligence-system/jobs/:run_id instead.");
    }
...
publicRouter.get([
  ...
  "/diligence-system/jobs/:run_id/qualified-review"
], async (req, res) => {
  try {
    assertRunId(req.params.run_id);
    ...
    if (message.includes("ARTIFACT_NOT_FOUND") || message.includes("QUALIFIED_REVIEW")) {
      return res.status(409).json({
        ok: false,
        error: "QUALIFIED_REVIEW_NOT_READY",
        message: "Qualified Review has not been generated for this run."
      });
    }
    return sendError(res, error);
  }
...
publicRouter.put("/diligence-system/qualified-review/:run_id/draft", async (req, res) => {
  try {
    ...
    await rebuildQualifiedReviewWorkspace({
      run: await getRunRecord(req.params.run_id),
      reviewer_values: saved.artifact.field_edits || {}
    });
    return res.json(await qualifiedReviewResponse(req.params.run_id));
  } catch (error) {
    return sendError(res, error);
...
publicRouter.put("/diligence-system/qualified-review/:run_id/sections/:section_id/attestation", async (req, res) => {
  try {
    ...
    const run = await getRunRecord(req.params.run_id);
    const handoff = await readRequiredArtifact(req.params.run_id, "qualified_review_handoff");
    const saved = await attestQualifiedReviewSection({
      run,
      handoff,
...
publicRouter.post("/diligence-system/qualified-review/:run_id/submit", async (req, res) => {
  try {
    ...
    return res.status(201).json({
      ...
      qualified_review_submission_request: saved.artifact
    });
  } catch (error) {
    return sendError(res, error);
  }
...
publicRouter.post("/diligence-system/qualified-review/:run_id/responses", (_req, res) => res.status(410).json({
  ok: false,
  error: "QUALIFIED_REVIEW_MATRIX_SUBMISSION_RETIRED",
  message: "Per-question matrix submission is retired. Use section-attested Qualified Review draft and submit routes."
}));
...
async function qualifiedReviewResponse(runId) {
  ...
  return {
    ...
    report_summary: {
      ...
      report_title: reportPayload?.report_shell?.report_title || "Interface Diligence Report"
    },
...
async function buildPublicDomainSelection(artifacts = []) {
  try {
    const [manifestRaw, registryRaw] = await Promise.all([
      readArtifactFromMetadata(artifacts, "active_run_package_manifest"),
      readArtifactFromMetadata(artifacts, "active_threat_registry_manifest")
    ]);
    const manifest = unwrapArtifact(manifestRaw, "active_run_package_manifest");
    const registry = unwrapArtifact(registryRaw, "active_threat_registry_manifest");

    const selectionStage = String(manifest.selection_stage || "");
    const primaryPackage = String(manifest.primary_domain_package || manifest.derived_primary_domain || "");
    const derivedSector = String(manifest.derived_primary_domain || primaryPackage);
    const resolved = selectionStage === DOMAIN_SELECTION_STAGE && Boolean(primaryPackage);
    const inferredPackages = [];

    if (primaryPackage) inferredPackages.push(primaryPackage);
    if (manifest.ai_package_mount === "AI_PRIMARY") inferredPackages.push("ai-governance");
    if (manifest.ai_package_mount === "AI_OVERLAY_MOUNTED") inferredPackages.push("ai-governance");

    const mountedRegistryPackages = uniqueStrings(
      Array.isArray(registry.mounted_packages) && registry.mounted_packages.length
        ? registry.mounted_packages
        : inferredPackages
    );

    return {
      projection_version: PUBLIC_DOMAIN_SELECTION_VERSION,
      status: resolved
        ? String(manifest.primary_domain_status || manifest.validation?.status || "DERIVED")
        : "PENDING_PHASE_3",
      resolved,
      selection_stage: selectionStage || "PRE_PHASE_3",
      derived_sector: derivedSector,
      primary_domain_package: primaryPackage,
      primary_domain_status: String(manifest.primary_domain_status || "PROVISIONAL"),
      ai_overlay_status: String(manifest.ai_package_mount || "PROVISIONAL"),
      mounted_registry_packages: mountedRegistryPackages,
      regulatory_overlays: uniqueStrings(manifest.regulatory_overlays || [])
    };
  } catch {
    return pendingPublicDomainSelection();
  }
}

function pendingPublicDomainSelection() {
  return {
    projection_version: PUBLIC_DOMAIN_SELECTION_VERSION,
    status: "PENDING_PHASE_3",
    resolved: false,
    selection_stage: "PRE_PHASE_3",
    derived_sector: "",
    primary_domain_package: "",
    primary_domain_status: "PROVISIONAL",
    ai_overlay_status: "PROVISIONAL",
    mounted_registry_packages: [],
    regulatory_overlays: []
  };
}

async function readArtifactFromMetadata(artifacts, artifactName) {
  const metadata = (artifacts || []).find((artifact) => artifact.artifact_name === artifactName);
  if (!metadata?.drive_file_id) return null;
  return readJsonArtifactFromDrive(metadata.drive_file_id);
}
...
function publicRunResponse(run, options = {}) {
  return {
    ...
    diligence_qa_complete: run.diligence_qa_complete === true,
    assembly_authorized: run.assembly_authorized === true,
    assembly_complete: run.assembly_complete === true,
    generated_document_count: Number(run.generated_document_count || 0),
...
function publicAsyncResponse(result) {
  return {
    ...
    central_phase: result.central_phase || "",
    central_phase_label: result.central_phase_label || "",
    runner_mode: result.runner_mode || "",
    runner_state: result.runner_state,
    runner_last_error: safeRunnerDiagnostic(result.runner_last_error),
    artifact_count: Number(result.artifact_count || 0),
    poll: `/public/diligence-system/jobs/${result.run_id}`
  };
}
...
function publicTechnicalAnnexureResponse({ run, artifacts }) {
  const rows = publicTechnicalAnnexureArtifactList(artifacts);
  return {
    ok: true,
    run_id: run.run_id,
    public_label: "Public Technical Annexure",
    ...
    exclusion_rule: "Excludes platform secrets, provider telemetry, raw infrastructure logs, and private reviewer submissions.",
    artifact_count: rows.length,
    artifacts: rows,
    endpoint_contract: {
      public_manifest_route: true,
      full_payload_zip_export: false,
      full_payloads_inline: false
    }
  };
}

function publicTechnicalAnnexureArtifactList(artifacts) {
  return artifacts
    .map((artifact) => {
      const name = artifact.artifact_name || "";
      const privateExcluded = PRIVATE_ANNEXURE_ARTIFACTS.has(name);
      const uiExcluded = NON_ANNEXURE_UI_ARTIFACTS.has(name);
      return {
        artifact_name: name,
        phase: artifact.phase,
        lock_status: artifact.lock_status,
        latest_version: artifact.latest_version || artifact.version,
        updated_at: artifact.updated_at || artifact.created_at,
        included_in_public_annexure_manifest: !privateExcluded && !uiExcluded,
        exclusion_reason: privateExcluded
          ? "PRIVATE_REVIEWER_OR_DOCUMENT_STATE"
          : uiExcluded
            ? "UI_ARTIFACT_NOT_TECHNICAL_ANNEXURE"
            : ""
      };
    })
    .filter((row) => row.included_in_public_annexure_manifest);
}
...
function safeRunnerDiagnostic(value) {
  return String(value || "").replace(/GEMINI_API_KEYS_\d+/g, "provider-key").slice(0, 1000);
}
