...
import { saveQualifiedReviewSubmission } from "../services/qualified-review-submission.service.js";
...
publicRouter.post("/diligence-system/jobs-with-documents", (_req, res) => {
  return res.status(409).json({ ok: false, error: "DOCUMENT_UPLOAD_17_ROOT_CUTOVER_REQUIRED", message: "Uploaded-document intake is fenced until it is rebuilt against the active 17-root Source Discovery contract. The retired lossless-family uploader is not permitted in the central runtime." });
});
...
publicRouter.get(["/diligence-system/qualified-review/:run_id", "/diligence-system/jobs/:run_id/qualified-review"], async (req, res) => {
  try {
    ...
    return res.json({ ok: true, run_id: req.params.run_id, public_label: "Qualified Review", system_boundary: { source_system: "Interface Diligence Engine", entry_condition: "renderer_payload and Qualified Review artifacts exist", qualified_review_is_separate_system: true, shares_pipeline_run_id: true, no_document_assembly: true }, run_status: run.status, current_phase: run.current_phase, central_phase: run.central_phase || "", central_phase_label: run.central_phase_label || "", report_ready: true, report_renderer_ref: "renderer_payload", report_summary: { validation_status: reportPayload?.validation_status || reportPayload?.report_shell?.validation_status || "", report_title: reportPayload?.report_shell?.report_title || "Interface Diligence Report" }, qualified_review_handoff: handoff, qualified_review_renderer_payload: rendererPayload, qualified_review_validation_manifest: validationManifest, qualified_review_section_artifacts: sectionArtifacts, qualified_review_submission: submission });
  } catch (error) {
    return sendError(res, error);
  }
});

publicRouter.post("/diligence-system/qualified-review/:run_id/responses", async (req, res) => {
  try {
    requireRuntimeConfig();
    assertRunId(req.params.run_id);
    const run = await getRunRecord(req.params.run_id);
    ...
    return res.status(201).json({ ok: true, run_id: req.params.run_id, artifact_name: saved.artifact_name, version: saved.version, final_gate_status: saved.final_gate_status, validation: saved.validation, drive_file_id: saved.drive_file_id, drive_web_view_link: saved.drive_web_view_link, qualified_review_submission: saved.submission });
  } catch (error) {
    return sendError(res, error);
  }
});
