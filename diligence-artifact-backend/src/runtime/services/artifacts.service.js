...
export const ARTIFACTS_SERVICE_STATUS = Object.freeze({ central_runtime_service: "artifacts.service", phase3_domain_derivation_save_order_gate_enforced: true, phase3_manifest_update_order_gate_enforced: true, downstream_domain_derivation_required_before_activity: true, central_phase_aware: true });
...
async function assertArtifactSaveOrder(parsed, phaseContext) {
  ...
  if ([ART.targetProfileSourceIndex, ART.activityProfileSourceIndex, ART.dataPrivacyNavigationIndex, ART.cartographyIndex, ART.cartographyValidationManifest].includes(artifact_name)) await requireSavedArtifact(run_id, ART.legalSignalDerivationProfile, `SAVE_ORDER_BLOCKED:${artifact_name}_requires_legal_signal_derivation_profile`);
