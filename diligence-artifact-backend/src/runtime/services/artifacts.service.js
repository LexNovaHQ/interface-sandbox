...
export const ARTIFACTS_SERVICE_STATUS = Object.freeze({ central_runtime_service: "artifacts.service", phase2a_target_profile_source_index_save_order_gate_enforced: true, phase2b_domain_derivation_source_index_save_order_gate_enforced: true, phase3_domain_derivation_save_order_gate_enforced: true, phase3_manifest_update_order_gate_enforced: true, downstream_domain_derivation_required_before_activity: true, central_phase_aware: true });
...
function assertRuntimeWritePermission(agentId, artifactName) { if (RUNTIME_ARTIFACT_EXTRAS.has(artifactName)) return; assertCanWriteArtifact(agentId, artifactName); }
...
async function assertArtifactSaveOrder(parsed, phaseContext) {
  ...
  if (artifact_name === ART.targetProfileDeterministicMap) await requireSavedArtifact(run_id, ART.legalSignalDerivationProfile, "SAVE_ORDER_BLOCKED:target_profile_deterministic_map_requires_legal_signal_derivation_profile");
  if (artifact_name === ART.targetProfileSemanticProfile) await requireSavedArtifact(run_id, ART.targetProfileDeterministicMap, "SAVE_ORDER_BLOCKED:target_profile_semantic_profile_requires_target_profile_deterministic_map");
  if (artifact_name === ART.targetProfileSourceIndex) await requireSavedArtifact(run_id, ART.targetProfileSemanticProfile, "SAVE_ORDER_BLOCKED:target_profile_source_index_requires_target_profile_semantic_profile");
  if (artifact_name === ART.domainDerivationDeterministicMap) await requireSavedArtifact(run_id, ART.targetProfileSourceIndex, "SAVE_ORDER_BLOCKED:domain_derivation_deterministic_map_requires_target_profile_source_index");
  if (artifact_name === ART.domainDerivationSemanticProfile) await requireSavedArtifact(run_id, ART.domainDerivationDeterministicMap, "SAVE_ORDER_BLOCKED:domain_derivation_semantic_profile_requires_domain_derivation_deterministic_map");
  if (artifact_name === ART.domainDerivationSourceIndex) await requireSavedArtifact(run_id, ART.domainDerivationSemanticProfile, "SAVE_ORDER_BLOCKED:domain_derivation_source_index_requires_domain_derivation_semantic_profile");
  ...
  if (artifact_name === ART.targetForensics) { await requireSavedArtifact(run_id, ART.targetProfile, "SAVE_ORDER_BLOCKED:target_forensics_requires_target_profile"); await requireSavedArtifact(run_id, ART.domainDerivationProfile, "SAVE_ORDER_BLOCKED:target_forensics_requires_domain_derivation_profile"); }
  ...
  if (artifact_name === ART.challengeGate) await requireSavedArtifact(run_id, ART.exposureForensics, "SAVE_ORDER_BLOCKED:operator_challenge_requires_exposure_forensics");
}

async function assertArtifactLockOrder(body, phaseContext) { if (!LOCK_ADVANCE_STATUSES.has(body.status)) return; for (const artifactName of requiredWritesForPhaseContext(phaseContext)) { if (String(artifactName).includes("{")) continue; await requireSavedArtifact(body.run_id, artifactName, `PHASE_LOCK_BLOCKED:${phaseContext.internal_job_id}_requires_${artifactName}`); } }
