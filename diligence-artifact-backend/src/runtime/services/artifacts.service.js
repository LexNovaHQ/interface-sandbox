...
export const ARTIFACTS_SERVICE_STATUS = Object.freeze({ central_runtime_service: "artifacts.service", migration_status: "phase2_cartography_index_order_gates_synced", old_artifact_service_bridge_removed: true, old_constants_permissions_dependency_removed: true, central_phase_aware: true, source_to_phase6_order_gates_enforced: true, phase2_cartography_index_gates_enforced: true, old_legal_signal_gate_removed: true, old_data_privacy_navigation_index_gate_removed: true, phase7_semantic_batch_gate_enforced: true, phase8_dap_forensics_gate_enforced: true, old_m10_4b_4c_sidecar_removed: true });
...
async function assertArtifactSaveOrder(parsed) {
  ...
  if ([ART.targetProfileSourceIndex, ART.activityProfileSourceIndex, ART.dataProvenanceSourceIndex, ART.legalGovernanceSourceIndex, ART.cartographyIndex, ART.cartographyValidationManifest].includes(artifact_name)) await requireSavedArtifact(run_id, ART.cartographySemanticNavigationOverlay, `SAVE_ORDER_BLOCKED:${artifact_name}_requires_semantic_navigation_overlay`);
  if (artifact_name === ART.targetProfile) { await requireSavedArtifact(run_id, ART.cartographyIndex, "SAVE_ORDER_BLOCKED:target_profile_requires_cartography_index"); await requireSavedArtifact(run_id, ART.targetProfileSourceIndex, "SAVE_ORDER_BLOCKED:target_profile_requires_target_profile_source_index"); }
...
async function assertArtifactLockOrder(body, phaseContext) {
  ...
  if (requiresAny(ART.cartographyIndex, ART.cartographyValidationManifest, ART.targetProfileSourceIndex, ART.activityProfileSourceIndex, ART.dataProvenanceSourceIndex, ART.legalGovernanceSourceIndex)) await requireSavedArtifact(body.run_id, ART.cartographySemanticNavigationOverlay, "PHASE_LOCK_BLOCKED:compiled_indexes_require_semantic_overlay");
