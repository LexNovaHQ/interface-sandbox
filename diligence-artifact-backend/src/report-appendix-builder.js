...
const APPENDIX_MATERIAL_STATUSES = new Set(["TRIGGERED", "CONTROLLED_BY_VISIBLE_CONTROL", "CONTROLLED_BY_EXCLUSION", "CONTROLLED_BY_PUBLIC_EVIDENCE_LIMITATION"]);
...
export function buildForensicLedgerAppendix({ handoff, profiles, forensics, displayIdIndex }) {
  ...
  const dapGate = unwrap(profiles.data_provenance_profile_semantic_batch_gate, "data_provenance_profile_semantic_batch_gate");
  ...
  return {
    ...
    appendix_notice: "This appendix preserves audit, proof, and validation material from locked upstream artifacts.",
    full_ledger_summary: {
      ...
      phase7_dap_field_count: Number(dapGate.field_count || 0),
    ...
    full_registry_ledger: registryRows.map((row, index) => ({ appendix_ref: `REG-${String(index + 1).padStart(3, "0")}`, display_exposure_id: triggeredDisplayIds.get(row.Threat_ID) || null, threat_id: row.Threat_ID || "", registry_order: row.registry_order ?? index + 1, archetype: row.archetype || row.Archetype || "", surface: row.surface || row.Surface || "", route: safeText(row.route, "Route not specified"), route_reason: safeText(row.route_reason, "Route reason not specified"), final_material_status: statusLabel(row.final_material_status), internal_evaluation_status: statusLabel(row.internal_evaluation_status), trigger_status: statusLabel(row.trigger_status), limitations: safeText(row.limitations, "No row-specific limitation recorded") })),
    row_level_proof: registryRows.filter((row) => APPENDIX_MATERIAL_STATUSES.has(String(row.final_material_status || "").toUpperCase())).map((row) => ({ threat_id: row.Threat_ID || "", display_exposure_id: triggeredDisplayIds.get(row.Threat_ID) || null, material_status: statusLabel(row.final_material_status), basis_proof: safeText(row.material_projection?.basis_proof || row.basis_proof, "Basis proof not specified in material projection"), control_exclusion_evaluation: safeText(row.material_projection?.control_exclusion_evaluation || row.control_exclusion_evaluation, "Control/exclusion evaluation not specified"), evidence_source_basis: safeText(row.material_projection?.evidence_source_basis || row.evidence_source_basis, "Evidence/source basis not specified"), fp_mechanism: safeText(row.material_projection?.fp_mechanism || row.fp_mechanism, "FP mechanism not specified"), pain_tier: safeText(row.material_projection?.Pain_Tier || row.Pain_Tier, "Pain tier not specified"), pain_depth: safeText(row.material_projection?.Pain_Depth || row.Pain_Depth, "Pain depth not specified"), pain_category: safeText(row.material_projection?.Pain_Category || row.Pain_Category, "Pain category not specified"), review_route: safeText(row.material_projection?.review_route || row.review_route, "Qualified reviewer should verify") })),
    ...
    renderer_export_trace: { renderer_generated_by: "locked-report-renderer", renderer_deterministic: true, model_used_after_m11: false, split_exposure_profiles_used: true, phase7_dap_runtime_active: true },
    forensic_boundary: { no_chain_of_thought: true, no_hidden_scratchpad: true, no_legal_conclusions: true, appendix_only: true }
  };
}
...
export function collectLimitations({ profiles = {}, forensics = {}, handoff = {} } = {}) {
  ...
  pushLimit(rows, "phase7_dap_gate", profiles.data_provenance_profile_semantic_batch_gate?.errors);
  pushLimit(rows, "phase7_dap_validation_manifest", profiles.dap_semantic_batch_validation_manifest?.validation_quality_control_result?.errors);
