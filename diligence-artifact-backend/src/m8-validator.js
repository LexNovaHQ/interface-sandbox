const MATERIAL_TOP_LEVEL_KEYS = Object.freeze(["target_feature_profile"]);
const FORENSIC_TOP_LEVEL_KEYS = Object.freeze(["target_feature_profile_forensics"]);
...
const FORENSIC_FORBIDDEN_KEYS = Object.freeze(["target_feature_profile"]);
...
export function validateM8TargetFeatureOutput(output, { phase = "M8_TARGET_FEATURE_PROFILE" } = {}) {
  ...
  if (phase === "M8_TARGET_FEATURE_PROFILE") {
    ...
    if (!failures.length) {
      validateForensics(output.target_feature_profile_forensics, failures);
      for (const stale of STALE_STRINGS) if (deepContains(output, stale)) failures.push(`stale or forbidden string present: ${stale}`);
    }
  } else {
...
function validateForensics(forensics, failures) {
  ...
  if (containsAnyKey(forensics, FORENSIC_FORBIDDEN_KEYS)) failures.push("target_feature_profile_forensics contains forbidden material artifact");
...
function validateSelectedPaRows(forensics, failures) {
  ...
  for (const row of ledger) {
    ...
    if (!String(row.fd_field_id || "").startsWith("PA.")) failures.push(`selected_pa_field_derivation_ledger invalid fd_field_id for ${row.activity_reference || "unknown"}.${normalizeField(row) || "unknown"}`);
    if (!hasSourceUrl(row)) failures.push(`selected_pa_field_derivation_ledger missing source_url/source_urls for ${row.activity_reference || "unknown"}.${normalizeField(row) || "unknown"}`);
  }
}
...
function validateClassificationRow(row, type, failures) {
  ...
  if (!Array.isArray(row.conditions)) failures.push(`${label} conditions must be array`);
