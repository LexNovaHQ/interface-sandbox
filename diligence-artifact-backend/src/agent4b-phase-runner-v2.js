...
export async function runAgent4bExtendedDapPhase({ run, phase, contract }) {
  try {
    ...
    const artifact = normalizeExtendedDapMissingProofQueue(output?.[ARTIFACT_NAME]);
    ...
    await logEvent({ run_id: run.run_id, event_type: "AGENT4B_EXTENDED_DAP_COMPLETED", actor: ACTOR, payload: { phase, artifact_name: ARTIFACT_NAME, lock_status: status, field_count: artifact.field_count, model_usage: "NONE_DETERMINISTIC", missing_proof_queue_normalized: true } });
...
export async function buildAgent4bSidecarNonblocking({ run, dataForensicsArtifact }) {
  try {
    ...
    await logEvent({ run_id: run.run_id, event_type: "AGENT4B_EXTENDED_DAP_COMPLETED", actor: ACTOR, payload: { artifact_name: ARTIFACT_NAME, lock_status: status, field_count: artifact.field_count, model_usage: "NONE_DETERMINISTIC", mode: "sidecar_nonblocking", missing_proof_queue_normalized: true } });
  } catch (error) {
...
function normalizeExtendedDapMissingProofQueue(artifact) {
  if (!artifact || typeof artifact !== "object") return artifact;
  const missing = Array.isArray(artifact.missing_proof_requests) ? artifact.missing_proof_requests : [];
  return {
    ...artifact,
    missing_proof_requests: missing.map((row) => ({
      ...row,
      field_status: row.field_status || row.status || "",
      status: proofRequestStatus(row.status),
      required_action: row.required_action || "QUALIFIED_REVIEW_OR_TARGETED_REINVESTIGATION",
      blocking: false
    }))
  };
}
