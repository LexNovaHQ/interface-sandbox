...
export async function runM11OrchestratedPhase({ run, phase, contract }) {
  ...
  for (const batch of route.artifact.batch_plan || []) {
    ...
    const batchOutput = await runM11Batch({ run, phase, batch, compactPacket });
    ...
    if (!structuralValidation.ok) {
      const repair = buildStructuralBatchValidationArtifact({ batch, structuralValidation });
      await saveArtifact(artifactSaveBody({ run_id: run.run_id, phase, agent_id: AGENT_5, artifact_name: validationName, artifact: repair, lock_status: "REPAIR_REQUIRED" }));
      await lockPhase({ run_id: run.run_id, phase, agent_id: AGENT_5, status: "REPAIR_REQUIRED", next_phase: phase });
      return;
    }

    const validationArtifact = await runM12BatchValidation({ run, phase, batch, compactPacket, batchOutput, structuralValidation, routePlan: route.artifact });
...
function buildStructuralBatchValidationArtifact({ batch, structuralValidation }) { return { exposure_registry_batch_validation: { batch_id: batch.batch_id, batch_group: batch.batch_group, status: "REPAIR_REQUIRED", validation_owner: "backend_structural_validator", semantic_m12_validation_status: "REPAIR_REQUIRED", expected_threat_ids: batch.expected_threat_ids || [], failures: structuralValidation.failures || [] } }; }
