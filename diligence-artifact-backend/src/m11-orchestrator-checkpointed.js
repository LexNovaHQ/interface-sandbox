...
export async function runM11OrchestratedPhase({ run, phase, contract }) {
  ...
  for (const batch of route.artifact.batch_plan || []) {
    ...
    if (completed) {
      ...
      await logEvent({ run_id: run.run_id, event_type: "M11_CHECKPOINT_REUSED", actor: AGENT_5, payload: { artifact_name: `exposure_registry_batch__${batch.batch_id}`, lock_status: completed.batchLockStatus } });
      continue;
    }
  ...
  await logEvent({ run_id: run.run_id, event_type: "M11_ORCHESTRATED_PHASE_COMPLETED", actor: AGENT_5, payload: { batch_prompt_mode: "semantic_evidence_application_then_backend_materialization", route_status: route.lock_status, batch_count: acceptedBatches.length, forensic_status: finalStatus } });
...
async function readAcceptedCheckpoint({ run_id, artifact_name }) { try { const result = await readArtifact({ run_id, artifact_name, agent_id: AGENT_5 }); return isAccepted(result.lock_status) ? { artifact: result.artifact, lock_status: result.lock_status } : null; } catch (_error) { return null; } }
async function readCompletedBatchCheckpoint({ run_id, batch_id }) { const validation = await readAcceptedCheckpoint({ run_id, artifact_name: `exposure_registry_batch_validation__${batch_id}` }); const batch = await readAcceptedCheckpoint({ run_id, artifact_name: `exposure_registry_batch__${batch_id}` }); return validation && batch ? { validationArtifact: validation.artifact, batchArtifact: batch.artifact, validationLockStatus: validation.lock_status, batchLockStatus: batch.lock_status } : null; }
