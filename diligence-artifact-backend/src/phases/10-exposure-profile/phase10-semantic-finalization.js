...
export const SEMANTIC_PACKET_VERSION = "M11_PACKAGE_SCOPED_SEMANTIC_PACKET_v1";
export const SEMANTIC_LEDGER_VERSION = "M11_PACKAGE_SCOPED_SEMANTIC_LEDGER_v1";
export const LAYER3_VERSION = "M11_DYNAMIC_LAYER3_v1";
export const FORENSICS_VERSION = "M11_DOMAIN_AGNOSTIC_FORENSICS_v1";

const STATUS_VALUES = new Set(["yes", "no", "partial"]);
const FINAL_STATUSES = Object.freeze({
  triggered: "TRIGGERED",
  visible: "CONTROLLED_BY_VISIBLE_CONTROL",
  exclusion: "CONTROLLED_BY_EXCLUSION",
  limitation: "CONTROLLED_BY_PUBLIC_EVIDENCE_LIMITATION"
});
const SEMANTIC_ROW_FIELDS = new Set([
  "Threat_ID", "trigger_status", "target_match", "basis_proof",
  "control_exclusion_evaluation", "evidence_source_basis",
  "applied_fp_mechanism", "row_limitations", "status_inputs"
]);
const STATUS_INPUT_FIELDS = Object.freeze([
  "target_match_present", "hunter_conditions_met", "trigger_if_met",
  "exclude_if_met", "visible_control_present",
  "visible_control_defeats_or_reduces_exposure", "evidence_sufficient",
  "public_evidence_limitation", "false_positive_concern"
]);
const MATERIAL_FIELDS = Object.freeze([
  "Threat_ID", "Threat_Name", "target_match", "evaluation_status", "basis_proof",
  "control_exclusion_evaluation", "evidence_source_basis", "fp_mechanism",
  "Archetype", "Subcategory", "Surface", "authority_anchors", "Pain_Tier",
  "Pain_Depth", "Pain_Category", "Legal_Pain", "remediation", "review_route",
  "row_limitations"
]);
...
export function buildPackageScopedSemanticPacket({ batch, routePlan, upstreamArtifacts = {}, manifest } = {}) {
  ...
  const base = {
    ...
    registry_rows: rows.map((row) => ({
      ...
      deterministic_registry_spine: buildDeterministicSpine(row)
    })),
  ...
  const dco = upstreamArtifacts.domain_control_obligation_profile
    ? buildM11DomainControlObligationHandoff({
        domainControlObligationProfile: upstreamArtifacts.domain_control_obligation_profile,
        activeThreatRows: rows.map((row) => row.registry_row)
      })
    : null;
  ...
  const packet = {
    ...
    semantic_packet_validation: {
      status: "PASS",
      one_package: true,
      one_stream: true,
      one_archetype_group: true,
      exact_registry_row_key_count: rows.length,
      exact_threat_id_count: rows.length
    }
  };
  if (JSON.stringify(packet).length > Number(batch.max_packet_chars || 180000)) {
    throw new Error(`M11_SEMANTIC_PACKET_CEILING_EXCEEDED:${batch.batch_id}`);
  }
...
export function validateSemanticLedger({ semanticOutput, batch, routePlan } = {}) {
  ...
  for (const [field, expected] of Object.entries({
    semantic_contract_version: SEMANTIC_LEDGER_VERSION,
    batch_id: batch.batch_id,
    batch_group: batch.batch_group,
    stream_id: batch.stream_id,
    stream_type: batch.stream_type,
    package_id: batch.package_id,
    source_domain: batch.source_domain
  })) if (String(ledger[field] ?? "") !== String(expected ?? "")) failures.push(`BATCH_IDENTITY_MISMATCH:${field}`);
  ...
  for (const [index, row] of rows.entries()) {
    ...
    for (const field of ["target_match", "basis_proof", "control_exclusion_evaluation", "evidence_source_basis", "applied_fp_mechanism", "row_limitations"]) {
      if (typeof row?.[field] !== "string") failures.push(`SEMANTIC_TEXT_FIELD_INVALID:${threatId}:${field}`);
    }
  ...
  return {
    exposure_registry_batch_validation: {
      ...
      returned_threat_ids: [...returnedIds],
      status: failures.length ? "REPAIR_REQUIRED" : "PASS",
      failures,
      limitations: [],
      exact_coverage: failures.length === 0,
...
export function assembleAcceptedBatch({ semanticOutput, batch, routePlan } = {}) {
  ...
  const rows = asArray(batch.expected_registry_row_keys).map((key) => {
    ...
    return {
      ...
      deterministic_registry_spine: buildDeterministicSpine(routeRow),
      semantic_evidence_application: semanticRow,
      final_material_status: deriveFinalMaterialStatus(semanticRow.status_inputs),
      material_projection: buildMaterialProjection(routeRow, semanticRow)
    };
  });
  return {
    m11_batch_registry_ledger: {
      schema_version: "m11_batch_registry_ledger.v3.package_scoped.accepted",
      batch_id: batch.batch_id,
      stream_id: batch.stream_id,
      stream_type: batch.stream_type,
      package_id: batch.package_id,
...
export function buildDynamicWorkpad({ manifest, routePlan, acceptedBatches = [], batchValidations = [] } = {}) {
  ...
  const rows = asArray(route.route_rows).map((routeRow) => {
    ...
    return {
      registry_row_key: routeRow.registry_row_key,
      package_id: routeRow.package_id,
      source_domain: routeRow.source_domain,
      stream_id: routeRow.stream_id,
      stream_type: routeRow.stream_type,
      Threat_ID: routeRow.Threat_ID,
      deterministic_registry_spine: buildDeterministicSpine(routeRow),
      semantic_evidence_application: null,
      final_material_status: "NOT_TRIGGERED_NOT_APPLICABLE",
      workpad_only: true,
  ...
  return {
    exposure_registry_workpad_98: {
      schema_version: "exposure_registry_workpad.v3.dynamic",
      artifact_token_stability_note: "artifact name retained for compatibility; row count is dynamic",
      expected_registry_row_key_count: expected,
      actual_registry_row_key_count: rows.length,
      mounted_packages: [...asArray(manifest.mounted_packages)],
      registry_rows: rows,
      batch_validation_count: batchValidations.length,
      final_status_counts: countBy(rows, (row) => row.final_material_status),
      merge_validation: { status: "PASS", failures: [], dynamic_count_reconciled: true }
    }
  };
}
  ...
  return {
    controlled: {
      exposure_registry_controlled_profile: {
        schema_version: "exposure_registry_controlled_profile.v3.dynamic",
        controlled_rows: material.filter((row) => row.final_material_status !== FINAL_STATUSES.triggered).map((row) => ({
          registry_row_key: row.registry_row_key,
          package_id: row.package_id,
          stream_id: row.stream_id,
          stream_type: row.stream_type,
          ...row.material_projection
        }))
      }
    },
    triggered: {
      exposure_registry_triggered_profile: {
        schema_version: "exposure_registry_triggered_profile.v3.dynamic",
        triggered_rows: material.filter((row) => row.final_material_status === FINAL_STATUSES.triggered).map((row) => ({
          registry_row_key: row.registry_row_key,
          package_id: row.package_id,
          stream_id: row.stream_id,
          stream_type: row.stream_type,
...
export function buildDomainAgnosticForensics({ manifest, routePlan, workpad, controlledProfile, triggeredProfile, acceptedBatches = [], batchValidations = [] } = {}) {
  ...
  const streamTrace = asArray(route.stream_plans).map((stream) => {
    const streamRows = asArray(work.registry_rows).filter((row) => row.stream_id === stream.stream_id);
    return {
      stream_id: stream.stream_id,
      stream_type: stream.stream_type,
      package_id: stream.package_id,
      expected_registry_rows: stream.expected_registry_rows,
      workpad_rows: streamRows.length,
      status_counts: countBy(streamRows, (row) => row.final_material_status),
      reconciliation_status: streamRows.length === stream.expected_registry_rows ? "PASS" : "FAIL"
    };
  });
  if (streamTrace.some((row) => row.reconciliation_status !== "PASS")) failures.push("STREAM_RECONCILIATION_FAILED");
  ...
  return {
    exposure_registry_profile_forensics: {
      ...
      registry_set_fingerprint: manifest.registry_set_fingerprint,
      mounted_packages: [...asArray(manifest.mounted_packages)],
      expected_registry_row_key_count: expectedKeys.size,
      route_row_count: expectedKeys.size,
      workpad_row_count: workKeys.size,
      material_profile_row_count: materialKeys.size,
      workpad_only_row_count: workKeys.size - materialKeys.size,
      accepted_batch_count: acceptedBatches.length,
      batch_validation_count: batchValidations.length,
      stream_trace: streamTrace,
      batch_trace: asArray(route.batch_plan).map((batch) => ({
        batch_id: batch.batch_id,
        package_id: batch.package_id,
        stream_id: batch.stream_id,
        stream_type: batch.stream_type,
      ...
      row_trace: asArray(work.registry_rows).map((row) => ({
        registry_row_key: row.registry_row_key,
        Threat_ID: row.Threat_ID,
        package_id: row.package_id,
        stream_id: row.stream_id,
        ...
        semantic_evaluated: Boolean(row.semantic_evidence_application),
        workpad_only: Boolean(row.workpad_only)
      })),
      forensic_lock_gate_result: {
        status: failures.length ? "REPAIR_REQUIRED" : "PASS",
        failures,
        dynamic_registry_count_verified: failures.length === 0,
        compound_identity_reconciled: failures.length === 0,
...
function buildDeterministicSpine(row = {}) {
  ...
  return {
    ...
    stream_id: row.stream_id,
    stream_type: row.stream_type,
    Archetype: registry.Archetype || registry.FIELD21 || "",
    Subcategory: registry.FIELD22 || registry.Subcategory || "",
    Surface: registry.Surface || "",
    authority_anchors: [registry.Authority_IN, registry.Authority_EU, registry.Authority_US].filter(Boolean),
    ...
    base_fp_mechanism: registry.FP_Mechanism || "",
    remediation_source: registry.Lex_Nova_Fix || "",
    review_route: registry.review_route || "QUALIFIED_REVIEW",
    Hunter_Trigger: registry.Hunter_Trigger || "",
    Hunter_Trigger_Parsed: registry.Hunter_Trigger_Parsed || null,
    route: row.route,
    route_reason: row.route_reason,
    matched_activity_references: [...asArray(row.matched_activity_references)]
  };
...
function buildMaterialProjection(routeRow, semanticRow) {
  const spine = buildDeterministicSpine(routeRow);
  const evaluationStatus = deriveFinalMaterialStatus(semanticRow.status_inputs);
  const row = {
    Threat_ID: spine.Threat_ID,
    Threat_Name: spine.Threat_Name,
    target_match: semanticRow.target_match,
    evaluation_status: evaluationStatus,
    basis_proof: semanticRow.basis_proof,
    control_exclusion_evaluation: semanticRow.control_exclusion_evaluation,
    evidence_source_basis: semanticRow.evidence_source_basis,
    fp_mechanism: semanticRow.applied_fp_mechanism || spine.base_fp_mechanism,
    Archetype: spine.Archetype,
    Subcategory: spine.Subcategory,
    Surface: spine.Surface,
    authority_anchors: spine.authority_anchors,
    Pain_Tier: spine.Pain_Tier,
    Pain_Depth: spine.Pain_Depth,
    Pain_Category: spine.Pain_Category,
    Legal_Pain: spine.Legal_Pain,
    remediation: spine.remediation_source,
    review_route: spine.review_route,
    row_limitations: semanticRow.row_limitations
  };
  return Object.fromEntries(MATERIAL_FIELDS.map((field) => [field, row[field] ?? ""]));
}

function assertBatchIsolation(batch, rows) {
  if (!batch?.batch_id || !batch.package_id || !batch.stream_id || !batch.stream_type) throw new Error("M11_BATCH_IDENTITY_MISSING");
  if (rows.length !== Number(batch.row_count)) throw new Error(`M11_BATCH_ROW_COUNT_MISMATCH:${batch.batch_id}`);
  if (rows.length < 1 || rows.length > 15) throw new Error(`M11_BATCH_ROW_LIMIT_INVALID:${batch.batch_id}:${rows.length}`);
  for (const row of rows) {
    if (row.package_id !== batch.package_id || row.stream_id !== batch.stream_id || row.stream_type !== batch.stream_type) throw new Error(`M11_BATCH_STREAM_ESCAPE:${batch.batch_id}:${row.registry_row_key}`);
  }
  if (new Set(rows.map((row) => row.Threat_ID)).size !== rows.length) throw new Error(`M11_BATCH_CANONICAL_ID_COLLISION:${batch.batch_id}`);
}
...
function countBy(rows, selector) {
  const counts = {};
  for (const row of rows) { const key = selector(row); counts[key] = (counts[key] || 0) + 1; }
