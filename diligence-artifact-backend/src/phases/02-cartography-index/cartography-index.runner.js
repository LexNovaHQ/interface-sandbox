...
const PROFILE_INDEXES = Object.freeze([]);
const PROFILE_BY_ROOT = Object.freeze({
  ...
  technical_docs_api: ["activity_profile_source_index"],
  docs_api_data_flow: ["activity_profile_source_index"],
  integrations_ecosystem: ["activity_profile_source_index"],
  pricing_commercial_availability: ["activity_profile_source_index"],
  use_case_customer_industry: ["activity_profile_source_index"],
  support_help_resources: ["activity_profile_source_index"]
});
const TARGET_ACTIVITY_DOC_TYPES = Object.freeze({
  terms_of_service: ["activity_profile_source_index"],
...
export async function runCartographyIndexJob({ run, internalJobId, contract, readArtifacts, readArtifact, saveArtifact } = {}) {
  ...
  for (const artifactName of job.writes) {
    ...
    await saveArtifact({ artifact_name: artifactName, artifact, lock_status: artifact.status || artifact.lock_status || "LOCKED" });
...
function buildSourceInventory({ run, artifacts }) {
  ...
  for (const artifactName of rootEntries) {
    ...
    if (!sources.length) rows.push(sourceInventoryRow({ run, artifactName, sourceRoot, source: {}, sourceClass: "common_root", sourceStatus: "EMPTY_OR_MISSING" }));
    for (const source of sources) rows.push(sourceInventoryRow({ run, artifactName, sourceRoot, source, sourceClass: "common_root", sourceStatus: "AVAILABLE" }));
  }
...
function buildCompiledIndexes({ run, artifacts }) {
  ...
  const dataPrivacyNavigationIndex = unwrap(artifacts.data_privacy_navigation_index);
  ...
  const validation = validateCompiledIndexes({ inventory, spine, matrix, overlay, dataPrivacyNavigationIndex, routes });
  const cartographyIndex = baseArtifact({ run, artifact_type: "cartography_index", layer_id: "P2-L5", index_version: "phase2_cartography_index_v3_dedicated_2a_2b_2c_2d_indexes", execution_mode: "DETERMINISTIC_LED_SEMANTIC_GUIDED_NEW_INPUT_CONTRACTS", navigation_only: true, source_inventory_ref: "cartography_source_inventory", locator_spine_ref: "cartography_locator_spine", profile_route_matrix_ref: "cartography_profile_route_matrix", semantic_navigation_overlay_ref: "cartography_semantic_navigation_overlay", legal_cartography_index_ref: "legal_cartography_index", legal_signal_derivation_profile_ref: "legal_signal_derivation_profile", target_profile_source_index_ref: "target_profile_source_index", domain_derivation_source_index_ref: "domain_derivation_source_index", activity_profile_source_index_ref: "activity_profile_source_index", data_privacy_navigation_index_ref: "data_privacy_navigation_index", profile_indexes: { target_profile_source_index: indexRef(artifacts.target_profile_source_index), domain_derivation_source_index: indexRef(artifacts.domain_derivation_source_index), activity_profile_source_index: indexRef(artifacts.activity_profile_source_index), data_privacy_navigation_index: indexRef(dataPrivacyNavigationIndex) }, target_profile_source_index_externalized_to_p2a: true, domain_derivation_source_index_externalized_to_p2b: true, activity_profile_source_index_externalized_to_p2c: true, data_privacy_navigation_index_externalized_to_p2d: true, downstream_free_read_forbidden: true, contains_lossless_text: false, contains_excerpts: false, contains_summaries: false, contains_profile_answers: false, contains_legal_or_compliance_conclusions: false });
  return { cartography_index: cartographyIndex, cartography_validation_manifest: validation };
}

function indexRef(index = {}) { return { artifact_type: index?.artifact_type || "MISSING", route_count: index?.route_count || index?.deterministic_navigation_spine?.data_source_routes?.length || 0, present: Boolean(index && typeof index === "object" && index.artifact_type) }; }
function profileIndexArtifact({ run, name, routes }) { return baseArtifact({ run, artifact_type: name, layer_id: "P2-L5", index_version: "phase2_cartography_index_v2_p2a_target_index_externalized", execution_mode: "DETERMINISTIC_COMPILED_FROM_CARTOGRAPHY", navigation_only: true, route_count: routes.length, route_groups: groupRoutes(routes), routes: routes.map(stripRouteForIndex), contains_lossless_text: false, contains_excerpts: false, contains_summaries: false, contains_profile_answers: false, contains_legal_or_compliance_conclusions: false }); }
function validateCompiledIndexes({ inventory, spine, matrix, overlay, dataPrivacyNavigationIndex, routes }) { const failures = []; const warnings = []; const locatorIds = new Set((spine.locators || []).map((row) => row.locator_id)); const inventoryArtifacts = new Set((inventory.rows || []).map((row) => row.source_artifact)); const overlaysByRoute = new Set((overlay.overlays || []).map((row) => row.route_id)); for (const route of matrix.routes || []) { if (!route.locator_id || !locatorIds.has(route.locator_id)) failures.push(`ROUTE_MISSING_VALID_LOCATOR:${route.route_id || "missing"}`); if (!route.lossless_text_pointer?.artifact_name || !inventoryArtifacts.has(route.lossless_text_pointer.artifact_name)) failures.push(`ROUTE_MISSING_LOSSLESS_TEXT_POINTER:${route.route_id || "missing"}`); if (!overlaysByRoute.has(route.route_id)) warnings.push(`ROUTE_MISSING_SEMANTIC_OVERLAY:${route.route_id || "missing"}`); } const compiled = { cartography_source_inventory: inventory, cartography_locator_spine: spine, cartography_profile_route_matrix: matrix, cartography_semantic_navigation_overlay: overlay }; const dataRoutes = dataPrivacyNavigationIndex?.deterministic_navigation_spine?.data_source_routes || []; if (containsForbiddenEvidence(compiled)) failures.push("INDEX_CONTAINS_FORBIDDEN_EVIDENCE_OR_SUMMARY_FIELD"); if (dataPrivacyNavigationIndex?.artifact_type !== "data_privacy_navigation_index") failures.push("DATA_PRIVACY_NAVIGATION_INDEX_NOT_RESTORED"); if (!dataPrivacyNavigationIndex?.semantic_navigation_overlay?.batch_navigation_pointers?.length) failures.push("DATA_PRIVACY_NAVIGATION_INDEX_MISSING_BATCH_POINTERS"); if (dataRoutes.length !== 5) failures.push("DATA_PRIVACY_NAVIGATION_INDEX_MISSING_DATA_SOURCE_ROUTES"); for (const route of dataRoutes) if (route.present !== true) warnings.push(`DATA_PRIVACY_NAVIGATION_INDEX_DATA_ROUTE_LIMITED:${route.route_id}`); return baseArtifact({ run: {}, artifact_type: "cartography_validation_manifest", layer_id: "P2-L5", status: failures.length ? "REPAIR_REQUIRED" : "LOCKED", validation_status: failures.length ? "FAIL" : "PASS", failures, warnings, route_count: routes.length, profile_index_counts: {}, restored_artifacts_verified: { target_profile_source_index_externalized_to_p2a: true, domain_derivation_source_index_externalized_to_p2b: true, activity_profile_source_index_externalized_to_p2c: true, data_privacy_navigation_index_externalized_to_p2d: dataPrivacyNavigationIndex?.artifact_type === "data_privacy_navigation_index", legal_cartography_index: true, legal_signal_derivation_profile: true, data_privacy_navigation_data_source_routes_present: dataRoutes.length === 5 }, hard_failures_enforced: CARTOGRAPHY_INDEX_CONTRACT.validation_hard_failures }); }
...
function sourceInventoryRow({ run, artifactName, sourceRoot, source, sourceClass, sourceStatus, docType = "", legalDocInventoryLink = "" }) { const sourceId = source.source_id || source.manifest_id || legalDocInventoryLink || artifactName; return { run_id: run?.run_id || "", source_artifact: artifactName, source_root: sourceRoot, source_id: sourceId, source_url: source.canonical_url || source.url || source.final_url || "", doc_type: docType || source.doc_type || "", source_class: sourceClass, available_for_indexing: sourceStatus === "AVAILABLE" || sourceStatus === "EXTRACTED" || sourceStatus === "LOCKED", source_status: sourceStatus, neutral_bucket_tags: neutralBucketTagsForSource({ sourceRoot, source }), lossless_text_pointer: { artifact_name: artifactName, source_id: sourceId, text_field: "lossless_text" }, legal_doc_inventory_link: doc.doc_id || artifactName }; }
function locatorRow({ row, text, unitType, headingPath, start, end }) { const normalizedHeadingPath = (headingPath || []).map(cleanHeading).filter(Boolean); return { source_inventory_id: row.source_inventory_id, source_artifact: row.source_artifact, source_root: row.source_root, source_id: row.source_id, source_url: row.source_url, doc_type: row.doc_type, unit_type: unitType, heading_path: normalizedHeadingPath, char_range: [start, end], text_hash: createHash("sha256").update(String(text || "").slice(start, end)).digest("hex"), lossless_text_pointer: { artifact_name: row.source_artifact, source_id: row.source_id, text_field: "lossless_text", char_range: [start, end] }, locator_contains_text: false }; }
...
function routeProfilesForLocator(locator) { if (locator.source_artifact?.startsWith("legal_doc_")) return TARGET_ACTIVITY_DOC_TYPES[locator.doc_type || "other"] || []; return PROFILE_BY_ROOT[locator.source_root] || []; }
...
function defaultPriority({ locator, profileIndex }) { if (profileIndex === "activity_profile_source_index" && ["product_service", "platform_feature_solution", "technical_docs_api", "docs_api_data_flow"].includes(locator.source_root)) return "PRIMARY"; return "SECONDARY"; }
function semanticLabelsForRoute(route) { const text = `${route.source_root} ${route.doc_type} ${(route.heading_path || []).join(" ")} ${route.source_url}`.toLowerCase(); const labels = []; if (/api|docs|developer|sdk|webhook|authentication|model/.test(text)) labels.push("technical_docs_route_candidate"); if (/pricing|plan|enterprise|commercial|sales/.test(text)) labels.push("commercial_route_candidate"); if (/product|feature|solution|platform|integration/.test(text)) labels.push("activity_route_candidate"); return labels.length ? [...new Set(labels)] : ["generic_route_candidate"]; }
...
function fieldFamiliesForRoute(route, labels) { const families = new Set(); if (route.profile_index === "activity_profile_source_index") families.add("ACTIVITY"); for (const label of labels) { if (label.includes("technical")) families.add("TECH"); if (label.includes("commercial")) families.add("COMMERCIAL"); } return [...families]; }
function overlayTagsForRoute(route) { const tags = new Set(); const text = `${route.source_root} ${route.doc_type} ${(route.heading_path || []).join(" ")} ${route.source_url}`.toLowerCase(); if (/ai|model|agent|llm/.test(text)) tags.add("ai-native"); if (/api|developer|sdk|webhook/.test(text)) tags.add("developer-tools"); return [...tags]; }
...
function buildInventoryGaps({ artifacts, rootEntries, rows }) { const gaps = []; if (!rootEntries.length) gaps.push({ gap_id: "P2.GAP.ROOTS", status: "MISSING", message: "No lossless_root__ artifacts were available for cartography indexing." }); if (!rows.some((row) => row.source_class === "legal_doc")) gaps.push({ gap_id: "P2.GAP.LEGAL_DOCS", status: "MISSING_OR_NOT_DISCOVERED", message: "No legal_doc inventory rows were available for M9 legal cartography indexing." }); if (!artifacts.source_discovery_handoff) gaps.push({ gap_id: "P2.GAP.HANDOFF", status: "MISSING", message: "source_discovery_handoff was not available." }); return gaps; }
