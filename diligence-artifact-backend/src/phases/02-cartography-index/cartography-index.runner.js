...
const PROFILE_INDEXES = Object.freeze(["activity_profile_source_index"]);
...
function buildCompiledIndexes({ run, artifacts }) {
  ...
  const dataPrivacyNavigationIndex = buildPhase7DataPrivacyNavigationIndex({ dapRegistryManifest: { artifact_type: "dap_registry_manifest", material_rules: [] }, strategicDerivationMatrix: { semantic_batch_plan: PHASE7_DAP_SEMANTIC_BATCH_PLAN }, artifacts });
  const validation = validateCompiledIndexes({ inventory, spine, matrix, overlay, profileIndexes, dataPrivacyNavigationIndex, routes });
  const cartographyIndex = baseArtifact({ run, artifact_type: "cartography_index", layer_id: "P2-L5", index_version: "phase2_cartography_index_v2_p2a_target_index_externalized", execution_mode: "DETERMINISTIC_LED_SEMANTIC_GUIDED_NEW_INPUT_CONTRACTS", navigation_only: true, source_inventory_ref: "cartography_source_inventory", locator_spine_ref: "cartography_locator_spine", profile_route_matrix_ref: "cartography_profile_route_matrix", semantic_navigation_overlay_ref: "cartography_semantic_navigation_overlay", legal_cartography_index_ref: "legal_cartography_index", legal_signal_derivation_profile_ref: "legal_signal_derivation_profile", target_profile_source_index_ref: "target_profile_source_index", data_privacy_navigation_index_ref: "data_privacy_navigation_index", profile_indexes: Object.fromEntries(PROFILE_INDEXES.map((name) => [name, { artifact_name: name, route_count: profileIndexes[name].route_count }])), target_profile_source_index_externalized_to_p2a: true, downstream_free_read_forbidden: true, contains_lossless_text: false, contains_excerpts: false, contains_summaries: false, contains_profile_answers: false, contains_legal_or_compliance_conclusions: false });
