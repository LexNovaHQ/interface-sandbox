...
function profileIndexArtifact({ run, name, routes }) { return baseArtifact({ run, artifact_type: name, layer_id: "P2-L5", index_version: "phase2_cartography_index_v1", execution_mode: "DETERMINISTIC_COMPILED_FROM_CARTOGRAPHY", navigation_only: true, route_count: routes.length, route_groups: groupRoutes(routes), routes: routes.map(stripRouteForIndex), contains_lossless_text: false, contains_excerpts: false, contains_summaries: false, contains_profile_answers: false, contains_legal_or_compliance_conclusions: false }); }
function validateCompiledIndexes({ inventory, spine, matrix, overlay, profileIndexes, dataPrivacyNavigationIndex, routes }) {
  const failures = [];
  const warnings = [];
...
function defaultPriority({ locator, profileIndex }) { if (profileIndex === "activity_profile_source_index" && ["product_service", "platform_feature_solution", "technical_docs_api_developer"].includes(locator.source_root)) return "PRIMARY"; if (profileIndex === "target_profile_source_index" && ["homepage_landing", "about_company", "legal_identity_notice"].includes(locator.source_root)) return "PRIMARY"; return "SECONDARY"; }
function semanticLabelsForRoute(route) { const text = `${route.source_root} ${route.doc_type} ${(route.heading_path || []).join(" ")} ${route.source_url}`.toLowerCase(); const labels = []; if (/api|docs|developer|sdk|webhook|authentication|model/.test(text)) labels.push("technical_docs_route_candidate"); if (/pricing|plan|enterprise|commercial|sales/.test(text)) labels.push("commercial_route_candidate"); if (/product|feature|solution|platform|integration/.test(text)) labels.push("activity_route_candidate"); if (/about|company|contact|legal-notice|imprint/.test(text)) labels.push("target_identity_route_candidate"); return labels.length ? [...new Set(labels)] : ["generic_route_candidate"]; }
...
function fieldFamiliesForRoute(route, labels) { const families = new Set(); if (route.profile_index === "target_profile_source_index") families.add("TARGET"); if (route.profile_index === "activity_profile_source_index") families.add("ACTIVITY"); for (const label of labels) { if (label.includes("technical")) families.add("TECH"); if (label.includes("commercial")) families.add("COMMERCIAL"); } return [...families]; }
