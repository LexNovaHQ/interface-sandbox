...
export function buildPhaseRouteRuntimeReadPlan({ internalJobId, phaseRoutingManifest } = {}) {
  ...
  const routerArtifactReads = deliveryMode === P2G_SOURCE_BUCKET_DELIVERY_MODE ? unique([
    ...(route.required_index_artifacts || []),
    ...staticPrimaryEvidence,
    ...(route.allowed_legal_artifacts || [])
  ]) : [];
  const consumerContextReads = deliveryMode === P2G_SOURCE_BUCKET_DELIVERY_MODE ? unique([
...
export async function readPhaseRouteRuntimePacket({ internalJobId, readArtifacts, consumerAgentId } = {}) {
  ...
  return Object.freeze({
    route: plan,
    artifacts: Object.freeze({ ...routedArtifacts, [P2G_PHASE_ROUTING_ARTIFACTS.manifest]: manifest, phase_route_runtime_packet: runtimePacket })
  });
}
...
async function readDynamicM11BatchInputs({ readArtifacts, consumerAgentId, artifacts }) {
  ...
  return {
    ...
    m12_global_dynamic_artifact_manifest: {
      ...
      expected_batch_artifacts: batchNames,
      expected_batch_validation_artifacts: validationNames,
