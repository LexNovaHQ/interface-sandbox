...
export async function readPhaseRouteRuntimePacket({ internalJobId, readArtifacts, consumerAgentId } = {}) {
  ...
  const routerArtifacts = plan.router_artifact_reads.length ? await readArtifacts({ reads: plan.router_artifact_reads, agent_id: ROUTER_AGENT_ID, strict: true }) : {};
  ...
  const runtimePacket = Object.freeze({ artifact_type: "phase_route_runtime_packet", routing_authority: ROUTING_AUTHORITY, internal_job_id: internalJobId, route_id: plan.route_id, bucket_id: plan.bucket_id, delivery_mode: plan.delivery_mode, source_bucket_delivered: sourceBucketDelivered, lossless_evidence_role: sourceBucketDelivered ? "PRIMARY_EVIDENCE" : "NOT_DELIVERED_TO_DERIVED_ONLY_JOB", index_role: sourceBucketDelivered ? "MANDATORY_NAVIGATION_MAP_INTO_PRIMARY_EVIDENCE" : "DERIVED_ONLY_PACKET_NO_PRIMARY_SOURCE_NAVIGATION", navigation_rule: P2G_ROUTING_DOCTRINE, direct_lossless_as_fallback_allowed: false, free_corpus_read_allowed: false, profile_forensics_inputs_allowed: false, router_artifact_reads: plan.router_artifact_reads, consumer_context_reads: plan.consumer_context_reads, expanded_legal_document_artifacts: Object.freeze(Object.keys(expandedLegalDocuments)), dynamic_inputs: plan.dynamic_inputs, delivered_artifacts: Object.freeze(deliveredNames) });
...
async function readDynamicM11BatchInputs({ readArtifacts, consumerAgentId, artifacts }) {
  ...
  return { m11_batch_artifacts: batchNames.filter((name) => loadedBatches?.[name]).map((name) => ({ artifact_name: name, batch_id: name.replace(/^exposure_registry_batch__/, ""), artifact: loadedBatches[name] })), m12_batch_validation_artifacts: validationNames.filter((name) => loadedValidations?.[name]).map((name) => ({ artifact_name: name, batch_id: name.replace(/^exposure_registry_batch_validation__/, ""), artifact: loadedValidations[name] })), m12_global_dynamic_artifact_manifest: { batch_count: batchPlan.length, expected_batch_artifacts: batchNames, expected_batch_validation_artifacts: validationNames, missing_batch_artifacts: missingBatchArtifacts, missing_batch_validation_artifacts: missingBatchValidationArtifacts, routing_authority: ROUTING_AUTHORITY } };
}

function assertManifest(manifest = {}) { if (!manifest || typeof manifest !== "object" || Array.isArray(manifest)) throw new Error("P2G_RUNTIME_MANIFEST_MISSING"); if (manifest.artifact_type !== P2G_PHASE_ROUTING_ARTIFACTS.manifest) throw new Error("P2G_RUNTIME_MANIFEST_WRONG_ARTIFACT_TYPE"); if (manifest.routing_authority !== ROUTING_AUTHORITY) throw new Error("P2G_RUNTIME_MANIFEST_WRONG_AUTHORITY"); if (manifest.doctrine?.lossless_evidence_is_primary !== true) throw new Error("P2G_RUNTIME_MANIFEST_LOSSLESS_PRIMARY_MISSING"); if (manifest.doctrine?.index_role !== "MANDATORY_NAVIGATION_MAP_INTO_PRIMARY_EVIDENCE") throw new Error("P2G_RUNTIME_MANIFEST_INDEX_ROLE_MISSING"); if (manifest.doctrine?.direct_lossless_as_fallback_allowed !== false) throw new Error("P2G_RUNTIME_MANIFEST_FALLBACK_FORBIDDEN"); if (manifest.doctrine?.free_corpus_read_allowed !== false) throw new Error("P2G_RUNTIME_MANIFEST_FREE_CORPUS_FORBIDDEN"); }
function assertExactRoutedArtifacts(artifacts = {}, allowedArtifactNames = [], routeId) { const allowed = new Set(allowedArtifactNames); for (const key of Object.keys(artifacts || {})) { if (!allowed.has(key)) throw new Error(`P2G_RUNTIME_UNDECLARED_ARTIFACT_DELIVERED:${routeId}:${key}`); if (!VIRTUAL_DYNAMIC_ARTIFACTS.includes(key) && FORENSICS_MARKERS.some((marker) => String(key).includes(marker))) throw new Error(`P2G_RUNTIME_FORENSICS_ARTIFACT_DELIVERED:${routeId}:${key}`); } for (const key of allowed) if (!(key in artifacts)) throw new Error(`P2G_RUNTIME_DECLARED_ARTIFACT_NOT_DELIVERED:${routeId}:${key}`); }
