  ...
  try {
    ...
    for (const file of fs.readdirSync(OBLIGATION_CATALOG_DIR).sort()) {
      ...
      try {
        ...
        // Malformed catalogs are intentionally skipped. Phase 2 must remain navigation-safe and non-blocking.
      }
    }
  } catch {
    return catalogs;
...
export function buildDomainControlObligationSemanticProfile({ deterministicMap = {} } = {}) {
  ...
  const warnings = [];
  const obligationFamilyRouting = buildObligationFamilyRouting({
    controlSourceRoutes: det.control_source_routes || [],
    legalIndexRoutes: det.legal_index_routes || [],
    warnings
  });
...
function buildControlSourceRoutes(artifacts = {}) {
  return CONTROL_SOURCE_ROUTE_CATALOG.map((entry) => Object.freeze({
    ...
    pointers: Object.freeze((entry.source_artifacts || []).map((artifactName) => Object.freeze({
      artifact_name: artifactName,
      present: Boolean(artifacts[artifactName]),
      navigation_scope: "phase1_source_contract_via_index"
    }))),
    contains_lossless_text: false,
    contains_obligation_posture: false
  }));
...
function buildLegalIndexRoutes(artifacts = {}) {
  return DOMAIN_CONTROL_OBLIGATION_LEGAL_INDEX_INPUTS.map((artifactName) => Object.freeze({
    route_id: `${LEGAL_ROUTE_ID_PREFIX}${artifactName}`,
    artifact_name: artifactName,
    route_class: "LEGAL_INDEX_SELECTIVE_LOCATOR_ROUTE",
    present: Boolean(artifacts[artifactName]),
    navigation_scope: "selective_locator_only",
    contains_lossless_text: false,
    contains_obligation_posture: false
  }));
}
...
function buildObligationFamilyRouting({ controlSourceRoutes = [], legalIndexRoutes = [], warnings = [] } = {}) {
  ...
  const allowedLocators = new Set(LOCATOR_FAMILY_REGISTRY);
  ...
  for (const catalog of OBLIGATION_CATALOGS) {
    ...
    for (const family of families) {
      ...
      const locatorFamilies = [];
      for (const locator of Array.isArray(family.locator_families) ? family.locator_families : []) {
        if (allowedLocators.has(locator)) locatorFamilies.push(locator);
        else warnings.push(`UNKNOWN_LOCATOR_FAMILY:${catalog.domain_id || "unknown"}:${family.id || "unknown"}:${locator}`);
      }
      const shellTargets = [];
...
function buildObligationShellLocatorInventory(obligationFamilyRouting = []) {
  return OBLIGATION_SHELL_FIELDS.map((field) => {
    ...
    return Object.freeze({
      shell_field_id: field,
      shell_field_key: `obligation_record.${field}`,
      downstream_owner: "DOMAIN_CONTROL_OBLIGATION_PROFILE",
      locator_families: Object.freeze([...locatorFamilies].sort()),
      action: "LOCATE_ONLY",
      derived_value_forbidden: true,
      contains_obligation_posture: false
    });
...
function buildAccessGapLedger(controlSourceRoutes = []) {
  return controlSourceRoutes
    .filter((route) => !(route.pointers || []).some((pointer) => pointer.present === true))
    .map((route) => Object.freeze({
      gap_id: `DCONI-GAP-${route.route_code}`,
      route_id: route.route_id,
      source_artifacts: Object.freeze([...(route.source_artifacts || [])]),
      gap_status: "SOURCE_NOT_PRESENT_IN_PHASE1_INPUTS",
      navigation_gap_only: true,
      obligation_posture_conclusion: false
    }));
}
