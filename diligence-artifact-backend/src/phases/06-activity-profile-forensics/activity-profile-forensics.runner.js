...
import { buildM8DeterministicFeatureForensics } from "../../deterministic-profile-forensics.js";

export const ACTIVITY_PROFILE_FORENSICS_RUNNER_STATUS = Object.freeze({
  phase_runner: "activity-profile-forensics.runner",
  central_phase_id: ACTIVITY_PROFILE_FORENSICS_CONTRACT.central_phase_id,
  public_label: ACTIVITY_PROFILE_FORENSICS_CONTRACT.public_label,
  ...
  source_helper: ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.source_helper,
  writes: [...ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.writes],
  reads: [...ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.reads]
});
...
export async function runActivityProfileForensicsPhase({ run, internalJobId = "M8_TARGET_FEATURE_PROFILE_FORENSICS", contract, readArtifacts, saveArtifact } = {}) {
  ...
  return {
    ...
    model_usage: "NONE_DETERMINISTIC",
    activity_profile_forensics_phase_runner_used: true,
    source_helper: ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.source_helper,
    validator: ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.validator
  };
}

function assertRuntimeContract(contract = {}) {
  if (contract.central_phase_id !== ACTIVITY_PROFILE_FORENSICS_CONTRACT.central_phase_id) throw new Error("ACTIVITY_PROFILE_FORENSICS_CONTRACT_MISMATCH");
  if (contract.public_label !== ACTIVITY_PROFILE_FORENSICS_CONTRACT.public_label) throw new Error("ACTIVITY_PROFILE_FORENSICS_LABEL_MISMATCH");
  if (JSON.stringify(contract.reads || []) !== JSON.stringify(ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.reads)) throw new Error("ACTIVITY_PROFILE_FORENSICS_READS_MISMATCH");
  if (JSON.stringify(contract.writes || []) !== JSON.stringify(ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.writes)) throw new Error("ACTIVITY_PROFILE_FORENSICS_WRITES_MISMATCH");
}

function assertAllowedRuntimeArtifacts(artifacts = {}) {
  const allowed = new Set(ACTIVITY_PROFILE_FORENSICS_CONTRACT.deterministic_job.reads);
  for (const key of Object.keys(artifacts)) if (!allowed.has(key)) throw new Error(`ACTIVITY_PROFILE_FORENSICS_UNEXPECTED_RUNTIME_ARTIFACT:${key}`);
}
