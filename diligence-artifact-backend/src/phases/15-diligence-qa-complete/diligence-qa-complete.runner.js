...
export const DILIGENCE_QA_COMPLETE_RUNNER_VERSION = "phase15_diligence_qa_complete_runner.v1";
...
export function runDiligenceQaComplete({
  ...
  const receipt = freezeImmutableArtifact({
    ...receiptBody,
    immutable_hash: hashImmutableArtifact(receiptBody)
  });

  if (errors.length) {
    throw new Error(`DILIGENCE_QA_COMPLETE_FAILED:${errors.join("|")}`);
  }
  return receipt;
}
...
function validateDocuments({ authority, document_activation_manifest, errors, warnings }) {
  ...
  for (const document of documents) {
    ...
    if (document.status === "ACTIVE") {
      ...
      if (!(document.placeholder_bindings || []).length && !(document.clause_actions || []).length && document.blank_operational_instrument_only !== true && document.inherit_only !== true) {
        warnings.push(`ACTIVE_DOCUMENT_HAS_NO_DIRECT_QR_BINDINGS:${document.document_id}`);
      }
    }
  }
...
function validateReviewReadyBoundary({ qualified_review_submission, document_activation_manifest, errors }) {
  for (const [name, boundary] of [
    ["submission", qualified_review_submission.review_ready_boundary],
    ["document_manifest", document_activation_manifest.review_ready_boundary]
  ]) {
