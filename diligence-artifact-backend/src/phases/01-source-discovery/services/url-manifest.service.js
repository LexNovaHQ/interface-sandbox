...
import { ADAPTERS, COMMON_ROOTS, COMMON_ROOT_ARTIFACT_NAMES, LEGAL_DOC_RULES, ROOT_TRAVERSAL_POLICY, adapterExpansionPathsFromPreflight, emptyNeutralBuckets, legalDocTypeFromUrlOrRoute, neutralBucketsForSource, noLockNoNarrow, primaryNeutralBucket, selectedAdaptersFromPreflight } from "./source-discovery-taxonomy.service.js";
...
const DATA_FLOW_SIGNALS = ["upload", "storage", "retention", "delete", "deletion", "export", "webhook", "connector", "auth", "authentication", "permission", "audit", "log", "subprocessor", "training", "customer-data", "customer-content", "file", "image", "audio", "document"];
...
export async function buildSourceUrlManifestArtifact({ run, preflightContext = {} }) {
  ...
  return {
    deduped_url_manifest: {
      ...
      taxonomy_version: "PHASE1_AGNOSTIC_URL_MANIFEST_v2_FULL_ROOT_MATRIX",
...
function regulatedTokenPresent(path) { return /(credit|loan|health|clinical|hiring|recruit|legal|biometric|kyc|children|minor|student|payment|banking|underwriting|medical|patient)/i.test(path); }
