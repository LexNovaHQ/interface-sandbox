...
import {
  ADAPTERS,
  API_DATA_FLOW_FAMILY_SEGMENTS,
  COMMON_ROOTS,
  COMMON_ROOT_ARTIFACT_NAMES,
  LEGAL_DOC_RULES,
  ROOT_TRAVERSAL_POLICY,
  adapterExpansionPathsFromPreflight,
  allDomainHintPaths,
  emptyNeutralBuckets,
  legalDocTypeFromUrlOrRoute,
  loadedDomainHintPacksSummary,
  neutralBucketsForSource,
  noLockNoNarrow,
  primaryNeutralBucket,
  selectedAdaptersFromPreflight
} from "./source-discovery-taxonomy.service.js";
...
function classifyCandidate(candidate, rootHost) {
  ...
  classifyRegulatoryLicensingStatus(ctx, matches);
  classifyGrievanceComplaints(ctx, matches);
  classifySupportHelpResources(ctx, matches);
  classifyLegalDocumentSurfaces(ctx, matches);
  return mergeMatchesByRoot(matches);
}

function classifyHomepage({ path, subdomain }, matches) {
  if (!subdomain && path === "/") add(matches, "homepage_landing", "primary_homepage", "target_boundary", "PRIMARY", "NONE", "Exact submitted/root host homepage.", ["TARGET_IDENTITY_SIGNAL", "COMMERCIAL_POSITIONING_SIGNAL"]);
}
...
function classifyContactNotice({ path }, matches) {
  if (!matchAny(path, ["/contact", "/contact-us", "/support/contact", "/legal", "/privacy/contact"])) return;
  const route = path.includes("privacy") ? "privacy_contact" : path.includes("legal") ? "legal_contact" : path.includes("support") ? "support_contact" : "general_contact";
  add(matches, "contact_notice", route, "contact_notice", "PRIMARY", "NONE", "Contact notice matrix route matched.", ["CONTACT_NOTICE_SIGNAL", route.includes("privacy") ? "DATA_PROCESSING_SIGNAL" : null, route.includes("legal") ? "LEGAL_NOTICE_SIGNAL" : null].filter(Boolean));
}
...
function classifyPlatformFeatureSolution({ path }, matches) {
  ...
  const first = path.split("/").filter(Boolean)[0] || "platform";
  const route = pathDepth(path) <= 1 ? `${first}_root` : first === "features" ? "feature_child" : first === "solutions" ? "solution_child" : "platform_child";
  add(matches, "platform_feature_solution", route, "product_activity", "PRIMARY", "NONE", "Platform/feature/solution matrix route matched.", ["PRODUCT_ACTIVITY_SIGNAL", path.includes("automation") ? "AI_MECHANISM_SIGNAL" : null].filter(Boolean));
}
...
function classifyIntegrationsEcosystem({ path }, matches) {
  if (!matchAny(path, ["/integrations", "/connectors", "/apps", "/marketplace"])) return;
  const route = pathDepth(path) <= 1 ? path.split("/").filter(Boolean)[0] + "_root" : "integration_child";
  add(matches, "integrations_ecosystem", route, "technical_evidence", "SECONDARY", "SECONDARY_SUPPORT", "Integration/ecosystem matrix route matched.", ["API_INTEGRATION_SIGNAL", hasDataFlowSignal(path) ? "DATA_FLOW_SIGNAL" : null].filter(Boolean));
}
function classifyPricingCommercialAvailability({ path }, matches) {
  if (!matchAny(path, ["/pricing", "/api-pricing", "/plans", "/enterprise", "/contact-sales"])) return;
  const route = path.includes("api-pricing") ? "api_pricing" : path.includes("enterprise") ? "enterprise_sales" : path.includes("contact-sales") ? "contact_sales" : path.includes("plans") ? "plans_page" : "pricing_page";
  add(matches, "pricing_commercial_availability", route, "commercial_terms", "SECONDARY", "SECONDARY_SUPPORT", "Pricing/commercial availability matrix route matched.", ["COMMERCIAL_AVAILABILITY_SIGNAL"]);
}
function classifyUseCaseCustomerIndustry({ path }, matches) {
  if (!matchAny(path, ["/use-cases", "/industries", "/customers", "/stories", "/case-studies"])) return;
  const route = path.includes("industr") ? pathDepth(path) <= 1 ? "industry_index" : "industry_child" : path.includes("customers") ? "customer_index" : path.includes("stories") || path.includes("case-studies") ? "customer_story" : pathDepth(path) <= 1 ? "use_case_index" : "use_case_child";
  add(matches, "use_case_customer_industry", route, "use_case_context", "SECONDARY", "SECONDARY_SUPPORT", "Use-case/customer/industry matrix route matched.", ["CUSTOMER_SEGMENT_SIGNAL", regulatedTokenPresent(path) ? "REGULATED_ACTIVITY_SIGNAL" : null].filter(Boolean));
}
function classifyPrivacyDataProcessing({ path }, matches) {
  if (!matchAny(path, ["/privacy", "/privacy-policy", "/privacy-center", "/data-protection", "/gdpr", "/data-processing", "/data-processing-agreement", "/data-processing-addendum", "/dpa", "/subprocessors", "/subprocessor", "/cookies", "/cookie-policy"])) return;
  const route = path.includes("subprocessor") ? "subprocessor_list" : path.includes("cookie") ? "cookie_policy" : path.includes("dpa") || path.includes("data-processing-agreement") || path.includes("data-processing-addendum") ? "dpa_or_data_processing_addendum" : path.includes("privacy-center") ? "privacy_center" : path.includes("gdpr") ? "gdpr_privacy_notice" : path.includes("data-processing") ? "data_processing_page" : "privacy_policy";
  add(matches, "privacy_data_processing", route, "data_processing", "PRIMARY", "NONE", "Privacy/data-processing matrix route matched.", ["DATA_PROCESSING_SIGNAL", "LEGAL_DOCUMENT_SIGNAL", route.includes("subprocessor") ? "VENDOR_PROCESSING_SIGNAL" : null].filter(Boolean));
}
function classifySecurityTrustCompliance({ path }, matches) {
  if (!matchAny(path, ["/security", "/security-center", "/data-security", "/trust", "/trust-center", "/compliance", "/compliance-center", "/soc-2", "/iso-27001", "/certifications"])) return;
  const route = path.includes("soc-2") ? "soc2_signal" : path.includes("iso-27001") ? "iso27001_signal" : path.includes("trust") ? "trust_center" : path.includes("compliance") || path.includes("certifications") ? "compliance_center" : pathDepth(path) <= 1 ? "security_overview" : "security_child";
  add(matches, "security_trust_compliance", route, "trust_compliance", "PRIMARY", "NONE", "Security/trust/compliance matrix route matched.", ["SECURITY_TRUST_SIGNAL", "DATA_PROCESSING_SIGNAL"]);
}
function classifyDataGovernanceControls({ path }, matches) {
  if (!matchAny(path, ["/customer-data", "/enterprise-privacy", "/data-residency", "/retention", "/deletion", "/data-export", "/data-deletion"])) return;
  const route = path.includes("residency") ? "data_residency" : path.includes("retention") ? "retention_controls" : path.includes("deletion") ? "deletion_controls" : path.includes("export") ? "data_export_controls" : path.includes("enterprise-privacy") ? "enterprise_privacy_controls" : "customer_data_controls";
  add(matches, "data_governance_controls", route, "data_governance", "PRIMARY", "NONE", "Data-governance controls matrix route matched.", ["DATA_GOVERNANCE_SIGNAL", "DATA_PROCESSING_SIGNAL"]);
}
function classifyAiSafetyTransparency({ path }, matches) {
  if (!matchAny(path, ["/responsible-ai", "/ai-policy", "/ai-transparency", "/transparency", "/safety", "/model-card", "/model-cards", "/model-details", "/usage-policy"])) return;
  const route = path.includes("model-card") ? pathDepth(path) <= 1 ? "model_card" : "model_card_child" : path.includes("model-details") ? "model_details" : path.includes("usage-policy") ? "usage_policy" : path.includes("safety") ? "safety_policy_or_page" : path.includes("transparency") ? "ai_transparency" : path.includes("ai-policy") ? "ai_policy" : "responsible_ai";
  add(matches, "ai_safety_transparency", route, "ai_governance", "PRIMARY", "NONE", "AI safety/transparency matrix route matched.", ["AI_MECHANISM_SIGNAL", "AI_SAFETY_TRANSPARENCY_SIGNAL", "LEGAL_DOCUMENT_SIGNAL"]);
}
function classifyRegulatoryLicensingStatus({ path }, matches) {
  if (!matchAny(path, ["/licenses", "/licence", "/licences", "/licensing", "/regulatory", "/regulation", "/regulatory-disclosures", "/regulatory-information", "/disclosures", "/registrations", "/authorised", "/authorized", "/bank-partners", "/partner-banks", "/banking-partners", "/sponsor-bank"])) return;
  const route = path.includes("bank-partner") || path.includes("partner-bank") || path.includes("sponsor-bank") || path.includes("banking-partner") ? "bank_partner_sponsor_bank" : path.includes("disclosure") ? "regulatory_disclosure" : path.includes("registration") ? "registration_disclosure" : path.includes("licen") ? "licensing_status" : "regulatory_information";
  add(matches, "regulatory_licensing_status", route, "regulatory_operating_context", "PRIMARY", "NONE", "Regulatory/licensing matrix route matched.", ["LICENSING_REGULATORY_SIGNAL", "REGULATED_ACTIVITY_SIGNAL", route.includes("bank") ? "COUNTERPARTY_INSTITUTION_SIGNAL" : null, route.includes("disclosure") ? "CONSUMER_DISCLOSURE_SIGNAL" : null].filter(Boolean));
}
function classifyGrievanceComplaints({ path }, matches) {
  if (!matchAny(path, ["/grievance", "/grievance-redressal", "/grievances", "/complaints", "/complaint", "/ombudsman", "/nodal-officer", "/grievance-officer", "/raise-a-complaint", "/customer-grievance"])) return;
  const route = path.includes("ombudsman") ? "ombudsman_escalation" : path.includes("nodal") ? "nodal_officer" : path.includes("officer") ? "grievance_officer" : path.includes("complaint") ? "complaints_route" : "grievance_redressal";
  add(matches, "grievance_complaints", route, "grievance_complaints_route", "PRIMARY", "NONE", "Grievance/complaints matrix route matched.", ["GRIEVANCE_REDRESSAL_SIGNAL", "CONTACT_NOTICE_SIGNAL", path.includes("ombudsman") || path.includes("nodal") ? "CONSUMER_DISCLOSURE_SIGNAL" : null].filter(Boolean));
}
function classifySupportHelpResources({ path }, matches) {
  if (!matchAny(path, ["/help", "/support", "/faq", "/knowledge-base"]) && !isBlogPath(path)) return;
  const route = isBlogPath(path) ? "blog_or_resource" : path.includes("faq") ? "faq_root" : path.includes("knowledge-base") ? "knowledge_base" : pathDepth(path) <= 1 ? "support_root" : "support_article";
  add(matches, "support_help_resources", route, "support_context", "SECONDARY", "SECONDARY_SUPPORT", "Support/help resources matrix route matched.", ["SUPPORT_CONTEXT_SIGNAL"]);
}
function classifyLegalDocumentSurfaces({ path }, matches) {
  ...
  else if (["fair_practice_code", "interest_rate_policy", "key_fact_statement", "most_important_terms_conditions", "kyc_aml_policy", "deposit_insurance_disclosure", "schedule_of_charges", "responsible_lending_policy"].includes(legal.docType)) add(matches, "regulatory_licensing_status", legal.docType, "legal_document", "PRIMARY", "NONE", "Regulatory/financial disclosure legal document surface matched.", ["LEGAL_DOCUMENT_SIGNAL", "LICENSING_REGULATORY_SIGNAL", "CONSUMER_DISCLOSURE_SIGNAL"]);
  else if (["grievance_redressal_policy", "complaints_procedure"].includes(legal.docType)) add(matches, "grievance_complaints", legal.docType, "legal_document", "PRIMARY", "NONE", "Grievance/complaints legal document surface matched.", ["LEGAL_DOCUMENT_SIGNAL", "GRIEVANCE_REDRESSAL_SIGNAL", "CONTACT_NOTICE_SIGNAL"]);
...
function mergeMatchesByRoot(matches) {
  const byRoot = new Map();
  for (const match of matches) {
    const existing = byRoot.get(match.common_root);
    if (!existing) {
      byRoot.set(match.common_root, { ...match });
      continue;
    }
    ...
    better.source_signal_roles = unique([...(existing.source_signal_roles || []), ...(match.source_signal_roles || [])]);
    if (!better.technical_route_shape) better.technical_route_shape = existing.technical_route_shape || match.technical_route_shape || null;
    if (!better.api_data_flow_signal?.present) better.api_data_flow_signal = existing.api_data_flow_signal?.present ? existing.api_data_flow_signal : match.api_data_flow_signal || { present: false, basis: [] };
    byRoot.set(match.common_root, better);
  }
...
async function scoutLockedRootMatrix({ candidates, rejectedCandidates, scoutFailures, discoveryLog, rootUrl, rootHost }) {
  for (const root of COMMON_ROOTS) await scoutRootChain({ root, candidates, rejectedCandidates, scoutFailures, discoveryLog, rootUrl, rootHost });
}
async function scoutRootChain({ root, candidates, rejectedCandidates, scoutFailures, discoveryLog, rootUrl, rootHost }) {
  ...
  while (queue.length) {
    ...
    if (fetched.ok) {
      ...
      if (root.traversal_policy === "PRIMARY_FULL_EXTRACT") for (const href of extractHrefs(fetched.raw_text)) {
...
function enqueueUrl(queue, queued, url, rootHost) {
  const normalized = normalizeCandidateForManifest(url, rootHost);
  if (!normalized || queued.has(normalized.canonical_url_key)) return;
  queued.add(normalized.canonical_url_key);
  queue.push(normalized.fetch_url);
}
function candidateMatchesRoot(value, root, rootHost) {
  const normalized = normalizeCandidateForManifest(value, rootHost);
  if (!normalized) return false;
  const path = normalizePath(new URL(normalized.fetch_url).pathname);
  return (root.paths || []).some((prefix) => prefix !== "/" && (path === prefix || path.startsWith(`${prefix}/`))) || (root.id === "homepage_landing" && path === "/");
}
function adapterDiscovery(discoveredBy = []) {
  const adapter = (discoveredBy || []).some((item) => String(item || "").includes("EXPAND_ONLY_ADAPTER"));
  return { adapter_discovered: adapter, adapter_id: null, adapter_effect: adapter ? "EXPAND_ONLY" : "NONE", may_narrow_discovery: false, may_exclude_sources: false };
}
function addScopedAnchors(candidates, rejectedCandidates, html, rootUrl, rootHost) {
  const chunks = [...extractTagChunks(html, "header").map((chunk) => ({ chunk, by: "HEADER" })), ...extractTagChunks(html, "footer").map((chunk) => ({ chunk, by: "FOOTER" })), { chunk: html, by: "ROOT" }];
...
function addLinkedSitemaps(candidates, rejectedCandidates, html, rootUrl, rootHost) {
  for (const href of extractLinkedSitemaps(html)) addCandidate(candidates, rejectedCandidates, normalizeCandidateUrl(href, rootUrl, rootHost), "SITEMAP_LINK", rootHost);
}
async function scoutSitemaps({ candidates, rejectedCandidates, scoutFailures, discoveryLog, rootUrl, rootHost, rootHtml }) {
  ...
  for (const href of extractLinkedSitemaps(rootHtml)) {
    const url = normalizeCandidateUrl(href, rootUrl, rootHost);
...
async function scoutKnownPaths({ candidates, rejectedCandidates, scoutFailures, discoveryLog, rootUrl, rootHost, paths, probeClass }) {
  const origin = new URL(rootUrl).origin;
  for (const path of paths) {
    const url = new URL(path, origin).toString();
    const fetched = await safeFetchRaw(url);
    if (fetched.ok) {
      discoveryLog.push({ step: `${probeClass}_PATH_PROBE`, status: "PASS", url });
      addCandidate(candidates, rejectedCandidates, url, `${probeClass}_PATH_PROBE`, rootHost);
    } else if (![404, 410].includes(Number(fetched.http_status || 0))) {
      discoveryLog.push({ step: `${probeClass}_PATH_PROBE`, status: "LIMITED", url, error: fetched.error, http_status: fetched.http_status || null });
      scoutFailures.push({ url, stage: `${probeClass}_PATH_PROBE`, error: fetched.error, http_status: fetched.http_status || null });
    }
  }
}
...
function priority(routeFoundBy) {
  const value = String(routeFoundBy || "");
  if (value.endsWith("_ROOT_CHAIN")) return 5;
  return { ROOT: 1, HEADER: 2, FOOTER: 3, SITEMAP: 4, SITEMAP_LINK: 4, LEGAL_DOC_PATH_PROBE: 5, EXPAND_ONLY_ADAPTER_PATH_PROBE: 6 }[value] || 9;
}
...
function normalizeCandidateUrl(value, baseUrl, rootHost) {
  try {
...
function normalizeCandidateForManifest(value, rootHost) {
  try {
    ...
    const path = normalizePath(url.pathname);
    const canonicalUrl = `${url.protocol}//${host}${path}`;
    return { original_url: value, fetch_url: url.toString(), canonical_url: canonicalUrl, canonical_url_key: canonicalUrl };
  } catch { return null; }
}
...
async function fetchRaw(url) {
  ...
  try {
    const response = await fetch(url, { method: "GET", signal: controller.signal, redirect: "follow", headers: { "user-agent": "LexNovaHQ-DiligenceReviewer/1.0 (+phase1-agnostic-source-discovery)", "accept": "text/markdown,text/plain,text/html,application/xhtml+xml,application/json,application/xml,text/xml,*/*;q=0.5" } });
