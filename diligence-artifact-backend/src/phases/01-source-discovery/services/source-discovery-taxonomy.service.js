...
export function legalDocTypeFromUrlOrRoute(value) { const match = legalDocMatchFromUrlOrRoute(value); return { docType: match.docType, artifactName: match.artifactName }; }
export function legalDocMatchFromUrlOrRoute(value) {
  const normalized = normalizeLegalPhrase(value);
  for (const [docType, artifactName, terms] of LEGAL_DOC_RULES) {
    const matchedTerm = terms.find((term) => boundedLegalPhraseMatch(normalized, normalizeLegalPhrase(term)));
    if (matchedTerm) return { docType, artifactName, matchedTerm, match_mode: "BOUNDED_TOKEN_OR_PHRASE" };
  }
  return { docType: "other", artifactName: "legal_doc_other", matchedTerm: null, match_mode: "NO_BOUNDED_MATCH" };
}
...
function normalizeLegalPhrase(value) { return String(value || "").toLowerCase().replace(/https?:\/\//g, " ").replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ").trim(); }
function boundedLegalPhraseMatch(haystack, needle) { return Boolean(needle) && ` ${haystack} `.includes(` ${needle} `); }
