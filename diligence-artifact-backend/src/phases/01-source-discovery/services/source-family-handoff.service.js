...
export function buildSourceFamilyHandoffArtifact({ run, artifacts }) {
  ...
  return {
    source_discovery_handoff: {
      ...
      schema_version: "PHASE1_AGNOSTIC_SOURCE_DISCOVERY_HANDOFF_v1",
      status: hasPrimary(commonRootIndex) || legalDocs.length ? "LOCKED" : "LOCKED_WITH_LIMITATIONS",
      contract: {
        formal_output: "source_discovery_handoff",
        purpose: "Index Phase 1 common-root source files, neutral evidence buckets, and independent legal document artifacts for downstream profile consumers.",
    ...
    post_phase_1_domain_gate_handoff: {
      ...
      target_url: targetUrl,
      generated_by: "source_discovery_handoff",
      schema_version: "POST_PHASE_1_DOMAIN_GATE_HANDOFF_v1",
...
function accessMatrix() { return { global: { default_load: "common roots, neutral buckets, and independent legal documents only", index_only: "metadata only", cross_bucket: "exception only", legal_doc_rule: "load individual legal_doc_* artifacts for substance" }, central_phases: { POST_PHASE_1_DOMAIN_GATE: { buckets: ["commercial_positioning_sources", "product_activity_sources", "regulated_activity_signals", "ai_mechanism_signals", "data_processing_signals", "customer_segment_signals", "jurisdiction_market_signals"], source: "neutral_evidence_bucket_manifest" }, LEGAL_CARTOGRAPHY_INDEX: { artifacts: ["legal_doc_inventory", "legal_doc_extraction_index", "legal_doc_{DOC_TYPE}"], buckets: ["legal_terms_sources", "privacy_security_sources", "trust_compliance_sources", "contact_notice_sources", "regulated_activity_signals"] }, TARGET_PROFILE_REVIEW: { roots: ["homepage_landing", "about_company", "legal_identity_notice", "pricing_commercial_availability", "contact_notice", "operator_entity_signals", "supporting_company_signals"], buckets: ["company_identity_sources", "commercial_positioning_sources", "jurisdiction_market_signals", "contact_notice_sources", "pricing_plan_sources", "third_party_profile_sources"] }, ACTIVITY_PROFILE_REVIEW: { roots: ["product_service", "platform_feature_solution", "technical_docs_api_developer", "docs_api_data_flow", "pricing_commercial_availability", "use_case_customer_industry", "integrations_ecosystem"], buckets: ["product_activity_sources", "technical_docs_sources", "api_integration_sources", "pricing_plan_sources", "customer_segment_signals", "regulated_activity_signals", "ai_mechanism_signals"] }, DATA_PROVENANCE_PROFILE: { roots: ["privacy_data_processing", "security_trust", "technical_docs_api_developer", "docs_api_data_flow", "trust_compliance"], artifacts: ["legal_doc_privacy_policy", "legal_doc_data_processing_agreement", "legal_doc_subprocessor_list", "legal_doc_cookie_policy", "legal_doc_terms_of_service"], buckets: ["privacy_security_sources", "data_processing_signals", "technical_docs_sources", "api_integration_sources", "trust_compliance_sources", "contact_notice_sources"] }, EXPOSURE_PROFILE: { required_profiles: ["legal_cartography_index", "target_profile", "target_feature_profile", "data_provenance_profile_semantic_batch_gate"], source_trace: "source_discovery_handoff" }, COMPILER: { required_profiles: ["locked_outputs_only"], source_trace: "source_discovery_handoff" } } }; }
