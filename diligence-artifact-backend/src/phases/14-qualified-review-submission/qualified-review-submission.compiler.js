...
export const QUALIFIED_REVIEW_SUBMISSION_COMPILER_VERSION = "phase14_qualified_review_submission_compiler.v1";
...
export function compileQualifiedReviewSubmission({
  ...
  if (validation.errors.length) {
    throw new Error(`QUALIFIED_REVIEW_SUBMISSION_INVALID:${validation.errors.join("|")}`);
  }
  ...
  const suppressedActivationDecisions = buildSuppressedActivationDecisions({
    request: submission_request,
    finalFieldById
  });
  ...
  const finalLedger = freezeImmutableArtifact({
    ...ledgerBody,
    immutable_hash: hashImmutableArtifact(ledgerBody)
  });
  ...
  const documentActivationManifest = freezeImmutableArtifact({
    ...documentManifestBody,
    immutable_hash: hashImmutableArtifact(documentManifestBody)
  });
  ...
  const submission = freezeImmutableArtifact({
    ...submissionBody,
    immutable_hash: hashImmutableArtifact(submissionBody)
  });
...
function validateSubmissionInputs({ run, submission_request, qualified_review_handoff, qr_active_field_ledger, qr_registry_resolution_manifest, authority }) {
  ...
  for (const [fieldId, edit] of Object.entries(submission_request.field_edits || {})) {
    ...
    if (edit?.confirmed_activation_probe === true) {
      warnings.push(`SUPPRESSED_ACTIVATION_PROBE_RETAINED:${fieldId}`);
      continue;
    }
    errors.push(`SUBMISSION_UNKNOWN_FIELD_EDIT:${fieldId}`);
  }
  return {
    errors,
    warnings,
...
function buildFinalFields({ ledger, request }) {
  return (ledger.active_fields || []).map((field) => {
    ...
    for (const [atomicKey, sourceRecord] of Object.entries(field.atomic_values || {})) {
      ...
      if (!hasMaterialValue(value) && edit?.not_applicable !== true) {
        throw new Error(`QUALIFIED_REVIEW_FINAL_VALUE_MISSING:${field.qr_field_id}:${atomicKey}`);
      }
      finalAtomicValues[atomicKey] = value;
      atomicSources[atomicKey] = finalSource({ edit, reviewerHasValue, sourceRecord });
...
function buildDocumentActivationManifest({ run, authority, registryResolution, finalFields, finalFieldById }) {
  const errors = [];
  ...
  const aiRegistry = authority.registries.find((loaded) => loaded.registry.registry_id === "AI_QR_BRIDGE_REGISTRY")?.registry || null;
  const rules = new Map((aiRegistry?.document_activation_rules || []).map((rule) => [rule.document_id, rule]));
  const registryState = (registryResolution.registry_resolutions || []).find((row) => row.registry_id === "AI_QR_BRIDGE_REGISTRY");
  const scopeStates = new Map((registryState?.subpackages || []).map((row) => [row.registry_scope, row.state]));
  const context = {
    lane_a_active: scopeStates.get("LANE_A") === "ACTIVE",
    lane_b_active: scopeStates.get("LANE_B") === "ACTIVE",
    fields: Object.fromEntries(finalFields.map((field) => [field.qr_field_id, field]))
  };
  const aiRegistryActive = activeRegistryIds.has("AI_QR_BRIDGE_REGISTRY");
  ...
  const documents = templateDocuments.map((template) => {
    ...
    const evaluation = aiRegistryActive && rule
      ? evaluateActivationRule(rule.activate_when || {}, context)
      : { matched: false, trace: [aiRegistryActive ? "RULE_MISSING" : "AI_REGISTRY_INACTIVE"] };
...
function buildDocumentBindings({ documentId, finalFields, errors }) {
    ...
    for (const binding of field.document_bindings || []) {
      ...
      for (const [atomicKey, token] of Object.entries(binding.value_placeholders || binding.placeholders || {})) {
        ...
        placeholderByToken.set(token, {
          ...
          value,
          source: field.atomic_sources[atomicKey]
        });
      }
      if ((binding.actions || []).includes("SELECT_CLAUSE")) {
        clauseActions.push({
          qr_field_id: field.qr_field_id,
  ...
  return {
    placeholder_bindings: [...placeholderByToken.values()],
...
function emptyDocumentBindings() {
  return { placeholder_bindings: [], clause_actions: [], action_tokens: [], unresolved_qr_placeholder_count: 0 };
}
...
function evaluateActivationRule(rule, context) {
  ...
  const allMatched = all.every((row) => row.matched);
  const anyMatched = !any.length || any.some((row) => row.matched);
  return {
    matched: allMatched && anyMatched,
    trace: [
      ...all.map((row) => `ALL:${row.expression}:${row.matched}`),
      ...any.map((row) => `ANY:${row.expression}:${row.matched}`)
    ]
  };
}
...
function evaluateCondition(rawCondition, context) {
  ...
  if (match) return result(expression, Boolean(resolveVariable(match[1], context)) === (match[2].toLowerCase() === "true"));
  match = expression.match(/^([A-Za-z0-9_]+)\s*>\s*(-?\d+(?:\.\d+)?)$/i);
  if (match) return result(expression, Number(resolveVariable(match[1], context)) > Number(match[2]));
  match = expression.match(/^([A-Za-z0-9_]+)\s+is\s+not\s+empty$/i);
  if (match) return result(expression, hasMaterialValue(resolveVariable(match[1], context)));
  ...
  if (match) {
    ...
    return result(expression, candidates.some((candidate) => haystack.includes(candidate)));
  }
...
function summarizeFinalFields(finalFields, suppressedActivationDecisions) {
  const allAtomicSources = finalFields.flatMap((field) => Object.values(field.atomic_sources || {}));
  return {
    ...
    reviewer_edited_atomic_count: allAtomicSources.filter((source) => source === "REVIEWER_EDIT").length,
    reviewer_attested_phase12_atomic_count: allAtomicSources.filter((source) => source === "REVIEWER_ATTESTED_PHASE_12").length,
    reviewer_attested_market_based_atomic_count: allAtomicSources.filter((source) => source === "REVIEWER_ATTESTED_MARKET_BASED").length,
    suppressed_activation_decision_count: suppressedActivationDecisions.length
  };
}

function finalSource({ edit, reviewerHasValue, sourceRecord }) {
  ...
  if (reviewerHasValue && !sameValue(edit.atomic_values, {})) {
    return sameValue(edit.atomic_values?.[Object.keys(edit.atomic_values || {})[0]], sourceRecord?.value)
      ? `REVIEWER_ATTESTED_${sourceRecord?.source || "UNRESOLVED"}`
      : "REVIEWER_EDIT";
  }
  return `REVIEWER_ATTESTED_${sourceRecord?.source || "UNRESOLVED"}`;
}

function result(expression, matched) { return { expression, matched: Boolean(matched) }; }
