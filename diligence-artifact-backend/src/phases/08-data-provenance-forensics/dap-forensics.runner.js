...
export async function runDapForensicsPhase({ run, internalJobId = "DATA_PROVENANCE_PROFILE_FORENSICS", contract, readArtifacts, saveArtifact } = {}) {
  ...
  return Object.freeze({ ok: true, output: profile, saved_artifacts: ["dap_forensics_profile"], phase_lock_status: status, model_usage: "NONE_DETERMINISTIC", internal_job_id: internalJobId });
}
...
export function buildDapForensicsProfile({ run = {}, artifacts = {} } = {}) {
  ...
  return Object.freeze({ dap_forensics_profile: Object.freeze({
    ...
    profile_version: "phase8_dap_forensics_profile_v1",
    run_id: run.run_id || "UNKNOWN_RUN",
    phase_id: "DATA_PROVENANCE_FORENSICS",
    source_boundary: "PHASE7_LOCKED_DAP_BATCH_ARTIFACTS_ONLY",
    forensic_contract: Object.freeze({ contract_name: CONTRACT_NAME, trace_scope: "Phase 7 DAP navigation index, route manifest, 17 DAP batch artifacts, validation manifest, and semantic batch gate", source_of_truth: "saved_phase7_artifacts", model_generated_forensics_allowed: false, deterministic_forensic_profile: true }),
    forensic_boundary: Object.freeze({ material_profile_re_emitted: false, semantic_forensic_profile_retired: true, old_m10_forensics_reused: false, four_b_four_c_reused: false, artifact_name: "dap_forensics_profile" }),
...
function assertRuntimeContract(contract = {}) { if (contract.central_phase_id !== DAP_FORENSICS_CONTRACT.central_phase_id) throw new Error(`DAP_FORENSICS_CONTRACT_MISMATCH:${contract.central_phase_id || "missing"}`); assertSameArray(contract.reads || [], DAP_FORENSICS_CONTRACT.deterministic_job.reads, "DAP_FORENSICS_READS"); assertSameArray(contract.writes || [], DAP_FORENSICS_CONTRACT.deterministic_job.writes, "DAP_FORENSICS_WRITES"); }
...
function assertDapForensicsProfile(profile = {}) { for (const branch of DAP_FORENSICS_CONTRACT.output_contract.required_branches) if (!(branch in profile)) throw new Error(`DAP_FORENSICS_MISSING_BRANCH:${branch}`); if (profile.forensic_contract?.model_generated_forensics_allowed !== false) throw new Error("DAP_FORENSICS_MODEL_GENERATED_TRACE_NOT_FORBIDDEN"); if (profile.forensic_boundary?.old_m10_forensics_reused !== false) throw new Error("DAP_FORENSICS_OLD_M10_REUSED"); if (profile.forensic_boundary?.four_b_four_c_reused !== false) throw new Error("DAP_FORENSICS_4B_4C_REUSED"); }
