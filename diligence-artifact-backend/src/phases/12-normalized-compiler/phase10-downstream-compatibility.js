...
export function buildPhase10CompilerCompatibility({ artifacts = {} } = {}) {
  ...
  for (const row of workpadRows) {
    ...
    if (MATERIAL.has(status)) {
      ...
      workpadOnlyRows.push(withCustody(row, null, status));
...
function withCustody(row, projection, status) {
  return {
    ...
    Threat_ID: row.Threat_ID || projection?.Threat_ID || "",
...
function summarizeByStream(rows) {
  const map = new Map();
  for (const row of rows) {
    const key = row.stream_id || `${row.stream_type || "UNKNOWN"}::${row.package_id || "UNKNOWN"}`;
    const current = map.get(key) || { stream_id: key, stream_type: row.stream_type || "", package_id: row.package_id || "", row_count: 0, final_status_counts: {} };
    current.row_count += 1;
    const status = String(row.final_material_status || "UNKNOWN");
    current.final_status_counts[status] = (current.final_status_counts[status] || 0) + 1;
    map.set(key, current);
  }
  return [...map.values()];
}
function summarizeByPackage(rows) {
  const map = new Map();
  for (const row of rows) map.set(row.package_id || "UNKNOWN", (map.get(row.package_id || "UNKNOWN") || 0) + 1);
  return [...map.entries()].map(([package_id, row_count]) => ({ package_id, row_count }));
}
