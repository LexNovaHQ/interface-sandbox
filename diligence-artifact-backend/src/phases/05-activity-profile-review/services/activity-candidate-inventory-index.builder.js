...
// Compatibility alias for the untouched Pass 3 runner. It always delegates to the
// deterministic baseline builder. Legacy two-argument calls are accepted only to retain
// run metadata; without lossless units they produce a controlled empty baseline.
...
export function validateFeatureCandidateInventoryIndex(input) {
  ...
  if (inventory?.artifact_type !== FEATURE_CANDIDATE_INVENTORY_ARTIFACT) {
    failures.push("artifact_type must be feature_candidate_inventory");
  }
  if (inventory?.inventory_version !== FEATURE_CANDIDATE_INVENTORY_VERSION) {
    failures.push("inventory_version mismatch");
  }
  if (inventory?.derivation_mode !== FEATURE_CANDIDATE_INVENTORY_MODE) {
    failures.push("inventory derivation_mode mismatch");
  }
  if (inventory?.source_index_artifact !== "activity_profile_source_index") {
    failures.push("inventory must derive from activity_profile_source_index");
  }
  for (const field of [
  ...
  if (inventory?.index_boundary?.deterministic_baseline_only !== true) {
    failures.push("deterministic baseline boundary missing");
  }
  if (inventory?.index_boundary?.source_index_is_navigation_only !== true) {
    failures.push("source index navigation boundary missing");
  }
  if (inventory?.index_boundary?.lossless_primary_evidence_read !== true) {
    failures.push("lossless primary evidence boundary missing");
  }
  if (inventory?.index_boundary?.phase2g_routed_packet_is_read_ceiling !== true) {
    failures.push("Phase 2G read ceiling boundary missing");
  }
  if (inventory?.index_boundary?.no_source_text_copy !== true) {
    failures.push("source text copy boundary missing");
  }
  if (inventory?.index_boundary?.no_package_specific_activity_classification !== true) {
    failures.push("package classification boundary missing");
  }
  if (inventory?.deterministic_baseline_metadata?.provider_called !== false) {
    failures.push("deterministic baseline must not call provider");
  }
  if (inventory?.deterministic_baseline_metadata?.package_taxonomy_applied !== false) {
    failures.push("deterministic baseline must not apply package taxonomy");
  }
  ...
  for (const candidate of inventory?.candidates || []) {
    ...
    if (JSON.stringify(exactFields) !== JSON.stringify(expectedFields)) {
      failures.push(`${candidate?.candidate_id || "candidate"}:candidate_field_set_mismatch`);
    }
    if (!candidate.candidate_id || !candidate.canonical_feature_key) {
      failures.push("candidate missing id/key");
    }
    if (candidateIds.has(candidate.candidate_id)) failures.push(`duplicate candidate id:${candidate.candidate_id}`);
    candidateIds.add(candidate.candidate_id);
    if (candidateKeys.has(candidate.canonical_feature_key)) {
      failures.push(`duplicate canonical key:${candidate.canonical_feature_key}`);
    }
    candidateKeys.add(candidate.canonical_feature_key);
    if (candidate.evidence_grounded !== true) {
      failures.push(`${candidate.candidate_id || "candidate"}:evidence_grounded_must_be_true`);
    }
    if (!Array.isArray(candidate.source_pointers) || !candidate.source_pointers.length) {
      failures.push(`${candidate.candidate_id || "candidate"}:source_pointers_missing`);
    }
    for (const pointer of candidate.source_pointers || []) {
      validateSourcePointer(pointer, candidate.candidate_id || "candidate", failures);
    }
  }

  for (const rawHit of inventory?.raw_feature_hit_index || []) {
    if (rawHit?.evidence_unit_mapped !== true) {
      failures.push(`${rawHit?.raw_hit_id || "raw_hit"}:evidence_unit_not_mapped`);
    }
    validateSourcePointer(rawHit?.source_pointer, rawHit?.raw_hit_id || "raw_hit", failures);
  }

  if (inventory?.raw_hit_count !== (inventory?.raw_feature_hit_index || []).length) {
    failures.push("raw_hit_count mismatch");
  }
  if (inventory?.canonical_candidate_count !== (inventory?.candidates || []).length) {
    failures.push("canonical_candidate_count mismatch");
  }
...
export function validateFeatureCandidateCoverage(inventoryInput, profileInput, options = {}) {
  ...
  const add = (candidate_id, activity_reference, disposition, path, reason = "") => {
    ...
    coverageRows.push({
      candidate_id,
      activity_reference: activity_reference || null,
  ...
  for (const activity of profile.activities || []) {
    for (const candidate_id of activity.source_candidate_ids || []) {
      add(candidate_id, activity.activity_reference, activity.coverage_disposition, "activity.source_candidate_ids");
    }
  }
  ...
  for (const row of options.exclusionLedger || options.candidate_exclusion_ledger || []) {
    if (row.reason || row.exclusion_reason) {
      add(
...
export function buildM8FeatureCoverageForensics(inventoryInput, profileInput, options = {}) {
  ...
  return Object.freeze({
    ...
    raw_feature_hit_derivation_ledger: (inventory?.raw_feature_hit_index || []).map(({
      raw_hit_id,
      raw_type,
      source_pointer,
      evidence_unit_ref
    }) => ({ raw_hit_id, raw_type, source_pointer, evidence_unit_ref })),
    canonicalization_derivation_ledger: inventory?.canonicalization_index || [],
    dedup_decision_ledger: inventory?.dedup_index || [],
    parent_child_overlap_ledger: validation.coverage_rows || [],
...
function buildEvidenceUnitIndex(payload) {
  ...
  const visit = (value, structuralPath) => {
    ...
    if (isStructuralEvidenceUnit(value)) {
      ...
      units.push(record);
      for (const alias of evidenceAliases(value)) {
        if (!aliases.has(alias)) aliases.set(alias, record);
      }
    }
...
function validateSourcePointer(pointer, ownerId, failures) {
  ...
  if (!pointer.source_artifact || !pointer.source_root || !pointer.route_class) {
    failures.push(`${ownerId}:source_pointer_missing_routing_identity`);
  }
...
function unwrapActivityProfileSourceIndex(input) {
  if (input?.activity_profile_source_index && typeof input.activity_profile_source_index === "object") {
    return input.activity_profile_source_index;
  }
  if (input?.artifact?.activity_profile_source_index && typeof input.artifact.activity_profile_source_index === "object") {
    return input.artifact.activity_profile_source_index;
  }
  if (input?.artifact && typeof input.artifact === "object" && (
    input.artifact.activity_candidate_source_locator_map ||
    input.artifact.product_capability_locator_map
  )) {
...
function unwrapLosslessUnitsByRoot(input) {
  ...
  if (input.lossless_units_by_root && typeof input.lossless_units_by_root === "object") {
    return input.lossless_units_by_root;
  }
...
function candidateRowAllowed(row = {}) {
  return row &&
    row.route_action === "LOCATE_ONLY" &&
    row.candidate_creation_allowed !== false &&
...
function compareRawHits(a, b) {
  return [
    a.source_root,
    a.activity_route_class,
    a.capability_key,
    a.source_id,
    stablePointer(a.source_pointer)
  ].join("::").localeCompare([
    b.source_root,
    b.activity_route_class,
    b.capability_key,
    b.source_id,
    stablePointer(b.source_pointer)
  ].join("::"));
}
...
function coverageResult(status, failures, uncovered, coverageRows) {
  return {
    ...
    failures,
    uncovered_candidates: uncovered.map(({
      candidate_id,
      candidate_name,
      candidate_type,
      canonical_feature_key
    }) => ({ candidate_id, candidate_name, candidate_type, canonical_feature_key })),
...
function containsEvidenceCopy(value) {
  ...
  return Object.keys(value).some((key) => FORBIDDEN_EVIDENCE_COPY_FIELDS.has(key)) ||
    Object.values(value).some(containsEvidenceCopy);
}

function containsPackageClassification(value) {
  if (!value || typeof value !== "object") return false;
  if (Array.isArray(value)) return value.some(containsPackageClassification);
  return Object.keys(value).some((key) => FORBIDDEN_PACKAGE_CLASSIFICATION_FIELDS.has(key)) ||
    Object.values(value).some(containsPackageClassification);
}
