...
const PRODUCT_FAMILIES = new Set(["P1_PRODUCT", "P2_PLATFORM_FEATURE_SOLUTION", "P3_AI_CAPABILITY_TECHNICAL", "P5_ENTERPRISE_PRICING"]);
...
export function buildFeatureCandidateInventoryIndex(sourceArtifacts, options = {}) {
  ...
  return Object.freeze({
    ...
    run_id: options.runId || findRunId(artifacts) || null,
    derivation_mode: FEATURE_CANDIDATE_INVENTORY_MODE,
    source_families_indexed: [...new Set(rawHits.map((hit) => hit.source_family).filter(Boolean))].sort(),
    ...
    index_limitations: rawHits.length ? [] : ["No public product/activity candidates were detected in the loaded product/activity source families."]
  });
}
...
function harvestArtifactHits(artifact = {}) {
  const family = String(artifact.root_family || artifact.artifact_name || "").replace(/^lossless_family__/, "");
  if (!PRODUCT_FAMILIES.has(family)) return [];
  return (artifact.sources || []).flatMap((source) => harvestSourceHits(source, family));
}

function harvestSourceHits(source = {}, family) {
  ...
  const pointer = toPointer(source, family);
  const hits = [];
  if (family === "P1_PRODUCT" && (hasPathSegment(url, "products") || routeType.includes("product"))) hits.push(rawHit({ source, family, pointer, raw_name: title || "Product Wrapper", raw_type: RAW_TYPES.PRODUCT_WRAPPER, wrapper_or_surface: normalizeSlug(title || slug || "product"), capability_key: normalizeSlug(title || slug || "product"), surface_key: "product-wrapper", confidence_basis: "product_route_or_title" }));
  if (family === "P2_PLATFORM_FEATURE_SOLUTION" && (slug || title)) hits.push(rawHit({ source, family, pointer, raw_name: title || humanize(slug), raw_type: RAW_TYPES.FEATURE_PAGE, wrapper_or_surface: "feature-page", capability_key: normalizeSlug(title || slug), surface_key: "feature-page", confidence_basis: routeType || "feature_route" }));
  if (family === "P3_AI_CAPABILITY_TECHNICAL") {
    if (hasPathSegment(url, "apis")) hits.push(rawHit({ source, family, pointer, raw_name: `${humanize(slug || "api")} API`, raw_type: RAW_TYPES.STANDALONE_API, wrapper_or_surface: "api", capability_key: normalizeSlug(slug || title || "api"), surface_key: "standalone-api", confidence_basis: "api_route" }));
    if (hasPathSegment(url, "models")) hits.push(rawHit({ source, family, pointer, raw_name: "Model Catalogue", raw_type: RAW_TYPES.MODEL_CATALOGUE, wrapper_or_surface: "models", capability_key: "model-catalogue", surface_key: "model-catalogue", confidence_basis: "models_route" }));
    if (hasPathSegment(url, "docs")) hits.push(rawHit({ source, family, pointer, raw_name: "API Documentation Surface", raw_type: RAW_TYPES.DOCS_TECHNICAL_CAPABILITY, wrapper_or_surface: "docs", capability_key: "api-documentation-surface", surface_key: "docs-technical-capability", confidence_basis: "docs_route" }));
  }
  if (family === "P5_ENTERPRISE_PRICING") for (const name of pricingNames(text)) hits.push(rawHit({ source, family, pointer, raw_name: name, raw_type: RAW_TYPES.PRICING_CONFIRMED_CAPABILITY, wrapper_or_surface: "pricing", capability_key: normalizeSlug(name), surface_key: "commercial-pricing", confidence_basis: "pricing_text" }));
  return hits;
}
...
function rawHit({ source, family, pointer, raw_name, raw_type, wrapper_or_surface, capability_key, surface_key, confidence_basis }) { return { source_family: family, source_id: source.source_id || source.manifest_id || "", source_url: source.canonical_url || source.url || source.final_url || "", raw_name, raw_type, wrapper_or_surface, capability_key, surface_key, confidence_basis, source_pointer: pointer }; }
