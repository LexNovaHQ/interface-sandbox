...
const LOCK_STATUSES = Object.freeze(["LOCKED", "LOCKED_WITH_LIMITATIONS", "REINVESTIGATION_REQUIRED", "CONTROLLED_FAILURE"]);
const SOURCE_CORPUS_STATUSES = Object.freeze(["FOUND_AS_PRIMARY_SOURCE", "FOUND_EMBEDDED_IN_LEGAL_CORPUS", "FOUND_AS_LINKED_REFERENCE", "REFERENCED_BUT_NOT_FETCHED", "STANDALONE_SOURCE_ABSENT", "SOURCE_REJECTED_OR_FAILED", "UNKNOWN_NOT_SEARCHED", "FOUND_THIN"]);
...
const FORBIDDEN_KEYS = Object.freeze(["source_discovery_handoff", "target_profile", "target_feature_profile", "data_provenance_profile", "exposure_registry_profile", "challenge_gate", "final_output_handoff", "renderer_payload", "legal_advice", "compliance_conclusion", "sufficiency_conclusion", "enforceability_assessment", "risk_conclusion", "registry_evaluation", "m6_authorization_status", "m6_bucket_subcategory", "license_validity", "required_license", "applicable_regulator", "regulatory_compliance_status", "grievance_sufficiency", "grievance_compliance_status", "ombudsman_requirement"]);
const FORBIDDEN_STRING_VALUES = Object.freeze(["REFERENCED_NOT_AUTHORIZED_BY_M6", "M6-authorized", "M6 authorized", "authorized by M6", "not authorized by M6", "license is valid", "license required", "regulatory compliance confirmed", "grievance mechanism sufficient"]);
...
export function validateM9LegalCartographyIndex(output) {
  ...
  if (artifact.downstream_rules?.source_discovery_is_navigation_not_legal_authority !== true) failures.push("downstream_rules.source_discovery_is_navigation_not_legal_authority must be true");
  ...
  if (artifact.downstream_rules?.phase1_v5_source_contract_required !== true) failures.push("downstream_rules.phase1_v5_source_contract_required must be true");
  if (artifact.downstream_rules?.regulatory_grievance_conclusions_forbidden !== true) failures.push("downstream_rules.regulatory_grievance_conclusions_forbidden must be true");
...
function validateLocatorRows(artifact, failures) { if (asArray(artifact.control_language_locator).some((row) => row.display_in_main_report !== false || row.technical_annexure_only !== true)) failures.push("control_language_locator rows must be technical-annexure-only"); for (const key of ["semantic_navigation_index", "priority_semantic_locator", "qualified_review_locator", "legal_notice_locator", "dispute_resolution_locator", "governing_law_venue_locator", "contact_grievance_locator", "regulatory_governance_locator", "grievance_redressal_locator", "consumer_disclosure_locator", "counterparty_institution_locator"]) for (const row of asArray(artifact[key])) if (row.status) validateStatus(row, failures, key); for (const key of ["regulatory_governance_locator", "grievance_redressal_locator", "consumer_disclosure_locator", "counterparty_institution_locator"]) for (const row of asArray(artifact[key])) if (row.legal_or_regulatory_conclusion_forbidden !== true) failures.push(`${key} rows must forbid legal/regulatory conclusions`); }
...
function validateArtifactClass(row, failures, location) { if (!Object.prototype.hasOwnProperty.call(row, "artifact_class")) return; if (FORBIDDEN_ARTIFACT_CLASSES.includes(row.artifact_class)) failures.push(`${location} forbidden artifact_class: ${row.artifact_class}`); else if (!ARTIFACT_CLASSES.includes(row.artifact_class)) failures.push(`${location} invalid artifact_class: ${row.artifact_class}`); }
function validateSourceType(row, failures, location) { if (Object.prototype.hasOwnProperty.call(row, "source_type") && !SOURCE_TYPES.includes(row.source_type)) failures.push(`${location} invalid source_type: ${row.source_type}`); }
function rejectForbiddenExactStatus(row, failures, location) { for (const value of Object.values(row || {})) if (FORBIDDEN_EXACT_STATUS_VALUES.includes(value)) failures.push(`${location} forbidden exact status: ${value}`); }
function validateEmbeddedCoverageCompleteness(artifact, failures) { const coverage = new Set(asArray(artifact.document_coverage_index).map((row) => row.document_id || row.artifact_id)); for (const unit of asArray(artifact.document_structure_index)) if (["ANNEXURE", "SCHEDULE", "APPENDIX", "ADDENDUM", "EXHIBIT"].includes(unit.unit_type) && !coverage.has(unit.unit_id) && !coverage.has(unit.section_id)) failures.push(`embedded unit missing coverage row: ${unit.unit_id || unit.section_id}`); }
function collectRows(artifact) { return Object.values(artifact || {}).flatMap((value) => Array.isArray(value) ? value : []); }
...
function containsStringValue(value, needle) { if (typeof value === "string") return value.toLowerCase().includes(String(needle).toLowerCase()); if (!value || typeof value !== "object") return false; if (Array.isArray(value)) return value.some((item) => containsStringValue(item, needle)); return Object.values(value).some((item) => containsStringValue(item, needle)); }
function hasBadSourceSyntax(value) { return /\bundefined\b|\[object Object\]/i.test(value); }
function asArray(value) { return Array.isArray(value) ? value : []; }
function fail(failures) { return { status: "FAIL", failed_gates: failures, repair_instructions: failures }; }
