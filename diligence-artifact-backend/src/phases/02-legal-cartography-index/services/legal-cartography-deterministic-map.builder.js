...
export function buildM9DeterministicMap({ run = {}, artifacts = {} } = {}) {
  ...
  return {
    [M9_DETERMINISTIC_ARTIFACT_NAME]: {
      ...
      schema_version: "M9_DETERMINISTIC_LEGAL_STACK_INDEX_v10_MAIN_RESTORED_PHASE1_V4",
      model_used: false,
      artifact_role: "Indexes loaded legal/governance documents and derives QR legal signal maps from loaded Phase 1 v4 legal/common-root corpus only.",
      ...
      legal_governance_source_coverage: { input_contract: "phase1_v4_common_roots_plus_individual_legal_doc_artifacts", source_artifact_count: sourceArtifactsRead.length, loaded_document_count: documentMap.length, macro_unit_count: macroUnitMap.length, embedded_unit_count: embeddedUnitMap.length, semantic_label_queue_count: semanticLabelQueue.length, missing_or_limited_count: missingSourceMap.length, legal_notice_contact_signal_count: legalNoticeContactSignalMap.length, liability_cap_signal_count: liabilityCapSignalMap.length, sla_support_signal_count: slaSupportSignalMap.length },
...
function collectLegalGovernanceSources(artifacts = {}) {
  ...
  for (const artifactName of LEGAL_GOVERNANCE_SOURCE_ARTIFACTS) {
    ...
    if (!rows.length) gaps.push(missingSourceRow({ artifactName, reason: "No loaded source rows in Phase 1 v4 legal/governance common-root artifact." }));
    sources.push(...rows);
  }
...
function ingestSource(ctx) {
  const { source, documentMap, artifactInventoryMap, legalDocumentIndex, macroUnitMap, embeddedUnitMap, noticeCandidateMap, controlLanguageCandidateMap, indemnityCandidateMap, legalNoticeContactSignalMap, liabilityCapSignalMap, slaSupportSignalMap, crossDocumentReferenceMap, semanticLabelQueue, qualityRepairQueue } = ctx;
  ...
  for (const unit of units) {
    ...
    deriveQrSignalRows({ unit, text, documentId, artifactClass, legalNoticeContactSignalMap, liabilityCapSignalMap, slaSupportSignalMap });
  }
...
function deriveQrSignalRows({ unit, text, documentId, artifactClass, legalNoticeContactSignalMap, liabilityCapSignalMap, slaSupportSignalMap }) {
...
function buildLimitations({ documentMap, macroUnitMap, missingSourceMap, qualityRepairQueue }) { const rows = []; if (!documentMap.length) rows.push("No loaded legal/governance document was indexed from Phase 1 v4 source contract."); if (!macroUnitMap.length) rows.push("No reliable legal/governance macro-unit map was produced."); if (missingSourceMap.length) rows.push(`${missingSourceMap.length} legal/governance source inputs were missing, sparse, or metadata-only.`); if (qualityRepairQueue.length) rows.push(`${qualityRepairQueue.length} cartography quality repair rows were generated.`); return rows; }
...
function dedupeSources(rows) { return dedupeRows(rows, (row) => `${row.source_artifact_name}|${row.source_id}|${row.lossless_text.slice(0, 160)}`); }
...
function unwrapArtifact(artifact) { if (!artifact || typeof artifact !== "object") return artifact; if (artifact.artifact && typeof artifact.artifact === "object") return artifact.artifact; if (artifact.data && typeof artifact.data === "object") return artifact.data; if (artifact.payload && typeof artifact.payload === "object") return artifact.payload; return artifact; }
