...
export async function runPhase10TargetedReinvestigation({ run, dispatch, contract, readArtifacts, buildPrompt = buildPhasePrompt, callProvider = callProviderJson } = {}) {
  ...
  await save("M11", "exposure_registry_profile_forensics", forensics, forensics?.forensic_lock_gate_result?.status === "PASS" ? "LOCKED" : "LOCKED_WITH_LIMITATIONS", run.run_id);
  return { ok: true, targeted_batch_id: targetBatch.batch_id, targeted_registry_row_keys: targetedKeys, unaffected_batch_count_reused: Math.max(0, array(routePlan.batch_plan).length - 1), unaffected_rows_preserved: true, rebuilt_artifacts: [batchValidationArtifactName(targetBatch), batchArtifactName(targetBatch), "exposure_registry_workpad_98", "exposure_registry_controlled_profile", "exposure_registry_triggered_profile", "exposure_registry_profile_forensics"], full_phase_batch_rerun_performed: false, normal_downstream_cascade_allowed: false };
}
...
async function callBatchModel({ run, dispatch, batch, packet, buildPrompt, callProvider, repair, priorOutput = null, priorValidation = null }) { const prompt = await buildPrompt({ prompt_files: repair ? REPAIR_PROMPTS : BATCH_PROMPTS, phase: `PHASE11_REINVESTIGATION:${repair ? "REPAIR" : "BATCH"}:${batch.batch_id}`, run: { ...run, phase11_reinvestigation_context: dispatch }, artifacts: repair ? { m11_batch_packet: packet.m11_batch_packet, m11_batch_registry_ledger: priorOutput, backend_structural_validation: priorValidation, phase11_reinvestigation_context: dispatch } : { m11_batch_packet: packet.m11_batch_packet, phase11_reinvestigation_context: dispatch }, writes: ["m11_batch_registry_ledger"], references: [] }); const result = await callProvider({ prompt, phase: `OPERATOR_CHALLENGE_PHASE10_${repair ? "REPAIR" : "REINVESTIGATION"}` }); return result?.json || result || {}; }
async function save(phase, artifact_name, artifact, lock_status, run_id) { return saveRuntimeArtifact({ run_id, phase, agent_id: "agent_5_exposure_registry", artifact_name, artifact, lock_status }); }
