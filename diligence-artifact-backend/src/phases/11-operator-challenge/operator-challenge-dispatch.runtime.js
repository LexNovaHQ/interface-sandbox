...
import {
  acquirePhase11DispatchLease,
  releasePhase11DispatchLease,
  buildPhase11DispatchCheckpoint,
  checkpointMayResume
} from "./operator-challenge-dispatch-checkpoint.js";
import {
...
export async function executePhase11ReinvestigationLoop({
  ...
  workerId = `phase11-${process.pid || "worker"}`
} = {}) {
  ...
  return {
    challenge_gate: {
      ...
      reinvestigation_dispatch_adapter: {
        schema_version: "phase11_reinvestigation_dispatch_return_adapter.v2.production",
        status: "COMPLETE",
        dispatch_count: dispatchCount,
        mutation_guard_active: true,
        durable_checkpoints_active: true,
        run_scoped_lease_active: true,
...
async function executeOneDispatch({ run, m12Contract, dispatch, previousCandidate, gate, readArtifacts, buildPrompt, callProvider }) {
  ...
  const beforeArtifacts = checkpoint?.payload?.before_artifacts || await readNamedArtifacts(run.run_id, dispatch.artifact_names, AGENT);
  const baselineVersions = checkpoint?.payload?.baseline_artifact_versions || dispatch.baseline_artifact_versions;

  if (!checkpoint) checkpoint = await saveCheckpoint(run, dispatch, "DISPATCH_CREATED", null, { dispatch, before_artifacts: beforeArtifacts, baseline_artifact_versions: baselineVersions });
  ...
  if (!["OWNER_RETURNED", "RETURN_VALIDATED", "ATTEMPT_RECORDED"].includes(checkpoint.stage)) {
    const currentVersions = await artifactVersions(run.run_id, dispatch.artifact_names);
    const ownerAlreadyReturned = versionAdvanced(baselineVersions, currentVersions);
    if (!ownerAlreadyReturned) {
      ...
      try {
        ...
        runtimeError = error;
        await logEvent({ run_id: run.run_id, event_type: "PHASE11_REINVESTIGATION_OWNER_ERROR", actor: AGENT, payload: { dispatch_id: dispatch.dispatch_id, owner_internal_job: dispatch.owner_internal_job, attempt_number: dispatch.attempt_number, message: error.message } });
      }
    }
  ...
  const afterArtifacts = await readNamedArtifacts(run.run_id, dispatch.artifact_names, AGENT);
  const mutation = validatePhase11TargetedMutation({ dispatch, beforeArtifacts, afterArtifacts });
  if (mutation.rollback_required) {
    await rollbackArtifacts({ run, dispatch, beforeArtifacts });
  ...
  return {
    ...
    receipt: {
      ...
      mutation_guard_status: mutation.status,
...
async function rollbackArtifacts({ run, dispatch, beforeArtifacts }) {
  const actor = OWNER_ACTOR[dispatch.owner_internal_job] || "agent_5_exposure_registry";
  for (const artifactName of dispatch.artifact_names) {
    if (beforeArtifacts[artifactName] === undefined) continue;
    await saveRuntimeArtifact({ run_id: run.run_id, phase: dispatch.owner_internal_job, agent_id: actor, artifact_name: artifactName, artifact: beforeArtifacts[artifactName], lock_status: "LOCKED_WITH_LIMITATIONS" });
  }
}
async function rebuildInventory({ run, m12Contract, readArtifacts }) {
  const routed = await readPhaseRouteRuntimePacket({ internalJobId: "M12", readArtifacts, consumerAgentId: m12Contract.actor_id || m12Contract.agent_id || AGENT });
  return buildOperatorChallengeInventory({ run, artifacts: routed.artifacts }).operator_challenge_inventory;
}
async function saveCheckpoint(run, dispatch, stage, previous, payload) {
  const checkpoint = buildPhase11DispatchCheckpoint({ run, dispatch, stage, previous, payload });
  await saveIndependent(run.run_id, CHECKPOINT, checkpoint, checkpoint.status === "COMPLETE" ? "LOCKED" : "CREATED");
  return checkpoint;
}
