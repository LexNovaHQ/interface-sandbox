...
async function runNormalizedCompilerPhase({ run, phase, contract }) {
  ...
  await logEvent({
    run_id: run.run_id,
    event_type: "NORMALIZED_COMPILER_PHASE_COMPLETED",
    ...
    payload: {
      ...
      normalized_section_artifacts_saved: saved.filter((name) => name.startsWith("normalized_section__")),
      normalized_section_artifact_save_count: saved.filter((name) => name.startsWith("normalized_section__")).length,
      legacy_compiler_phase_retired: true
    }
  });
...
async function loadDynamicM11Artifacts({ run_id, agent_id, artifacts, manifestKey }) {
  ...
  for (const batch of batchPlan) {
    ...
    try {
      m11BatchArtifacts.push({ batch_id: batch.batch_id, artifact_name: batchArtifactName, artifact: await readArtifactPayload({ run_id, artifact_name: batchArtifactName, agent_id }) });
    } catch (_error) {
      missingBatchArtifacts.push(batchArtifactName);
    }
    try {
      m12BatchValidationArtifacts.push({ batch_id: batch.batch_id, artifact_name: validationArtifactName, artifact: await readArtifactPayload({ run_id, artifact_name: validationArtifactName, agent_id }) });
    } catch (_error) {
