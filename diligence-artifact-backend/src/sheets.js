...
export async function ensureRunsSheet() {
  ...
  if (!existing) {
    await sheets.spreadsheets.batchUpdate({
      ...
      requestBody: {
        requests: [{ addSheet: { properties: { title: config.runsSheetName } } }]
      }
    });
  }

  const headerRange = `${quoteSheetName(config.runsSheetName)}!A1:H1`;
...
export async function appendRunDashboardRow(run) {
  ...
  const sheets = getSheetsClient();
  const range = `${quoteSheetName(config.runsSheetName)}!A:H`;
  const values = [[
    run.run_id,
    run.target || "",
    run.root_url || "",
...
export async function updateRunDashboardRow(run) {
  ...
  const range = `${quoteSheetName(config.runsSheetName)}!A${row}:H${row}`;
  const values = [[
    run.run_id,
    run.target || "",
    run.root_url || "",
