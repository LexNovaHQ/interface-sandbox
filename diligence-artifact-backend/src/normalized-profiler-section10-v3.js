...
export function buildNormalizedProfilerOutput({ run = {}, artifacts = {} } = {}) {
  ...
  const sections = {};
  for (const key of NORMALIZED_SECTION_KEYS) sections[key] = key === "methodology_limitations_forensic_annexure" ? section10 : safeObject(base[`normalized_section__${key}`]);
  ...
  const review_ready_section_handoff = buildReviewReadySectionHandoff({ run, sections: orderedSections, validation_status });
...
function normalizeSectionEnvelope(sectionValue, key, index, validation_status) {
  ...
  return {
    ...
    section_limitations: [],
    normalization: { ...(sectionObject.normalization || {}), profiler_version: NORMALIZED_PROFILER_VERSION, normalization_map_version: NORMALIZATION_MAP_VERSION, section_10_merged_forensic_annexure: true, separate_section_11_removed: true, full_forensic_payload_rendered_inline: false, report_body_section_limitations_removed: true, limitation_ledger_routed_to_section_10: true }
  };
}

function stripReportBodySectionLimitations(sectionObject) { return { ...sectionObject, section_limitations: [] }; }
...
function buildNormalizedReportManifest({ run, base, sections, validation_status }) {
  ...
  return {
    ...
    qualified_review_contract: { ...(baseManifest.qualified_review_contract || {}), may_prefill_review_ready_objects: true, must_preserve_source_refs: true, must_not_treat_public_signals_as_confirmed_private_facts: true, qualified_review_required_before_assembly_reliance: true }
  };
}

function buildReviewReadySectionHandoff({ run, sections, validation_status }) {
  return {
    handoff_type: "review_ready_section_handoff",
    profiler_version: NORMALIZED_PROFILER_VERSION,
    run_id: run.run_id || "UNKNOWN_RUN",
    validation_status,
    sections: NORMALIZED_SECTION_KEYS.map((key) => ({ section_id: key, artifact_name: `normalized_section__${key}`, section_title: sections[key]?.section_title || key, eligible_for_qualified_review: sections[key]?.qualified_review_mapping?.eligible !== false, qualified_review_category: sections[key]?.qualified_review_mapping?.category || key, requires_confirmation_before_assembly: true })),
...
function buildFinalOutputHandoff({ base, normalized_report_manifest, review_ready_section_handoff, sections, validation_status }) {
  ...
  return {
    final_output_handoff: {
      ...
      qualified_review_contract: normalized_report_manifest.qualified_review_contract,
...
function hostFromUrl(value) { try { return new URL(String(value || "")).hostname.replace(/^www\./i, ""); } catch { return safeText(value, "Target not specified"); } }
