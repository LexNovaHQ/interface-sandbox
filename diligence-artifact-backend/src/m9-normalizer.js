export function normalizeM9LegalCartographyIndex(output) {
  ...
  delete artifact.qualified_review_legal_signals;
...
function scrubLegacyStrings(value) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (typeof item === "string") value[key] = item.replaceAll("REFERENCED_NOT_AUTHORIZED_BY_M6", "REFERENCED_BUT_NOT_FETCHED").replaceAll("not authorized by M6", "referenced but not fetched").replaceAll("authorized by M6", "loaded in source corpus").replaceAll("M6-authorized", "source-corpus").replaceAll("M6 authorized", "source-corpus");
...
function ensureDownstreamRules(artifact) {
  ...
  artifact.downstream_rules.source_discovery_is_navigation_not_legal_authority = true;
  ...
  artifact.downstream_rules.legal_signal_derivation_profile_is_separate_job_b_artifact = true;
  artifact.downstream_rules.qualified_review_legal_signals_retired_from_m9_index = true;
  delete artifact.downstream_rules.qualified_review_legal_signals_index_only;
  delete artifact.downstream_rules.qualified_review_legal_signals_true_derived_object;
}
