...
export function deriveStrictM11FinalStatus({ semanticRow = {} }) {
  ...
  if (absenceOnly(semanticRow)) return S_B;
  return S_A;
}
...
function absenceOnly(row = {}) { const text = [row.target_match, row.basis_proof, row.control_exclusion_evaluation, row.evidence_source_basis, row.row_limitations].join(" ").toLowerCase(); const absence = ["no evidence", "not found", "does not mention", "does not show", "not visible", "missing", "absent", "not specified", "unable to confirm"].some((marker) => text.includes(marker)); const direct = ["states", "provides", "offers", "collects", "processes", "uses", "stores", "shares", "deploys", "requires", "documentation", "api docs", "dashboard", "signup", "checkout"].some((marker) => text.includes(marker)); return absence && !direct; }
