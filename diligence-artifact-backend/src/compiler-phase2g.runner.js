...
export const COMPILER_PHASE2G_RUNNER_STATUS = Object.freeze({
  runner: "compiler-phase2g.runner",
...
export async function runCompilerPhase2G({ run, internalJobId = "NORMALIZED_COMPILER", contract, readArtifacts } = {}) {
  ...
  return Object.freeze({
    ok: true,
    output,
    phase_lock_status: "LOCKED_WITH_LIMITATIONS",
...
function assertCallback(fn, label) {
  if (typeof fn !== "function") throw new Error(`COMPILER_PHASE2G_RUNNER_MISSING_CALLBACK:${label}`);
}
