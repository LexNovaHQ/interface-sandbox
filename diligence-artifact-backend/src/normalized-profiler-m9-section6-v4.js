...
export function buildNormalizedProfilerOutput({ run = {}, artifacts = {} } = {}) {
  ...
  output.normalized_report_manifest = {
    ...(output.normalized_report_manifest || {}),
    renderer_contract: { ...(output.normalized_report_manifest?.renderer_contract || {}), section_4_commercial_availability_posture_subsection_present: true, section_5_contact_consent_readiness_subsection_present: true, section_5_phase7_dap_batch_projection_present: true, section_5_dap_annexure_disclaimer_present: true, section_6_legal_cartography_summary_not_raw_index: true, section_6_legal_signal_derivation_profile_summary_present: true },
  ...
  output.final_output_handoff = {
    ...
    final_output_handoff: {
      ...
      terminal_checks: { ...(final.terminal_checks || {}), section_4_commercial_availability_posture_subsection_present: true, section_5_contact_consent_readiness_subsection_present: true, section_5_phase7_dap_batch_projection_present: true, section_5_dap_annexure_disclaimer_present: true, section_6_legal_cartography_summary_not_raw_index: true, section_6_legal_signal_derivation_profile_summary_present: true },
      compiler_trace: { ...(final.compiler_trace || {}), compiler_version: "normalized_profiler_compiler_replacement_v14_phase7_no_m10_bridge", section_4_commercial_availability_posture_subsection_present: true, section_5_contact_consent_readiness_subsection_present: true, section_5_phase7_dap_batch_projection_present: true, section_5_dap_annexure_disclaimer_present: true, section_6_legal_cartography_summary_not_raw_index: true, section_6_legal_signal_derivation_profile_summary_present: true, old_data_profile_runtime_retired: true, four_b_four_c_runtime_retired: true }
    }
  };
  return output;
}
...
function buildPhase7DapSection(section, artifacts, run) {
  ...
  return { ...base, section_title: "Data Provenance & Controls", reviewer_summary: appendSentence(buildPhase7DapReviewerSummary({ base, projection }), "Privacy contact routes and consent-manager readiness are projected from the Phase 7 CONTACT/CM batch rows where available."), subsections: [buildPhase7DapReportNoticeSubsection({ projection }), buildPhase7ContactConsentSubsection(projection), ...fullSubsections], section_limitations: [], source_artifacts_used: uniqueStrings([...sourceArtifacts, "dap_semantic_batch_route_manifest", "dap_semantic_batch_validation_manifest", "data_provenance_profile_semantic_batch_gate", "phase7_dap_report_projection"]), normalization: { ...(base.normalization || {}), section_5_contact_consent_readiness_subsection_present: true, contact_routes_source: "phase7_dap_report_projection.rows[family_code=CONTACT]", consent_manager_readiness_source: "phase7_dap_report_projection.rows[family_code=CM]", section_5_phase7_dap_batch_projection_present: true, section_5_dap_annexure_disclaimer_present: true, public_section_is_curated_summary_only: false, phase7_batch_projection_retained_in_report_body: true, phase7_batch_technical_evidence_retained_in_annexure: true, dap_field_count: fields.length, expected_dap_field_count: projection.expected_field_count || 150, all_fields_covered_once: projection.all_fields_covered_once === true, non_blocking_repair_required: Boolean(projection.non_blocking_repair_required) } };
}

function buildPhase7ContactConsentSubsection(projection) {
  const rows = Array.isArray(projection.rows) ? projection.rows.filter((row) => ["CONTACT", "CM"].includes(row.family_code)) : [];
  return { subsection_id: "privacy_contact_consent_manager_readiness", subsection_title: "Privacy Contact & Consent Manager Readiness", fields: rows.length ? rows.map((row) => row.public_field) : [{ field_id: "phase7_contact_consent_not_visible", label: "Contact / consent-manager readiness", value: { finding: "No CONTACT/CM Phase 7 batch row was available in the projected runtime artifacts.", basis_summary: "Phase 7 projection did not expose CONTACT/CM rows.", semantic_resolution_status: "SEMANTIC_MISSING_PROOF_REQUIRED" }, limitation: "Qualified review required before reliance.", qualified_review_note: "Confirm contact and consent-manager readiness during qualified review.", source_artifact: "phase7_dap_report_projection", source_path: "rows[family_code=CONTACT|CM]" }] };
}
...
function fallbackPhase7Subsections(base) { return Array.isArray(base.subsections) ? base.subsections : []; }
