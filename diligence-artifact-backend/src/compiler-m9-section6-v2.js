import { compileFinalOutputHandoff as compileBaseFinalOutputHandoff } from "./compiler.js";
...
export function compileFinalOutputHandoff({ run, artifacts }) {
  const output = compileBaseFinalOutputHandoff({ run, artifacts });
  const final = output.final_output_handoff?.final_output_handoff || {};
  const status = final.validation_status || output.normalized_report_manifest?.validation_status || run?.status || "LOCKED_WITH_LIMITATIONS";
  ...
  const normalizedSections = { ...(final.normalized_sections || {}) };
  ...
  output.final_output_handoff = {
    ...
    final_output_handoff: {
      ...
      compiler_trace: { ...(final.compiler_trace || {}), section_6_m9_summary_not_raw_index: true, compiler_version: "normalized_profiler_compiler_replacement_v7_m9_summary_section6" }
    }
  };
  return output;
}
