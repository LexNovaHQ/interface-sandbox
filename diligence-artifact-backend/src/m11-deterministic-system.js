...
const STATUS_INPUT_FIELDS = Object.freeze([
...
export function validateRegistryRows(rows, { expectedCount = EXPECTED_ACTIVE_REGISTRY_ROWS } = {}) {
  ...
  for (const row of rows) {
    ...
    for (const field of METADATA_REGISTRY_FIELDS) {
      if (isMissingRegistryValue(row[field])) metadata_limitations.push({ Threat_ID: threatId, field, severity: "WARNING_NON_BLOCKING", reason: `${threatId} missing non-critical registry metadata ${field}` });
    }
...
export function buildBatchPlan(routeRows) {
  ...
  for (const row of asArray(routeRows).filter((item) => item.route === ROUTED)) {
...
export function buildM11BatchPacket({ routePlan, batchId, upstreamArtifacts = {}, referencePacket = null }) {
  ...
  const rows = batch.expected_threat_ids.map((id) => {
    ...
    const routeRow = routeRowsById.get(id) || {};
    const parsed = parseHunterTrigger(row?.Hunter_Trigger || "");
    return { ...row, Hunter_Trigger_Parsed: parsed, deterministic_registry_spine: buildM11MaterialRowPrefill(row, routeRow), m9_legal_cartography_selection: selectLegalCartographyRowsForRegistryRow(row, legalCartography) };
  });
...
export function validateM11BatchLedger(batchLedgerRoot, expectedThreatIds = []) {
  ...
  for (const row of rows) {
    ...
    const allowed = new Set(["Threat_ID", "trigger_status", ...SEMANTIC_FIELDS]);
...
export function assembleM11AcceptedBatchLedger({ semanticBatch, batchPacket }) {
  const semanticLedger = semanticBatch?.m11_batch_registry_ledger || semanticBatch || {};
  const packet = batchPacket?.m11_batch_packet || batchPacket || {};
  const registryRowsById = new Map(asArray(packet.registry_rows).map((row) => [row.Threat_ID, row]));
  const materialRows = asArray(semanticLedger.batch_registry_ledger).map((semanticRow) => {
    const registryRow = registryRowsById.get(semanticRow.Threat_ID) || {};
    const prefill = registryRow.deterministic_registry_spine || buildM11MaterialRowPrefill(registryRow, {});
    const finalStatus = deriveM11FinalEvaluationStatus({ semanticRow });
    return pickMaterialFields({ ...prefill, target_match: semanticRow.target_match || "", basis_proof: semanticRow.basis_proof || "", control_exclusion_evaluation: semanticRow.control_exclusion_evaluation || "", evidence_source_basis: semanticRow.evidence_source_basis || "", fp_mechanism: semanticRow.applied_fp_mechanism || prefill.fp_mechanism || "", evaluation_status: finalStatus, row_limitations: semanticRow.row_limitations || "", trigger_status: semanticRow.trigger_status || "", status_inputs: semanticRow.status_inputs || {} });
  });
  return { m11_batch_registry_ledger: { ...semanticLedger, batch_registry_ledger: materialRows, accepted_batch_materialized: true, material_row_contract: "M11_THREE_LAYER_FULL_ROW_V1", final_status_owner: "backend_deterministic_finalizer" } };
}

export function deriveM11FinalEvaluationStatus({ semanticRow = {} }) {
  const inputs = semanticRow.status_inputs || {};
  if (isYes(inputs.exclude_if_met)) return "CONTROLLED_BY_EXCLUSION";
  if (isYesOrPartial(inputs.public_evidence_limitation) || isNo(inputs.evidence_sufficient) || isYes(inputs.false_positive_concern)) return "CONTROLLED_BY_PUBLIC_EVIDENCE_LIMITATION";
  if (isYesOrPartial(inputs.visible_control_present) && isYesOrPartial(inputs.visible_control_defeats_or_reduces_exposure)) return "CONTROLLED_BY_VISIBLE_CONTROL";
  if (isYesOrPartial(inputs.target_match_present) && isYesOrPartial(inputs.hunter_conditions_met) && isYesOrPartial(inputs.trigger_if_met) && !isYes(inputs.exclude_if_met) && isYesOrPartial(inputs.evidence_sufficient)) return "TRIGGERED";
  return "CONTROLLED_BY_PUBLIC_EVIDENCE_LIMITATION";
}
...
export function mergeExposureRegistryWorkpad98({ routePlan, acceptedBatches = [], batchValidations = [] }) {
  ...
  for (const routeRow of routeRows) {
    ...
    if (!batchRow) {
      ...
      registry_rows.push({ Threat_ID: threatId, registry_order: routeRow.registry_order, route: routeRow.route, route_reason: routeRow.route_reason, final_material_status: "CONTROLLED_FAILURE", workpad_status: "MISSING_ACCEPTED_BATCH_ROW", source_batch_id: routeRow.batch_id || null });
      continue;
    }
    const finalStatus = normalizeFinalMaterialStatus(batchRow);
    registry_rows.push({ Threat_ID: threatId, registry_order: routeRow.registry_order, route: routeRow.route, route_reason: routeRow.route_reason, archetype: routeRow.Archetype, surface: routeRow.Surface, trigger_status: batchRow.trigger_status || "", internal_evaluation_status: batchRow.evaluation_status || "", final_material_status: finalStatus, material_projection: pickMaterialFields(batchRow), source_batch_id: batchRow.batch_id || routeRow.batch_id || null, m12_batch_validation_status: validationIndex.get(batchRow.batch_id || routeRow.batch_id || "")?.status || "MISSING_VALIDATION", limitations: batchRow.row_limitations || "" });
  }
...
export function projectControlledProfile(workpadRoot) {
  ...
  const rows = asArray(workpad?.registry_rows).filter((row) => CONTROLLED_FINAL_STATUSES.has(row.final_material_status)).sort((a, b) => (a.registry_order ?? 9999) - (b.registry_order ?? 9999)).map((row) => pickMaterialFields(row.material_projection || row));
  return { exposure_registry_controlled_profile: { controlled_rows: rows } };
}

export function projectTriggeredProfile(workpadRoot) {
  const workpad = workpadRoot?.exposure_registry_workpad_98 || workpadRoot;
  const rows = asArray(workpad?.registry_rows).filter((row) => row.final_material_status === "TRIGGERED").sort((a, b) => (a.registry_order ?? 9999) - (b.registry_order ?? 9999)).map((row) => pickMaterialFields(row.material_projection || row));
...
function routeRegistryRow(row, routing, index) {
  ...
  return { Threat_ID: row.Threat_ID, Threat_Name: row.Threat_Name, Archetype: row.Archetype, Surface: row.Surface, FIELD21: row.FIELD21, FIELD22: row.FIELD22, FIELD23: row.FIELD23, registry_order: index + 1, route, route_reason, surfaces, registry_row: { ...row, Hunter_Trigger_Parsed: parseHunterTrigger(row.Hunter_Trigger || "") } };
}

function buildM11MaterialRowPrefill(row = {}, routeRow = {}) {
...
function deriveReviewRouteDefault(row = {}) {
  const sub = String(row.FIELD22 || deriveThreatIdPart(row.Threat_ID, 1)).toUpperCase();
  if (["PRV", "BIO", "DEC", "HRM"].includes(sub)) return "privacy/data-governance review";
  if (["IP", "CNS", "INF"].includes(sub)) return "product/legal-governance review";
  if (["TRD", "FRD"].includes(sub)) return "regulated-product review";
  return "legal-architecture review";
}
...
function collectM9ConsumptionIndex(acceptedBatches) {
  ...
  for (const artifact of asArray(acceptedBatches)) {
    ...
    for (const row of asArray(ledger?.batch_registry_ledger)) rows.push({ Threat_ID: row.Threat_ID, evidence_source_basis: row.evidence_source_basis || "" });
  }
  return rows;
}
...
function collectEvidenceBindingLedger(acceptedBatches) {
  ...
  for (const artifact of asArray(acceptedBatches)) {
    const ledger = artifact?.m11_batch_registry_ledger || artifact;
    for (const row of asArray(ledger?.batch_registry_ledger)) rows.push({ Threat_ID: row.Threat_ID, basis_proof: row.basis_proof || "", evidence_source_basis: row.evidence_source_basis || "", evaluation_status: row.evaluation_status || "" });
  }
  return rows;
}

function buildAssemblyLedger(workpadRows, controlledRows, triggeredRows) {
  const controlledSet = new Set(controlledRows.map((row) => row.Threat_ID).filter(Boolean));
  const triggeredSet = new Set(triggeredRows.map((row) => row.Threat_ID).filter(Boolean));
  return workpadRows.map((row) => ({ Threat_ID: row.Threat_ID, final_material_status: row.final_material_status, in_controlled_profile: controlledSet.has(row.Threat_ID), in_triggered_profile: triggeredSet.has(row.Threat_ID) }));
}

function buildEmissionManifest(workpadRows, controlledRows, triggeredRows) {
  return { workpad_rows: workpadRows.length, controlled_rows: controlledRows.length, triggered_rows: triggeredRows.length, split_material_outputs: true, material_row_contract: "M11_THREE_LAYER_FULL_ROW_V1" };
}

function buildRegistryLockGateResult(workpadRows, controlledRows, triggeredRows, batchValidations) {
  const failures = [];
  const controlledExpected = workpadRows.filter((row) => CONTROLLED_FINAL_STATUSES.has(row.final_material_status)).length;
