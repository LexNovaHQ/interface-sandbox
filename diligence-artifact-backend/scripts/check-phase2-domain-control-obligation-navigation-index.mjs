...
for (const catalog of OBLIGATION_CATALOGS) {
  assert.equal(catalog.may_narrow_navigation, false, `${catalog.domain_id} may not narrow navigation`);
  assert.equal(catalog.may_lock_domain, false, `${catalog.domain_id} may not lock domain`);
}

const output = buildDomainControlObligationNavigationArtifacts({
  runId: "TEST-RUN",
  artifacts: {
...
for (const gap of idx.access_gap_ledger) {
  assert.equal(gap.gap_status, "SOURCE_NOT_PRESENT_IN_PHASE1_INPUTS");
  assert.equal(gap.navigation_gap_only, true);
  assert.equal(gap.obligation_posture_conclusion, false);
}
for (const route of idx.control_source_routes) for (const pointer of route.pointers || []) {
  assert.ok(pointer.artifact_name.startsWith("lossless_root__"), `pointer must be phase1 root: ${pointer.artifact_name}`);
  assert.equal(["lossless_root__security_trust", "lossless_root__trust_compliance", "lossless_root__technical_docs_api_developer"].includes(pointer.artifact_name), false, `retired root used: ${pointer.artifact_name}`);
}
