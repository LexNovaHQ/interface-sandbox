...
for (const retired of ["archetype_codes", "archetype_derivation_basis", "package_scoped_archetype_codes", "archetype_derivation_ledger", "M8_DETERMINISTIC_FORENSIC_TRACE_CONTRACT_V2_SPLIT_TAXONOMY"]) {
  if (source.includes(retired)) throw new Error(`PHASE6_BEHAVIOR_CLASS_FORENSIC_SYNC_RETIRED_TOKEN:${retired}`);
}
for (const required of ["M8_DETERMINISTIC_FORENSIC_TRACE_CONTRACT_V3_BEHAVIOR_CLASS", "behavior_class_codes", "behavior_class_derivation_basis", "package_scoped_behavior_class_codes", "behavior_class_derivation_ledger"]) {
...
if (source !== before) {
  ...
  console.log("Phase 6 deterministic forensic Behavior Class sync: ALREADY_SYNCED");
}
