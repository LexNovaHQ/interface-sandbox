...
assert.equal(PIPELINE_CONTRACTS.P2G_PHASE_ROUTER.runtime_boundary_ends_at_operator_challenge ?? P2G_PHASE_ROUTER_CONTRACT.doctrine.runtime_boundary_ends_at_operator_challenge, true);
assert.equal(PIPELINE_CONTRACTS.P2G_PHASE_ROUTER.phase12_compiler_excluded ?? P2G_PHASE_ROUTER_CONTRACT.doctrine.phase12_compiler_excluded, true);
...
assert.equal(PIPELINE_CONTRACT_STATUS.phase2g_runtime_boundary_ends_before_compiler ?? P2G_PHASE_ROUTER_CONTRACT.doctrine.runtime_boundary_ends_at_operator_challenge, true);
assert.equal(PIPELINE_CONTRACT_STATUS.phase12_direct_profile_runtime_wired ?? P2G_PHASE_ROUTER_CONTRACT.doctrine.phase12_compiler_excluded, true);
...
assert.deepEqual(legalBucket.downstream_jobs, ["M12"]);
...
assert.equal(Object.prototype.hasOwnProperty.call(legalBucket.job_scoped_dynamic_inputs, "NORMALIZED_COMPILER"), false);
...
assert.equal(manifest.runtime_cutover_status, "RUNTIME_CUTOVER_COMPLETE_THROUGH_OPERATOR_CHALLENGE_PHASE12_EXCLUDED");
...
assert.equal(manifest.doctrine.runtime_boundary_ends_at_operator_challenge, true);
assert.equal(manifest.doctrine.phase12_compiler_excluded, true);
...
assert.equal(validationManifest.validated_doctrine.runtime_boundary_ends_at_operator_challenge, true);
assert.equal(validationManifest.validated_doctrine.phase12_compiler_excluded, true);
const serialized = JSON.stringify(manifest);
for (const forbidden of ["LOSSLESS_EVIDENCE_IS_FALLBACK", "fallback_allowed\":true", "direct_lossless_as_fallback_allowed\":true", "free_corpus_read_allowed\":true", "PHASE8_PLUS_DEFERRED", "NORMALIZED_COMPILER"]) assert.equal(serialized.includes(forbidden), false, `forbidden routing marker: ${forbidden}`);
