...
const OMITTED_PACKAGE_FILES = Object.freeze(["P2C_STEP1_OUTPUT_CONTRACT_LOCK.md", "P2C_REINVESTIGATION.md", "P2C_HYBRID_COMPILER_CONTRACT.md", "P2C_PROMPT_STACK_ORDER.md", "P2C_PACKET_VALIDATION.json"]);
const P2C_ROOTS = Object.freeze(["lossless_root__product_service", "lossless_root__platform_feature_solution", "lossless_root__technical_docs_api", "lossless_root__docs_api_data_flow", "lossless_root__integrations_ecosystem", "lossless_root__pricing_commercial_availability", "lossless_root__use_case_customer_industry", "lossless_root__support_help_resources", "lossless_root__ai_safety_transparency"]);
const CANDIDATE_ROOTS = Object.freeze(["lossless_root__product_service", "lossless_root__platform_feature_solution", "lossless_root__technical_docs_api", "lossless_root__docs_api_data_flow", "lossless_root__integrations_ecosystem", "lossless_root__pricing_commercial_availability"]);
const CONTEXT_ONLY_ROOTS = Object.freeze(["lossless_root__use_case_customer_industry", "lossless_root__support_help_resources", "lossless_root__ai_safety_transparency"]);
const WRITE_ORDER = Object.freeze(["activity_profile_deterministic_map", "activity_profile_semantic_profile", "activity_profile_source_index"]);
const ROUTE_MAPS = Object.freeze(["activity_candidate_source_locator_map", "product_capability_locator_map", "feature_mechanics_locator_map", "technical_mechanics_locator_map", "api_interaction_locator_map", "data_object_interaction_locator_map", "integration_action_locator_map", "commercial_availability_locator_map", "customer_use_context_locator_map", "support_operational_context_locator_map", "automation_transparency_context_locator_map", "human_control_context_locator_map", "external_action_context_locator_map", "input_output_object_context_locator_map"]);
const ROUTE_CLASSES = Object.freeze(["ACTIVITY_CANDIDATE_SOURCE_ROUTE", "PRODUCT_CAPABILITY_ROUTE", "FEATURE_MECHANICS_ROUTE", "TECHNICAL_MECHANICS_ROUTE", "API_INTERACTION_ROUTE", "DATA_OBJECT_INTERACTION_ROUTE", "INTEGRATION_ACTION_ROUTE", "COMMERCIAL_AVAILABILITY_ROUTE", "CUSTOMER_USE_CONTEXT_ROUTE", "SUPPORT_OPERATIONAL_CONTEXT_ROUTE", "AUTOMATION_TRANSPARENCY_CONTEXT_ROUTE", "HUMAN_CONTROL_CONTEXT_ROUTE", "EXTERNAL_ACTION_CONTEXT_ROUTE", "INPUT_OUTPUT_OBJECT_CONTEXT_ROUTE"]);
...
for (const file of EXPECTED_PACKAGE_FILES) { const text = read(`${PACKAGE_DIR}/${file}`); assert.ok(text.trim().length > 0, `${file} must not be empty`); assert.ok(text.split(/\r?\n/).length >= minLines(file), `${file} is suspiciously thin or truncated`); }
...
for (const root of P2C_ROOTS) { assert.ok(binding.includes(root), `binding missing ${root}`); assert.ok(moduleText.includes(root), `module missing ${root}`); assert.ok(referenceMap.includes(root), `reference map missing ${root}`); assert.ok(manifest.read_artifacts.includes(root), `manifest missing ${root}`); }
...
for (const artifact of WRITE_ORDER) { assert.ok(binding.includes(artifact), `binding missing write artifact ${artifact}`); assert.ok(terminal.includes(artifact), `terminal missing write artifact ${artifact}`); }
for (const routeMap of ROUTE_MAPS) { assert.ok(moduleText.includes(routeMap), `module missing ${routeMap}`); assert.ok(referenceMap.includes(routeMap), `reference map missing ${routeMap}`); }
for (const routeClass of ROUTE_CLASSES) assert.ok(validator.includes(routeClass), `validator rules missing route class ${routeClass}`);
...
for (const marker of ["Semantic", "coverage_ratio", "ready_for_compiler", "Forbidden Output", "Forbidden Source-Copy", "lossless_text"]) assert.ok(validator.includes(marker), `validator rules missing ${marker}`);
for (const marker of ["strict JSON", "No same-chat next-phase instructions", "Do not continue to:", "ACTIVITY_PROFILE_REVIEW", "Each save event must have one top-level root only"]) assert.ok(terminal.includes(marker), `terminal missing ${marker}`);
...
for (const text of [binding, controller, moduleText, referenceMap, validator, terminal]) { assert.equal(text.includes("source text may be copied"), false, "package must not allow source text copy"); assert.equal(text.includes("derive archetype"), false, "package must not instruct 2C to derive archetypes"); assert.equal(text.includes("derive surface"), false, "package must not instruct 2C to derive surfaces"); }
console.log("Phase 2C activity profile lean package quality audit: PASS");
function minLines(file) { if (file.endsWith("MANIFEST.json")) return 80; if (file.includes("REFERENCE_MAP")) return 200; if (file === "P2C_ACTIVITY_PROFILE_SOURCE_INDEX.md") return 220; if (file.includes("RUNTIME_BINDING")) return 55; return 40; }
