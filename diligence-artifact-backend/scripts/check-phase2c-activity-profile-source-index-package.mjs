...
for (const routeMap of ROUTE_MAPS) { assert.ok(moduleText.includes(routeMap), `module missing ${routeMap}`); assert.ok(referenceMap.includes(routeMap), `reference map missing ${routeMap}`); assert.ok(validator.includes(routeMap), `validator rules missing ${routeMap}`); }
