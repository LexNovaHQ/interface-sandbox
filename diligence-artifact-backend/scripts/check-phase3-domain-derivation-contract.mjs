...
for (const required of ["source_discovery_handoff", "cartography_index", "target_profile_source_index", "domain_derivation_source_index", "target_profile", "domain_selection_profile", "active_run_package_manifest"]) assert.ok(contract.reads.includes(required), `3B missing ${required}`);
...
for (const flag of ["semantic_first_deterministic_gated", "registry_driven_derivation", "registry_ladder_prompt_active", "hardcoded_domain_logic_forbidden", "new_domains_added_by_registry_not_prompt", "domain_derivation_source_index_required", "activity_profile_source_index_forbidden_until_2c_phase5", "legal_cartography_index_forbidden", "legal_signal_derivation_profile_forbidden", "legal_lossless_evidence_forbidden", "target_profile_is_context_not_proof"]) assert.equal(DOMAIN_DERIVATION_CONTRACT.boundary_rules[flag], true, `3B boundary flag missing: ${flag}`);
for (const requiredText of ["DOMAIN_DERIVATION_REGISTRY_v0.yaml", "package-catalog.v0.json", "condition-level", "evidence anchors", "domain_derivation_source_index", "activity_profile_source_index is reserved", "new supported domain", "Phase 3B must not emit Lane"]) assert.ok(prompt.includes(requiredText), `3B prompt missing structural marker: ${requiredText}`);
...
console.log(JSON.stringify({ check: "phase3 domain derivation contract", status: "PASS", enforced_gates: ["AGENT3_BOUND", "P2B_DOMAIN_DERIVATION_SOURCE_INDEX_REQUIRED", "ACTIVITY_PROFILE_INDEX_FORBIDDEN_IN_3B", "ACTIVE_REGISTRY_LADDER_PROMPT", "NO_DOMAIN_EXAMPLES_IN_PROMPT", "NEW_DOMAINS_BY_REGISTRY_NOT_PROMPT", "SCOPED_NON_LEGAL_LOSSLESS_READS", "LEGAL_INPUTS_FORBIDDEN", "REGISTRY_SEMANTIC_DERIVATION"] }, null, 2));
