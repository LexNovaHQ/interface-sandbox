...
import {
  ACTIVITY_TAXONOMY_RESOLVER_STATUS,
  resolveActivityTaxonomy
} from "../src/runtime/domain-gate/activity-taxonomy.resolver.js";
...
const fintechAi = await resolveActivityTaxonomy({
  primaryPackageId: "fintech",
  capabilityOverlayIds: ["ai-native"]
});

assert.equal(fintechAi.primary?.package_id, "fintech");
assert.ok(Array.isArray(fintechAi.primary.archetype_vocabulary));
assert.ok(fintechAi.primary.archetype_vocabulary.length > 0);
...
assert.ok(fintechAi.overlays[0].archetype_vocabulary.length > 0);
...
assert.ok(Array.isArray(saas.evidence_roots));
...
const unresolvedOverlay = await resolveActivityTaxonomy({
  primaryPackageId: "fintech",
  capabilityOverlayIds: ["nonexistent-overlay"]
});
