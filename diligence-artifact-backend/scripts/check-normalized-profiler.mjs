...
const artifacts = {
  ...
  target_feature_profile: { activities: [{ activity_reference: "A1", activity_feature_name: "Draft assistant", archetype_codes: ["CRT"], surface_context_tokens: ["Content&IP"] }], profile_level_limitations: [] },
...
for (const artifactName of NORMALIZED_SECTION_ARTIFACT_NAMES) { assert.ok(output[artifactName], `${artifactName} missing`); assert.equal(output[artifactName].artifact_name, artifactName); }
