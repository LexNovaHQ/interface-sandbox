...
function checkRegistries() {
  for (const relative of ["references/registry/AI_THREAT_REGISTRY.yaml", "references/registry/FinTech_Threat_Registry.yaml"]) {
    ...
    assert.equal(result.severity_presence_validation_status, "PASS");
    assert.equal(result.mounted_key_severity_validation_required, true);
    assert.equal(result.generic_cross_package_severity_vocabulary_forbidden, true);
  }
  ...
  for (const field of ["Pain_Tier", "Pain_Category", "Pain_Depth"]) assert.ok(invalid.failures.some((failure) => failure.endsWith(`:${field}`)));
}
...
function checkStaticMarkers() {
  ...
  for (const token of ["archetype_codes", "archetype_derivation_basis", "archetype_vocabulary", "NO_PRIMARY_ARCHETYPE_MATCH"]) assert.equal(canonicalEmitters.includes(token), false, token);
  ...
  for (const marker of ["parseMountedSeverityContract", "validateRowsAgainstMountedSeverityContract", "MOUNTED_REGISTRY_KEY"]) assert.ok(loader.includes(marker));
