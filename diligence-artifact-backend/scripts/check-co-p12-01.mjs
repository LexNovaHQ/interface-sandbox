...
async function checkRegistries() {
  const rawAiRows = parseAiThreatRegistryYaml(read("references/registry/AI_THREAT_REGISTRY.yaml"));
  const rawAiValidation = validateRegistryRows(rawAiRows, { expectedCount: rawAiRows.length });
  assert.equal(rawAiValidation.ok, false, "raw AI registry should expose the known incomplete fields before deterministic enrichment");

  const aiContext = await resolveActiveThreatRegistryContext({
    runId: "CO_P12_01_AI_TEST",
    artifacts: registryArtifacts("ai-governance", "AI_PRIMARY")
  });
  const aiRegistry = aiContext.registries[0];
  assert.equal(aiRegistry.package_id, "ai-governance");
  assert.equal(aiRegistry.validation_status, "PASS");
  assert.equal(aiRegistry.severity_validation_status, "PASS");
  assert.equal(aiRegistry.deterministic_enrichment_status, "PASS");
  assert.ok(aiRegistry.deterministic_enrichment_count >= 20);
  assert.ok(aiRegistry.deterministic_enrichments.some((entry) => entry.Threat_ID === "JDG_DEC_001" && entry.field === "Pain_Category" && entry.normalized_value === "Uncapped Money"));
  assert.ok(aiRegistry.deterministic_enrichments.some((entry) => entry.Threat_ID === "JDG_DEC_001" && entry.field === "Pain_Depth" && entry.normalized_value === "Corporate"));
  assert.ok(aiRegistry.deterministic_enrichments.some((entry) => entry.Threat_ID === "UNI_DEC_IN1" && entry.field === "Effective_Date" && entry.normalized_value === "TBD"));
  assert.ok(aiRegistry.deterministic_enrichments.some((entry) => entry.Threat_ID === "UNI_HAL_003" && entry.field === "Effective_Date" && entry.normalized_value === "Ongoing"));
  assertCompleteRegistryRows(aiRegistry.rows);
  assert.equal(aiContext.artifact.validation.deterministic_registry_enrichment_status, "PASS");
  assert.equal(aiContext.artifact.validation.mounted_key_severity_validation_status, "PASS");
  assert.equal(aiContext.artifact.deterministic_registry_enrichment.total_enrichment_count, aiRegistry.deterministic_enrichment_count);

  const fintechContext = await resolveActiveThreatRegistryContext({
    runId: "CO_P12_01_FINTECH_TEST",
    artifacts: registryArtifacts("fintech", "AI_CANDIDATE_ONLY")
  });
  const fintechRegistry = fintechContext.registries[0];
  assert.equal(fintechRegistry.package_id, "fintech");
  assert.equal(fintechRegistry.validation_status, "PASS");
  assert.equal(fintechRegistry.severity_validation_status, "PASS");
  assert.equal(fintechRegistry.deterministic_enrichment_status, "PASS");
  assertCompleteRegistryRows(fintechRegistry.rows);
  ...
  for (const field of [
    ...
    assert.ok(DETERMINISTIC_REGISTRY_SPINE_FIELDS.includes(field), `missing deterministic spine field:${field}`);
  }
  for (const field of ["registry_row_key", "package_id", "stream_id", "stream_type", "batch_id"]) {
    assert.ok(EXECUTION_CUSTODY_FIELDS.includes(field), `missing execution custody field:${field}`);
  }
}

function assertCompleteRegistryRows(rows) {
  for (const row of rows) {
    for (const field of ["Pain_Tier", "Pain_Category", "Pain_Depth", "Effective_Date"]) {
      assert.ok(String(row[field] ?? "").trim(), `${row.Threat_ID}:${field}`);
    }
  }
}
...
function checkSemanticFirewall() {
  ...
  assert.equal(
    validateSemanticLedger({ semanticOutput: valid, batch, routePlan })
      .exposure_registry_batch_validation.status,
    "PASS"
  );

  const illegal = semanticLedger(batch, { ...semanticRow(), Pain_Tier: "T1" });
  const result = validateSemanticLedger({ semanticOutput: illegal, batch, routePlan })
    .exposure_registry_batch_validation;
  assert.equal(result.status, "REPAIR_REQUIRED");
  assert.ok(result.failures.some(
    (failure) => failure.includes("MODEL_FIELD_FORBIDDEN") && failure.includes("Pain_Tier")
  ));
}
...
function checkExactGate() {
  ...
  for (const status of [
    ...
    assert.equal(compatibility(status, []).validation.status, "CONTROLLED_FAILURE");
  }
...
function checkStaticMarkers() {
  ...
  for (const token of [
    ...
    "NO_PRIMARY_ARCHETYPE_MATCH"
  ]) {
    assert.equal(canonicalEmitters.includes(token), false, token);
  }

  const p10 = read("src/phases/10-exposure-profile/phase10-semantic-finalization.js");
  for (const marker of [
    "deterministic_registry_spine_read_only",
    "model_must_not_emit_registry_spine_fields",
    "phase10_report_row.v1.complete_registry_spine"
  ]) {
    assert.ok(p10.includes(marker), marker);
  }
  ...
  return {
    domain_derivation_profile: {
      primary_domain_derivation: {
        selected_package: primaryPackage,
        status: "LOCKED"
      },
      ai_mount_derivation: {
...
function compatibility(status, warnings) {
  return rootCompatibility(artifacts(status, warnings));
}
...
function artifacts(status, advisoryWarnings) {
  ...
  return {
    ...
    exposure_registry_route_plan: {
      route_rows: [{ registry_row_key: row.registry_row_key }]
    },
    exposure_registry_workpad_98: {
      registry_rows: [{
        ...row,
        final_material_status: "TRIGGERED",
