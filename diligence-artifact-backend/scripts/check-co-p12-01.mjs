...
checkDoctrine();
checkPhase5Schema();
checkPhase5PositiveAndNegativeFixtures();
checkRegistrySeverityAndSpine();
checkSemanticRegistryFieldFirewall();
checkExactPhase11Gate();
checkActiveSourceMarkers();
...
function checkPhase5Schema() {
  assert.deepEqual(CLASSIFICATION_BLOCK_FIELDS, [
    "package_id", "behavior_class_codes", "behavior_class_derivation_basis",
    "surface_context_tokens", "surface_derivation_basis"
  ]);
  assert.deepEqual(OVERLAY_CLASSIFICATION_BLOCK_FIELDS, [
    "package_id", "overlay_id", "behavior_class_codes", "behavior_class_derivation_basis",
    "surface_context_tokens", "surface_derivation_basis"
  ]);
}

function checkPhase5PositiveAndNegativeFixtures() {
  const taxonomy = {
    ...
    primary: {
      ...
      surface_axes: [{ axis_id: "surface", tokens: [{ token: "Transaction-Data" }] }]
    },
    overlays: [{
      ...
      surface_axes: [{ axis_id: "surface", tokens: [{ token: "PII" }] }]
    }],
    limitations: [],
    routing_limitations: [],
    excluded_regulatory_overlay_ids: ["rbi-context"]
  };
  ...
  const output = {
    target_feature_profile: {
      activities: [{
        ...
        activity_feature_name: "Automated underwriting",
        activity_candidate_summary: "Assesses a submitted credit application.",
        mechanics_proof: "The supplied product material states the assessment workflow.",
        autonomy_human_control_signal: "Automated assessment with a stated review step.",
        data_content_object_touched: "Credit application data.",
        external_internal_action_signal: "Produces an underwriting recommendation.",
        primary_classification: {
          package_id: "fintech",
          behavior_class_codes: ["PAY"],
          behavior_class_derivation_basis: [basis("PAY", "Payments / Money Movement")],
      ...
      commercial_availability_posture: {
        ...
        free_trial_freemium_signal: "No public free tier identified.",
        beta_pilot_early_access_signal: "No public beta limitation identified.",
        paid_production_enterprise_plan_signal: "Paid enterprise availability is publicly described.",
        evidence_basis: ["Public commercial availability statements support the posture."],
        limitation: "Private contract terms were not reviewed."
      },
      profile_level_limitations: [],
      mounted_taxonomy_ref: {
        primary_package_id: "fintech",
        primary_key_version: "1.0",
        overlays: [{ overlay_id: "ai-native", package_id: "ai-governance", key_version: "4.0" }]
      }
    }
  };
  ...
  assert.notDeepEqual(
    output.target_feature_profile.activities[0].primary_classification.behavior_class_codes,
    output.target_feature_profile.activities[0].overlay_classifications[0].behavior_class_codes
  );
  assert.notDeepEqual(
    output.target_feature_profile.activities[0].primary_classification.surface_context_tokens,
    output.target_feature_profile.activities[0].overlay_classifications[0].surface_context_tokens
  );
...
function checkRegistrySeverityAndSpine() {
  const registryPaths = [
    "references/registry/AI_THREAT_REGISTRY.yaml",
    "references/registry/FinTech_Threat_Registry.yaml"
  ];
  for (const relative of registryPaths) {
    const rows = parseAiThreatRegistryYaml(read(relative));
    assert.ok(rows.length > 0, `${relative} must contain rows`);
    const validation = validateRegistryRows(rows, { expectedCount: rows.length });
    assert.equal(validation.ok, true, `${relative}: ${validation.failures.join("|")}`);
    assert.equal(validation.severity_validation_status, "PASS");
  }

  for (const field of ["Pain_Tier", "Pain_Category", "Pain_Depth", "Behavior_Class", "FP_Impact", "Provenance"]) {
    assert.ok(DETERMINISTIC_REGISTRY_SPINE_FIELDS.includes(field), `spine missing ${field}`);
  }
  for (const field of ["registry_row_key", "package_id", "stream_id", "stream_type", "batch_id"]) {
    assert.ok(EXECUTION_CUSTODY_FIELDS.includes(field), `custody missing ${field}`);
  }

  const bad = parseAiThreatRegistryYaml(read(registryPaths[0])).slice(0, 1);
  bad[0] = { ...bad[0], Pain_Tier: "", Pain_Category: "", Pain_Depth: "" };
  const invalid = validateRegistryRows(bad, { expectedCount: 1 });
  assert.equal(invalid.ok, false);
  assert.ok(invalid.failures.some((failure) => failure.includes("REGISTRY_PAIN_TIER_MISSING")));
  assert.ok(invalid.failures.some((failure) => failure.includes("REGISTRY_PAIN_CATEGORY_MISSING")));
  assert.ok(invalid.failures.some((failure) => failure.includes("REGISTRY_PAIN_DEPTH_MISSING")));
}

function checkSemanticRegistryFieldFirewall() {
  const batch = {
    batch_id: "PRIMARY__AI_GOVERNANCE__UNI__001",
    batch_group: "UNI",
...
function checkExactPhase11Gate() {
  const base = compatibilityArtifacts("PASS", []);
  assert.equal(buildPhase10CompilerCompatibility({ artifacts: base }).phase10_downstream_compatibility.validation.status, "PASS");

  const withLimitation = compatibilityArtifacts("PASS_WITH_LIMITATION", [{
    ...
    materiality_analysis: "The report must preserve the public-evidence limitation."
  }]);
  assert.equal(buildPhase10CompilerCompatibility({ artifacts: withLimitation }).phase10_downstream_compatibility.validation.status, "PASS_WITH_LIMITATION");

  for (const status of ["LOCKED", "LOCKED_WITH_LIMITATIONS", "REINVESTIGATION_REQUIRED", "CONTROLLED_FAILURE", ""]) {
    const artifacts = compatibilityArtifacts(status, []);
    assert.equal(buildPhase10CompilerCompatibility({ artifacts }).phase10_downstream_compatibility.validation.status, "CONTROLLED_FAILURE", status || "missing");
  }

  const noHandoff = compatibilityArtifacts("PASS", []);
  noHandoff.challenge_gate.compiler_handoff_allowed = false;
  assert.equal(buildPhase10CompilerCompatibility({ artifacts: noHandoff }).phase10_downstream_compatibility.validation.status, "CONTROLLED_FAILURE");

  const noFingerprint = compatibilityArtifacts("PASS", []);
  noFingerprint.challenge_gate.final_gate_fingerprint = "";
  assert.equal(buildPhase10CompilerCompatibility({ artifacts: noFingerprint }).phase10_downstream_compatibility.validation.status, "CONTROLLED_FAILURE");
}

function checkActiveSourceMarkers() {
  const files = [
  ...
  for (const retired of ["archetype_codes", "archetype_derivation_basis", "archetype_vocabulary", "NO_PRIMARY_ARCHETYPE_MATCH"]) {
    assert.equal(joined.includes(retired), false, `retired active Phase 5 token remains: ${retired}`);
  }

  const p10 = read("src/phases/10-exposure-profile/phase10-semantic-finalization.js");
  for (const marker of ["deterministic_registry_spine_read_only", "model_must_not_emit_registry_spine_fields", "phase10_report_row.v1.complete_registry_spine"]) assert.ok(p10.includes(marker), `Phase 10 marker missing: ${marker}`);
  ...
  assert.equal(gate.includes('"LOCKED", "LOCKED_WITH_LIMITATIONS"'), false);
}
...
function basis(code, name) {
  return {
    ...
    conditions_satisfied: ["The supplied material satisfies the mounted key condition."],
    trigger_if_applied: "The mounted-key trigger is satisfied.",
    exclude_if_checked: "No mounted-key exclusion defeats the classification.",
    material_basis: "The reviewed product mechanics support this classification.",
    limitation: "Public-material classification only."
  };
}
...
function semanticRow() {
  return {
    ...
    target_match: "The target uses passive assent.",
    basis_proof: "The reviewed signup description supports the match.",
    control_exclusion_evaluation: "No defeating visible control was identified.",
    evidence_source_basis: "Public product and legal materials.",
    applied_fp_mechanism: "Passive assent may fail to bind.",
    row_limitations: "Private assent logs were not reviewed.",
    status_inputs: {
...
function compatibilityArtifacts(status, advisoryWarnings) {
  const row = completeMaterialRow();
  return {
...
function completeMaterialRow() {
  return {
    ...
    Authority_IN: "Indian Contract Act 1872",
    Authority_EU: "—",
    ...
    Legal_Pain: "Passive assent may not form an enforceable agreement.",
    FP_Mechanism: "The signup uses passive assent.",
    FP_Impact: "Contract protections may not bind.",
    Lex_Nova_Fix: "Use conspicuous affirmative assent.",
    ...
    FIELD22: "CNS",
    FIELD23: 1,
    target_match: "The reviewed flow matches.",
    evaluation_status: "TRIGGERED",
    basis_proof: "Public signup material.",
    control_exclusion_evaluation: "No defeating control identified.",
    evidence_source_basis: "Public product and legal materials.",
    applied_fp_mechanism: "Passive assent may fail to bind.",
    row_limitations: "Private assent logs not reviewed.",
