...
const phaseRoot = "src/phases/10-exposure-profile";
...
const synchronizedWrapper = readFileSync(`${phaseRoot}/m11-final-synchronized.runner.js`, "utf8");
...
const phaseFiles = ["exposure-profile.runner.js", "m11-final-synchronized.runner.js", "m11-orchestrator-m11v2.js", "m11-deterministic-system-m11v2.js", "m11-deterministic-system.js", "m11-status-finalizer.js", "m11-deterministic-forensics-m11v2.js", "m11-deterministic-forensics.js", "m11-forensic-trace-index.js", "m11-lep-deterministic.js", "m11-batch-evidence-resolver.js"];
for (const file of phaseFiles) assert.equal(existsSync(`${phaseRoot}/${file}`), true, `missing phase-owned M11 file ${file}`);

assert.ok(phaseRunner.includes("./m11-final-synchronized.runner.js"));
assert.ok(synchronizedWrapper.includes("./m11-orchestrator-m11v2.js"));
assert.ok(synchronizedWrapper.includes("v18_final_runtime_package_downstream_sync"));
assert.ok(orchestrator.includes('phase_owned_path: "src/phases/10-exposure-profile"'));
...
assert.ok(pipeline.includes('../../phases/10-exposure-profile/exposure-profile.runner.js'), "central pipeline must import Phase 9 directly");
