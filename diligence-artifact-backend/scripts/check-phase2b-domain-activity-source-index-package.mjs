...
const PACKAGE_DIR = "agent-packages/phase_2b_domain_activity_source_index";
const read = (file) => fs.readFileSync(path.join(ROOT, file), "utf8");

const EXPECTED_PACKAGE_FILES = Object.freeze([
  "00_RUNTIME_CONTROLLER_PHASE2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md",
  "00_TERMINAL_RECEIPT_RULES_PHASE2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md",
  "00_VALIDATOR_RULES_PHASE2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md",
  "P2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md",
  "P2B_DOMAIN_ACTIVITY_SOURCE_INDEX_REFERENCE_MAP.yaml",
  "P2B_DOMAIN_ACTIVITY_SOURCE_INDEX_RUNTIME_BINDING_PACKET.yaml",
...
assert.equal(fs.existsSync(packagePath), true, "Phase 2B package folder must exist");
...
const binding = read(`${PACKAGE_DIR}/P2B_DOMAIN_ACTIVITY_SOURCE_INDEX_RUNTIME_BINDING_PACKET.yaml`);
const controller = read(`${PACKAGE_DIR}/00_RUNTIME_CONTROLLER_PHASE2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md`);
const moduleText = read(`${PACKAGE_DIR}/P2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md`);
const referenceMap = read(`${PACKAGE_DIR}/P2B_DOMAIN_ACTIVITY_SOURCE_INDEX_REFERENCE_MAP.yaml`);
const validator = read(`${PACKAGE_DIR}/00_VALIDATOR_RULES_PHASE2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md`);
const terminal = read(`${PACKAGE_DIR}/00_TERMINAL_RECEIPT_RULES_PHASE2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md`);
const manifest = JSON.parse(read(`${PACKAGE_DIR}/P2B_PACKET_MANIFEST.json`));

assert.equal(manifest.packet_name, "P2B_DOMAIN_ACTIVITY_SOURCE_INDEX_PACKET");
assert.equal(manifest.package_status, "LEAN_PACKAGE_CONTRACT_ONLY");
assert.equal(manifest.runtime_wiring_changed, false);
assert.equal(manifest.runtime_job_id, "P2B_DOMAIN_ACTIVITY_SOURCE_INDEX");
...
console.log("Phase 2B domain activity lean package quality audit: PASS");

function minLines(file) {
  if (file.endsWith("MANIFEST.json")) return 60;
  if (file.includes("REFERENCE_MAP")) return 170;
  if (file === "P2B_DOMAIN_ACTIVITY_SOURCE_INDEX.md") return 180;
  if (file.includes("RUNTIME_BINDING")) return 110;
  return 70;
}
