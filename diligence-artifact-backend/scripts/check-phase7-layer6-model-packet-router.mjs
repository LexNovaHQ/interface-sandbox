...
const validation = validatePhase7ModelWorkPacketManifest(packetManifest, { dapFieldPrefillMatrix: prefillMatrix });

assert.equal(packetManifest.artifact_type, "dap_model_work_packet_manifest");
assert.equal(packetManifest.router_policy.deterministic_only, true);
assert.equal(packetManifest.router_policy.no_model_call, true);
assert.equal(packetManifest.router_policy.bounded_packets_only, true);
assert.equal(packetManifest.router_policy.no_corpus_access, true);
assert.equal(packetManifest.router_policy.no_excerpts, true);
assert.equal(packetManifest.router_policy.no_full_document_payloads, true);
assert.equal(packetManifest.router_policy.no_final_dap_profile_values, true);
assert.ok(packetManifest.model_required_field_count > 0);
assert.ok(packetManifest.packet_count > 0);
assert.equal(packetManifest.unrouted_model_required_field_ids.length, 0);
assert.equal(validation.status, "PASS");
assert.equal(validation.all_model_required_fields_routed, true);
assert.equal(validation.bounded_packets_only, true);
...
for (const packet of packetManifest.packets) {
  ...
  assert.ok(packet.model_packet_family);
  assert.equal(packet.field_refs.length, packet.field_count);
  assert.equal(packet.packet_input_boundary.field_rules_only, true);
  assert.equal(packet.packet_input_boundary.ids_only_for_supporting_material, true);
  assert.equal(packet.packet_input_boundary.no_source_text, true);
  assert.equal(packet.packet_input_boundary.no_final_values, true);
}

console.log("Phase 7 Layer 6 model packet router: PASS");
