...
assert.equal(CARTOGRAPHY_INDEX_CONTRACT.implementation_status, "INPUT_CONTRACT_SYNCED_TO_PHASE1_V5_17_ROOT_MULTI_DOMAIN_UNION_PROBE_P2C_ACTIVITY_SOURCE_INDEX");
...
assert.equal(CARTOGRAPHY_INDEX_CONTRACT.phase2_profile_input_routes.activity_profile_source_index.domain_package_specific_activity_taxonomy_deferred_to_phase5, true);
...
assert.deepEqual(CARTOGRAPHY_LAYER5_ARTIFACT_NAMES, ["data_privacy_navigation_index", "cartography_index", "cartography_validation_manifest"]);
assert.ok(CARTOGRAPHY_ARTIFACT_NAMES.includes("target_profile_source_index"));
assert.ok(CARTOGRAPHY_ARTIFACT_NAMES.includes("domain_derivation_source_index"));
assert.ok(CARTOGRAPHY_ARTIFACT_NAMES.includes("activity_profile_source_index"));
assert.equal(CARTOGRAPHY_LAYER5_ARTIFACT_NAMES.includes("target_profile_source_index"), false);
assert.equal(CARTOGRAPHY_LAYER5_ARTIFACT_NAMES.includes("domain_derivation_source_index"), false);
assert.equal(CARTOGRAPHY_LAYER5_ARTIFACT_NAMES.includes("activity_profile_source_index"), false);
...
assert.ok(PIPELINE_CONTRACTS.P2B_DOMAIN_DERIVATION_SOURCE_INDEX.prompt_files.some((file) => file.includes("phase_2b_domain_derivation_source_index")));
...
assert.equal(PIPELINE_CONTRACTS.P2C_ACTIVITY_PROFILE_SOURCE_INDEX.next, "P2_INDEX_COMPILER_VALIDATION");
assert.ok(PIPELINE_CONTRACTS.P2C_ACTIVITY_PROFILE_SOURCE_INDEX.prompt_files.some((file) => file.includes("phase_2c_activity_profile_source_index")));
assert.ok(PIPELINE_CONTRACTS.P2_INDEX_COMPILER_VALIDATION.reads.includes("target_profile_source_index"));
assert.ok(PIPELINE_CONTRACTS.P2_INDEX_COMPILER_VALIDATION.reads.includes("domain_derivation_source_index"));
assert.ok(PIPELINE_CONTRACTS.P2_INDEX_COMPILER_VALIDATION.reads.includes("activity_profile_source_index"));
assert.equal(PIPELINE_CONTRACTS.P2_INDEX_COMPILER_VALIDATION.writes.includes("target_profile_source_index"), false);
assert.equal(PIPELINE_CONTRACTS.P2_INDEX_COMPILER_VALIDATION.writes.includes("domain_derivation_source_index"), false);
assert.equal(PIPELINE_CONTRACTS.P2_INDEX_COMPILER_VALIDATION.writes.includes("activity_profile_source_index"), false);

const expectedJobs = ["P2_SOURCE_INVENTORY_CARTOGRAPHY", "P2_LOCATOR_SPINE", "P2_PROFILE_ROUTE_MATRIX", "P2_SEMANTIC_NAVIGATION_OVERLAY", "M9", "P2A_TARGET_PROFILE_SOURCE_INDEX", "P2B_DOMAIN_DERIVATION_SOURCE_INDEX", "P2C_ACTIVITY_PROFILE_SOURCE_INDEX", "P2_INDEX_COMPILER_VALIDATION"];
...
for (const artifact of DOMAIN_DERIVATION_SOURCE_INDEX_ARTIFACT_NAMES) assert.ok(centralPhase2.terminal_outputs.includes(artifact), `central phase missing ${artifact}`);
for (const artifact of ACTIVITY_PROFILE_SOURCE_INDEX_ARTIFACT_NAMES) assert.ok(centralPhase2.terminal_outputs.includes(artifact), `central phase missing ${artifact}`);

for (const forbidden of ["legal_governance_source_index", "data_provenance_source_index", "compatibility.adapter", "CompatibilityArtifacts", "compatibility_adapter"]) assert.equal(activeText.includes(forbidden), false, `forbidden old/input-pollution marker present: ${forbidden}`);
...
for (const required of ["phase1_v5", "P2A_TARGET_PROFILE_SOURCE_INDEX", "target_profile_deterministic_map", "target_profile_semantic_profile", "target_profile_source_index", "P2B_DOMAIN_DERIVATION_SOURCE_INDEX", "domain_derivation_deterministic_map", "domain_derivation_semantic_profile", "domain_derivation_source_index", "P2C_ACTIVITY_PROFILE_SOURCE_INDEX", "activity_profile_deterministic_map", "activity_profile_semantic_profile", "activity_profile_source_index_owned_by_2c", "lossless_root__regulatory_licensing_status", "lossless_root__grievance_complaints", "regulatory_governance_locator", "grievance_redressal_locator", "target_profile_legal_signal_locators_owned_by_2a", "full_legal_governance_cartography_owned_by_2e", "cartography_validation_manifest", "contains_lossless_text: false", "semantic_guidance_only"]) assert.ok(activeText.includes(required), `active files missing ${required}`);
...
assert.equal(PIPELINE_CONTRACT_STATUS.phase2b_domain_derivation_source_index_owns_domain_derivation_index, true);
assert.equal(PIPELINE_CONTRACT_STATUS.phase2c_activity_profile_source_index_runtime_wired, true);
assert.equal(PIPELINE_CONTRACT_STATUS.phase2c_activity_profile_source_index_owns_activity_index, true);
...
console.log("Phase 2 cartography + P2A/P2B/P2C runtime wiring validator: PASS");
