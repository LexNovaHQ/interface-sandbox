...
function findOrCreateSubsection(section, subsectionId) {
  let subsection = section.subsections.find((item) => item.subsection_id === subsectionId);
  if (!subsection) {
    subsection = { subsection_id: subsectionId, subsection_title: subsectionId, fields: [] };
    section.subsections.push(subsection);
  }
  return subsection;
}

function buildFixtureValue(selector, row) {
  const sample = sampleAnswer(row);
  const valuePath = selector.split(".value")[1] || "";
  if (!valuePath) return sample;
  if (valuePath.startsWith("[*].")) return [{ [valuePath.slice(4)]: sample }];
  const keys = valuePath.replace(/^\./, "").split(".").filter(Boolean);
  return keys.reduceRight((nested, key) => ({ [key]: nested }), sample);
}

function sampleAnswer(row) {
  const options = Array.isArray(row.answer_options) ? row.answer_options : String(row.answer_options || "").split(",").map((value) => value.trim()).filter(Boolean);
  if ((row.answer_type === "dropdown" || row.answer_type === "select") && options.length) return options.includes("Yes") ? "Yes" : options[0];
  return row.demo_prefill_value || "Visible public signal for checker fixture";
}

function mergeFixtureValue(left, right) {
  if (Array.isArray(left) || Array.isArray(right)) return Array.isArray(left) ? left : right;
