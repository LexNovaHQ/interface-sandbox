...
for (const [before, after] of [
  ...
  if (source.includes(before)) {
    ...
    changed = true;
  } else if (after && !source.includes(after)) {
    throw new Error(`PHASE1_CONTRACT_CHECK_SYNC_MARKER_MISSING:${before.slice(0, 100)}`);
  }
}

if (!source.includes('assert.equal(fs.existsSync(path.join(ROOT, "src/phase-contracts.js")), false')) {
  throw new Error("PHASE1_CONTRACT_CHECK_SYNC_REQUIRED_ASSERTION_MISSING");
}
...
if (changed) {
  ...
  console.log("Phase 1 source-discovery check uses current runtime contract authority: ALREADY_SYNCED");
}
