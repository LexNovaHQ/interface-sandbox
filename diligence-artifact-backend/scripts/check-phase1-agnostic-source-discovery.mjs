...
assert.ok(extractionService.includes("saved_root_artifacts"), "source_family_index must advertise saved root artifacts");
...
assert.ok(validator.includes("root_artifact_manifest"), "source extraction validator must validate virtual root manifest");
...
assert.ok(pipeline.includes("sparse_lossless_root_resolution_enabled: true"), "pipeline must advertise sparse lossless root resolution");
assert.ok(pipeline.includes("resolveLosslessRootArtifact"), "pipeline must resolve virtual lossless roots through source_family_index");
assert.ok(pipeline.includes("root_artifact_manifest"), "pipeline root resolver must use source_family_index.root_artifact_manifest");
assert.ok(pipeline.includes("UNSAVED_EMPTY"), "pipeline root resolver must return controlled empty roots for unsaved sparse roots");
