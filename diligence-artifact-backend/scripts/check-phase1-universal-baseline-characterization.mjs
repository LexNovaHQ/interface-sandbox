...
import { buildUniversalSourceExtractionArtifactSet } from "../src/phases/01-source-discovery/services/universal-source-extraction.service.js";
...
async function executeFixture(fixture, origin, runId) {
  return buildUniversalSourceExtractionArtifactSet({
...
function manifestRow(page, index, origin) {
  ...
  return {
    ...
    canonical_url_key: `${page.entity_id}|${origin}${page.path}`,
    ...
    source_disposition: page.evidence_lane === "legal_instrument" ? "LEGAL_INSTRUMENT" : page.variant_family ? "SELECTED_PARTIAL_CONTRIBUTOR" : "SELECTED_CANONICAL",
    extraction_scope: page.evidence_lane === "legal_instrument" ? "FULL_DOCUMENT" : variant ? "STRUCTURED_COVERAGE_ONLY" : page.unique_material?.length ? "SELECTED_UNIQUE_SECTIONS" : "FULL_MAIN_CONTENT",
...
function runAssertions(sarvamArtifacts, paytmArtifacts) {
  ...
  check("SARVAM_VARIANT_SCOPE_CONTROL", () => {
    ...
    truthy(rows.length >= 2, "Sarvam retained variant rows missing");
...
function readFixture(name) { return JSON.parse(fs.readFileSync(path.join(FIXTURE_ROOT, name), "utf8")); }
function check(id, assertion) { try { assertion(); } catch (error) { failures.push({ id, message: error?.message || String(error) }); } }
