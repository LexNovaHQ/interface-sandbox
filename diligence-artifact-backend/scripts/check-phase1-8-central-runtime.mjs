...
for(const r of ["target_profile_source_index","domain_derivation_source_index","activity_profile_source_index","data_privacy_navigation_index","domain_control_obligation_navigation_index","legal_cartography_index","legal_signal_derivation_profile","domain_selection_profile","active_run_package_manifest",...tRoots,...dRoots,...aRoots,...dpRoots,...dconiRoots])assert.ok(PIPELINE_CONTRACTS.P2G_PHASE_ROUTER.reads.includes(r),`2G missing ${r}`);
...
assert.equal(PIPELINE_CONTRACTS.P2_INDEX_COMPILER_VALIDATION.writes.includes("phase_routing_manifest"),false,"P2_INDEX must not write routing manifest");
...
assert.equal(PIPELINE_CONTRACTS.DATA_PROVENANCE_PROFILE_LAYER4.reads.includes("phase_routing_manifest"),true,"Phase 7 must read 2G route manifest after first patch");
assert.equal(PIPELINE_CONTRACTS.DATA_PROVENANCE_PROFILE_FORENSICS.next,"M11");
for(const r of ["source_discovery_handoff","cartography_index","phase_routing_manifest","target_profile_source_index","legal_signal_derivation_profile","domain_selection_profile","active_run_package_manifest",...tRoots])assert.ok(PIPELINE_CONTRACTS.M7_TARGET_PROFILE.reads.includes(r),`3A missing ${r}`);
...
for(const r of ["source_discovery_handoff","cartography_index","phase_routing_manifest","target_profile_source_index","domain_derivation_source_index","target_profile","domain_selection_profile","active_run_package_manifest",...dRoots])assert.ok(PIPELINE_CONTRACTS.P3_DOMAIN_DERIVATION_LAYER.reads.includes(r),`3B missing ${r}`);
