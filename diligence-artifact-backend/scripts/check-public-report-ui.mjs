...
assert.ok(reportHtml.includes("report-table-overrides.css?v=full-public-tables-20260702"));
...
assert.ok(reportJs.includes("Full public table — scroll horizontally to view all columns."));
...
assert.ok(reportTableCss.includes("min-width: 960px"));
...
assert.equal(reportRenderer.includes("suppressed_row_count"), true);
...
assert.equal(fullPayload.public_technical_annexure.report_body_inlines_full_public_projection_tables, true);
assert.equal(fullPayload.sections.some((section) => section.section_id === "public_technical_annexure"), false);
assert.equal(JSON.stringify(fullPayload).includes("suppressed_row_count"), false);
assert.equal(JSON.stringify(fullPayload).includes("displayed_rows"), false);
const diagnosisField = section7.subsections.find((subsection) => subsection.subsection_title === "Exposure Diagnosis Table").fields[0];
assert.equal(Array.isArray(diagnosisField.value), true);
assert.equal(diagnosisField.value.length, 30);
