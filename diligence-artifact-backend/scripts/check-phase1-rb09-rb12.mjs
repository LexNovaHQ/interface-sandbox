...
console.log(JSON.stringify({check:"phase1 RB09 RB12",status:"PASS",decisions:selection.decisions.length,extract_rows:3,exact_duplicates_suppressed:1},null,2));
...
function fp(id,hash,blocks){return {candidate_id:id,fingerprint_id:`FP.${id}`,fetch_status:"FETCHED",exact_content_hash:hash,block_hashes:blocks.map((x,i)=>({block_index:i+1,sha256:hashBlock(x),character_count:x.length})),near_duplicate_signature:{sampled_hashes:blocks.map((x)=>hashBlock(x).slice(0,16))},template_signature:"sarvam.ai/template",title:id==="C4"?"Privacy Policy":"Translation",fingerprint_bytes_used:blocks.join(" ").length,warnings:[]};}
