...
const candidates = [candidate("CANON.EMPTY_SHELL", "/app"), candidate("CANON.PLACEHOLDER", "/loading"), candidate("CANON.LEGAL_SHELL", "/privacy-policy"), candidate("CANON.MATERIAL", "/product")];
...
for (const id of ["CANON.EMPTY_SHELL", "CANON.PLACEHOLDER", "CANON.LEGAL_SHELL"]) {
  const item = fingerprint(pass, id);
...
assert.ok(material.exact_content_hash && material.block_hashes.length > 0);
...
const internalEvidenceModel = { source_candidates: candidates.map((item) => ({ canonical_url: item.canonical_url, primary_root: item.canonical_url.endsWith("privacy-policy") ? "privacy_data_processing" : "product_service", root_candidates: [item.canonical_url.endsWith("privacy-policy") ? "privacy_data_processing" : "product_service"], feature_cluster: item.canonical_url.endsWith("product") ? "product" : "application_surface", evidence_lane: item.canonical_url.endsWith("privacy-policy") ? "legal_instrument" : "commercial_product", legal_instrument_candidate: item.canonical_url.endsWith("privacy-policy") })) };
...
corrupted.manifest_sources.push({ ...materialRow, manifest_id: "product_service.URL.999", canonical_candidate_id: "CANON.CORRUPTED", canonical_identity: "entity|https://example.test/corrupted", canonical_url: "https://example.test/corrupted", fetch_url: "https://example.test/corrupted", fingerprint_fetch_status: "FETCHED_NO_MATERIAL_CONTENT", fingerprint_extraction_eligible: false, content_materiality: { status: "NO_MATERIAL_CONTENT" }, exact_content_hash: null, selected_block_hashes: [] });
...
console.log(JSON.stringify({ check: "phase1 RB18 material-content extraction gate", status: "PASS", audited_urls: finalManifest.manifest_forensics.audited_rows, material_pages_authorized: finalManifest.manifest_sources.length, non_material_pages_rejected_before_clean_manifest: 3, empty_legal_route_confirmed: legalShellClassification.confirmed_legal_instrument, post_dedupe_physical_source_of_truth_proved: true, suppressed_duplicate_reference_allowed_without_physical_text: true, clean_manifest_fail_loud_boundary_proved: true }, null, 2));
...
function postDedupePhysicalFixture() {
  ...
  return { source_family_index: { discovered_source_index: [{ source_id: "product_service.SRC.001", common_root: "product_service" }, { source_id: "product_service.SRC.002", common_root: "product_service" }, { source_id: "product_service.SRC.003", common_root: "product_service" }], manifest_only_index: [{ source_id: "product_service.SRC.002", extraction_status: "SUPPRESS_EXACT_DUPLICATE", duplicate_owner_source_id: "product_service.SRC.001" }, { source_id: "product_service.SRC.003", extraction_status: "SUPPRESS_NO_RETAINED_MATERIAL_AFTER_SCOPE", canonical_owner_candidate_id: "CANON.OWNER", suppression_reason: "DECLARED_EXTRACTION_SCOPE_RETAINED_NO_BODY_OR_UNIQUE_BLOCK" }], root_artifact_manifest: { product_service: { common_root: "product_service", required_artifacts: ["lossless_root__product_service"], source_count: 1, source_ids: ["product_service.SRC.001"] } }, block_dedupe_forensics: { roots: { product_service: { source_forensics: [{ source_id: "product_service.SRC.001", action: "RETAIN" }, { source_id: "product_service.SRC.002", action: "SUPPRESS_EXACT_DUPLICATE", duplicate_owner_source_id: "product_service.SRC.001" }] } } } }, lossless_root__product_service: { common_root: "product_service", sources: [{ source_id: "product_service.SRC.001", extraction_scope: "FULL_MAIN_CONTENT", lossless_text: materialText }] }, legal_doc_inventory: { documents_found: [] } };
}
function candidate(id, pathname) { const url = `https://example.test${pathname}`; return { candidate_id: id, canonical_identity: `entity|${url}`, entity_id: "entity", entity_status: "PRIMARY_TARGET", canonical_url: url, fetch_url: url, aliases: [], member_candidate_ids: [], root_candidates: [pathname === "/privacy-policy" ? "privacy_data_processing" : "product_service"], legacy_manifest_ids: [`LEGACY.${id}`], extraction_authorized_by_legacy_manifest: true }; }
...
function legacyManifest(items) { return { run_id: "RB18-MATERIAL-GATE", target: "example.test", target_url: "https://example.test/", target_boundary: { primary_entity_id: "entity" }, manifest_sources: items.map((item, index) => ({ manifest_id: `LEGACY.${item.candidate_id}`, common_root: item.canonical_url.endsWith("privacy-policy") ? "privacy_data_processing" : "product_service", root_traversal_policy: "PRIMARY_FULL_EXTRACT", canonical_url: item.canonical_url, canonical_url_key: item.canonical_url, fetch_url: item.fetch_url, route_type: item.canonical_url.endsWith("privacy-policy") ? "privacy_policy" : "product_slug", route_type_aliases: [], materiality: "source_evidence", source_signal_roles: [], technical_route_shape: null, api_data_flow_signal: { present: false, basis: [] }, neutral_buckets: [], discovered_by: ["FIXTURE"], priority_route_found_by: "FIXTURE", priority_result: "PRIMARY_FOUND", admission_tier: "PRIMARY", variant_class: "NONE", variant_cluster_id: `fixture_${index}`, variant_rank: 1, extraction_decision: "EXTRACT", downstream_default: true, tier_reason: "fixture", legal_doc_candidate: item.canonical_url.endsWith("privacy-policy"), legal_doc_type: item.canonical_url.endsWith("privacy-policy") ? "privacy_policy" : "other", legal_doc_artifact_hint: item.canonical_url.endsWith("privacy-policy") ? "legal_doc_privacy_policy" : "legal_doc_other", phase_1_classification_effect: "SOURCE_ROUTING_ONLY_NOT_JOB_ROUTING" })) }; }
