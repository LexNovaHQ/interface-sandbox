...
| `target_data_provenance_profile` | Agent 4 | Agents 5–7 as authorized | M10 Phase B1 material artifact; after M7/M8 artifacts, before data forensics |
| `target_data_provenance_profile_forensics` | Agent 4 | Agents 5–7 as authorized | M10 Phase D forensic artifact; after saved data profile, before M11 |
...
| Agent 4 | read Agent 1 and Agent 3 required artifacts; run M10 only; save `target_data_provenance_profile` in M10 Phase B1; save `target_data_provenance_profile_forensics` in M10 Phase D; lock `M10_DATA_PROVENANCE` only after both save gates pass |
