...
if (expectedSourceIds.length && coverage.length < expectedSourceIds.length) warnings.push(`source_coverage rows (${coverage.length}) fewer than Stage 5 packet sources (${expectedSourceIds.length})`);
if (missingSourceCoverage.length) warnings.push(`source_coverage missing Stage 5 source IDs: ${missingSourceCoverage.join(", ")}`);
