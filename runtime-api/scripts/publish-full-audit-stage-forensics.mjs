...
function writeText(name, value) {
  ensureDir(outputRoot);
  fs.writeFileSync(filePath(name), value, "utf8");
  return name;
}
...
function pushLimited(arr, value, limit = 50) {
...
function collectSignals(root) {
  ...
  function visit(node, trail = "$") {
    ...
    if (Array.isArray(node)) {
      node.slice(0, 200).forEach((child, index) => visit(child, `${trail}[${index}]`));
      return;
    }
...
function summarizeTokens(tokenFields) {
  const summary = {
    ...
    raw_numeric_token_field_sum: 0,
    raw_numeric_token_field_count: 0,
    fields_found: tokenFields.length
  };
  ...
  if (summary.total_tokens === null && (summary.input_tokens !== null || summary.output_tokens !== null)) {
    summary.total_tokens = (summary.input_tokens || 0) + (summary.output_tokens || 0);
  }
  return summary;
}
...
function summarizeDurations(durationFields, artifactMetas) {
  ...
  return {
    duration_ms: explicitDurationMs !== null ? Math.round(explicitDurationMs) : artifactSpanMs,
    duration_source: explicitDurationMs !== null ? "explicit_runtime_fields" : "artifact_mtime_span_ms",
    artifact_span_ms: artifactSpanMs,
    explicit_duration_field_count: durationFields.length
  };
}
...
function summarizeValidationFromArtifacts(artifacts) {
  ...
  function scan(node, trail = "$") {
    ...
    for (const [key, value] of Object.entries(node)) {
      const lower = key.toLowerCase();
      if (lower === "validation" || lower.endsWith("validation")) validations.push({ path: `${trail}.${key}`, ok: value?.ok ?? null, reinvestigation_required: value?.reinvestigation_required === true });
  ...
  return {
    ...
    validation_ok_count: validations.filter((item) => item.ok === true).length,
    validation_reinvestigation_count: validations.filter((item) => item.reinvestigation_required === true).length,
    guardrail_count: guardrails.length,
    guardrail_ok_count: guardrails.filter((item) => item.ok === true).length,
    reinvestigation_request_count: reinvestigationRequests.length,
...
const stageSpecs = {
  ...
  stage6a: { label: "Stage 6A - Legal Cartography", artifacts: ["12-stage6a-legal-cartography.json", "12-stage6a-stage-result.json"] },
  stage6b: { label: "Stage 6B - Data Provenance", artifacts: ["13-stage6b-data-provenance-profile.json", "13-stage6b-stage-result.json"] },
  stage6integrated: { label: "Stage 6 Integrated Handoff", artifacts: ["14-stage6-integrated-handoff-validation.json"] }
};
...
for (const [stageId, spec] of Object.entries(stageSpecs)) {
  ...
  const modelSignals = signals.modelFields.map((item) => ({ path: item.path, value: item.value })).slice(0, 30);
  const attemptSignals = signals.attemptFields.map((item) => ({ path: item.path, value: item.value })).slice(0, 30);
  const forensic = {
    ok: metas.some((meta) => meta.present),
    artifact_type: "full_live_runtime_audit_stage_forensic_summary",
    generated_at: generatedAt,
    stage_id: stageId,
    label: ioRow.label || spec.label,
    status: ioRow.status || "UNKNOWN",
    input_summary_artifact: `${stageId}-input-summary.json`,
    output_summary_artifact: `${stageId}-output-summary.json`,
    forensic_summary_artifact: `${stageId}-forensic-summary.json`,
    source_artifacts: artifactNames,
    artifact_integrity: metas,
    duration,
    token_usage: tokenUsage,
    token_fields_found: signals.tokenFields.map((item) => ({ path: item.path, value: item.value })).slice(0, 40),
    model_fields_found: modelSignals,
    attempt_fields_found: attemptSignals,
    validation_and_guardrails: validation,
    notes: {
      duration_note: duration.duration_source === "artifact_mtime_span_ms" ? "No explicit runtime duration field was found in stage artifacts; duration_ms uses artifact mtime span as a forensic fallback." : "duration_ms was computed from explicit runtime duration fields found in artifacts.",
      token_note: tokenUsage.fields_found ? "Token usage was extracted from token/usage fields found in stage artifacts." : "No token/usage fields were found in the stage artifacts. The stage may be deterministic, or upstream runner did not persist token metadata."
    }
  };
...
const index = {
  ok: forensicRows.every((row) => row.status !== "FAIL"),
  artifact_type: "full_live_runtime_audit_stage_forensics_index",
  generated_at: generatedAt,
  stages: forensicRows
};
