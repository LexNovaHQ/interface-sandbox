...
function embeddedArtifactGuard(input, review) {
  const text = (input.source_bundle?.evidence_buffer || []).map((record) => record.clean_text_lossless || "").join("\n\n");
  const legalStack = Array.isArray(review?.legal_stack) ? review.legal_stack : [];
  const byType = Object.fromEntries(legalStack.map((doc) => [doc.document_type, doc]));
  return {
    has_dpa_evidence: textContains(text, ["data processing addendum", "data processing agreement", "annexure c"]),
    has_aup_evidence: textContains(text, ["acceptable use", "prohibited use", "usage restrictions"]),
    has_sla_evidence: textContains(text, ["service level agreement", "uptime", "availability commitment", "service credits", "annexure a"]),
    dpa_status: byType.DPA?.evidence_status || null,
    aup_status: byType.AUP?.evidence_status || null,
    sla_status: byType.SLA?.evidence_status || null,
    dpa_exists: byType.DPA?.exists ?? null,
    aup_exists: byType.AUP?.exists ?? null,
    sla_exists: byType.SLA?.exists ?? null
  };
}
...
const adapterResult = buildLegalStackReviewInput({
  ...
  runId: `stage6_legal_stack_review_input_${Date.now()}`,
  budget: {
...
const legalStage = await postJson(base, "/v1/diligence/stage", {
  stage: "legal_stack_review",
  input: stage6Input,
  options: { pool: process.env.STAGE6_POOL || "reasoning", maxOutputTokens: Number(process.env.STAGE6_MAX_OUTPUT_TOKENS || 8192), timeoutMs: Number(process.env.STAGE6_TIMEOUT_MS || 90000) }
});
...
console.log(JSON.stringify({
  ok: true,
  phase: "stage_6_legal_stack_review_e2e",
  adapter_version: stage6Input.legal_stack_review_input_version,
  budget_status: stage6Input.input_budget.budget_status,
  estimated_total_prompt_tokens: stage6Input.input_budget.estimated_total_prompt_tokens,
  actual_prompt_tokens: actualPromptTokens,
  token_estimate_drift_ratio: tokenDrift(actualPromptTokens, stage6Input.input_budget.estimated_total_prompt_tokens),
  included_sources: stage6Input.input_budget.included_sources.length,
  excluded_sources: stage6Input.input_budget.excluded_sources.length,
  legal_stack_count: review.legal_stack.length,
  legal_stack_statuses: Object.fromEntries(review.legal_stack.map((doc) => [doc.document_type, { exists: doc.exists, evidence_status: doc.evidence_status, document_url: doc.document_url }])),
  redline_count: review.document_stack_redline?.length || 0,
  assessment_count: review.legal_stack_assessment?.length || 0,
  limitation_count: review.limitations?.length || 0,
  embedded_artifact_guard: guard,
  validation_mode: legalStage.validation_mode,
  guardrail_validation_mode: legalStage.guardrail_validation_mode,
  runtime_instruction_configured: legalStage.prompt_metadata?.runtime_instruction_configured === true,
  model_metadata: legalStage.model_metadata || null
}, null, 2));
