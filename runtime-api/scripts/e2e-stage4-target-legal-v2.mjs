...
function extractSourceAudit(sourceBundle = {}) {
  ...
  const filtered = discoveryCandidates
    ...
    .map((item) => ({ id: item.candidate_id || item.artifact_id || item.source_id || null, url: item.source_url || item.url || null, normalized_url: normalizeUrlForCompare(item.source_url || item.url || ""), status: item.status || item.admission_status || null }))
...
function assertNoFilteredSourceLeak(stageName, output, filteredSources) {
  const json = stableString(output);
  const leaked = filteredSources.filter((source) => [source.id, source.url, source.normalized_url].filter(Boolean).filter((token) => token.length > 8).some((token) => json.includes(token)));
...
function assertCoverageGapAcknowledged(stageName, output, coverageGaps = []) {
  ...
  const json = stableString(output);
  const acknowledgesInsufficiency = /coverage_gap|coverage gap|not found|insufficient|missing|unavailable|not located|not discovered|updates|blog|releases/i.test(json);
  if (hasCoverageGap && !acknowledgesInsufficiency) fail(`${stageName} output does not acknowledge upstream coverage gap(s)`, { coverage_gaps: coverageGaps, preview: preview(output) });
...
console.log(JSON.stringify({ ok: true, step: "start", phase: "stage_4_target_feature_and_legal_stack_e2e", target: targetInput, runtime_url: base, capture_limit: Number(process.env.STAGE4_CAPTURE_LIMIT || 8) }, null, 2));

const discoveryResponse = await postJson(base, "/v1/source-discovery", { input: targetInput, options: { max_search_results_per_family: Number(process.env.STAGE4_MAX_SEARCH_RESULTS_PER_FAMILY || 4), probe_timeout_ms: Number(process.env.STAGE4_PROBE_TIMEOUT_MS || 8000) } });
...
const legalStackResponse = await postJson(base, "/v1/diligence/stage", {
  ...
  input: { source_bundle: sourceBundle, target_feature_profile: targetFeatureProfile, coverage_context: coverageContext },
  options: { poolAlias: "reasoning" }
});
...
const complianceAudit = runPromptComplianceAudit({ sourceBundle, coverageGaps, evidenceRefinerResponse, targetFeatureResponse, targetFeatureProfile, legalStackResponse, legalStackReview });
