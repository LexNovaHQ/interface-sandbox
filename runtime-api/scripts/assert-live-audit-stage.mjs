...
function assertStage1() {
  ...
  pass("Stage 1 - Source Discovery", "01-source-discovery.json", `sources=${sourceCount}`);
}
...
function assertStage2() {
  ...
  pass("Stage 2 - Source Capture", "02-source-capture.json", `records=${records.length}; lossless=${capturedText.length}`);
}
...
function assertStage3() {
  ...
  pass("Stage 3 - Evidence Refiner / Source Bundle", "03-evidence-refiner-source-bundle.json + 04-evidence-junction.json", `records=${records.length}; product_lossless=${losslessProductRecords.length}`);
}
...
function assertStage4() {
  ...
  pass("Stage 4 - Company / Target Profile", "05-target-profile.json", `target=${brandName}`);
}
