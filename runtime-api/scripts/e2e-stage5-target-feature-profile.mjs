...
async function buildStandaloneInputs(base, targetInput) {
  ...
  return { sourceBundle, junction, companyProfile: companyProfileStage.company_profile, cacheMode: "standalone_product_family_full_packet" };
}
...
writeJson(stage5CachePath, { cache_version: "stage5_target_feature_profile_e2e_cache_v1", generated_at: new Date().toISOString(), target_input: targetInput, source_bundle: sourceBundle, evidence_junction: adapterResult.evidence_junction || junction, company_profile: companyProfile, target_profile_v2: companyProfile, target_feature_profile_input: stage5Input, target_feature_candidate_index: stage5Input.target_feature_candidate_index, stage5_feature_discovery: featureStage.stage5_feature_discovery || null, target_feature_audit_ledger: targetFeatureAuditLedger, stage5_audit_ledger: targetFeatureAuditLedger, product_family_discovery_sources: adapterResult.product_family_discovery_sources || [], product_family_primary_sources: adapterResult.product_family_primary_sources || [], product_family_supporting_sources: adapterResult.product_family_supporting_sources || [], product_family_duplicate_sources: adapterResult.product_family_duplicate_sources || [], product_family_non_feature_context_sources: adapterResult.product_family_non_feature_context_sources || [], stage5_candidate_clusters: adapterResult.stage5_candidate_clusters || [], source_review: adapterResult.source_review || stage5Input.source_bundle?.source_review || null, target_feature_profile_stage_result: featureStage, target_feature_profile: profile, feature_profile_v2: profile });
...
console.log(JSON.stringify({ ok: true, phase: "stage_5_target_feature_profile_e2e", cache_mode: cacheMode, cache_path: stage5CachePath, source_bundle_version: sourceBundle.source_bundle_version, evidence_junction_version: junction.evidence_junction_version, adapter_version: stage5Input.target_feature_profile_input_version, target_feature_candidate_index_source: "adapter/shared_builder_post_discovery_checklist", audit_ledger_source: targetFeatureAuditLedger ? "runtime_guardrail_or_shared_builder" : "missing", stage5_feature_discovery_count: featureStage.stage5_feature_discovery?.discovered_features?.length || 0, budget_status: stage5Input.input_budget.budget_status, estimated_total_prompt_tokens: estimatedTotalPromptTokens, actual_prompt_tokens: actualPromptTokens, token_estimate_drift_ratio: tokenDrift(actualPromptTokens, estimatedTotalPromptTokens), included_sources: stage5Input.input_budget.included_sources.length, excluded_sources: stage5Input.input_budget.excluded_sources.length, product_family_discovery_source_count: adapterResult.product_family_discovery_sources?.length || 0, product_family_primary_source_count: adapterResult.product_family_primary_sources?.length || 0, product_family_supporting_source_count: adapterResult.product_family_supporting_sources?.length || 0, product_family_duplicate_source_count: adapterResult.product_family_duplicate_sources?.length || 0, product_family_non_feature_context_count: adapterResult.product_family_non_feature_context_sources?.length || 0, deterministic_cluster_count: adapterResult.stage5_candidate_clusters?.length || 0, deterministic_candidate_count: stage5Input.target_feature_candidate_index?.candidate_count || 0, feature_count: profile.feature_inventory.length, feature_titles: profile.feature_inventory.map((feature) => feature.feature_name || feature.feature_id).filter(Boolean), completeness_status: profile.commercial_scan?.completeness_status || null, validation_mode: featureStage.validation_mode, guardrail_validation_mode: featureStage.guardrail_validation_mode, model_metadata: featureStage.model_metadata || null }, null, 2));
