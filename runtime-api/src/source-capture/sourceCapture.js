...
function isBlockedIpv6(value) {
  ...
  if (text.startsWith("::ffff:")) {
    return isBlockedIpv4(text.slice(7));
  }
  ...
  return (
    ...
    text.startsWith("fe81") ||
    text.startsWith("fe82") ||
    text.startsWith("fe83") ||
    text.startsWith("fe84") ||
    text.startsWith("fe85") ||
    text.startsWith("fe86") ||
    text.startsWith("fe87") ||
    text.startsWith("fe88") ||
    text.startsWith("fe89") ||
    text.startsWith("fe8a") ||
    text.startsWith("fe8b") ||
    text.startsWith("fe8c") ||
    text.startsWith("fe8d") ||
    text.startsWith("fe8e") ||
    text.startsWith("fe8f") ||
...
export async function validateCaptureUrl(value) {
  ...
  if (!normalized) {
    throw new BlockedCaptureUrlError("invalid_url", String(value || ""));
  }

  const parsed = new URL(normalized);

  if (!["http:", "https:"].includes(parsed.protocol)) {
    throw new BlockedCaptureUrlError("blocked_protocol", normalized);
  }

  const hostname = parsed.hostname.toLowerCase().replace(/\.$/, "");

  if (isBlockedHostname(hostname)) {
    throw new BlockedCaptureUrlError("blocked_hostname", normalized);
  }

  if (net.isIP(hostname)) {
    if (isBlockedIpAddress(hostname)) {
      throw new BlockedCaptureUrlError("blocked_ip", normalized);
    }
    return normalized;
  }
  ...
  if (!Array.isArray(resolved) || resolved.length === 0) {
    throw new BlockedCaptureUrlError("dns_lookup_empty", normalized);
  }

  const blockedAddress = resolved.find((entry) => isBlockedIpAddress(entry.address));
  if (blockedAddress) {
    throw new BlockedCaptureUrlError("blocked_resolved_ip", normalized);
  }
