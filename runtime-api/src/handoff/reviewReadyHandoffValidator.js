...
const REQUIRED_FUNCTIONAL_SECTIONS = Object.freeze([
...
const RETIRED_STAGE10_KEYS = Object.freeze([
  "legal_stack_review",
...
function containsRetiredKey(value, path = "$", hits = []) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => containsRetiredKey(item, `${path}[${index}]`, hits));
    return hits;
  }
...
function validatePrefill(prefill, errors) {
  ...
  VAULT_GROUPS.forEach((group) => {
    Object.entries(prefill[group] || {}).forEach(([localPath, suggestion]) => {
      ...
      if (!["high", "medium", "low"].includes(String(suggestion.confidence))) errors.push(`Vault suggestion at ${fieldPath} has invalid confidence: ${suggestion.confidence}`);
...
function validateQuestions(questions, errors) {
  ...
  questions.forEach((question, index) => {
    ...
    ["section_key", "question_key", "question", "answer_type", "vault_match", "vault_paths", "assembly_paths", "priority", "required_for", "prefill_status"].forEach((key) => {
      if (!(key in question)) errors.push(`vault_confirmation_questions[${index}] missing ${key}.`);
    });
    if (!Array.isArray(question.vault_paths)) errors.push(`vault_confirmation_questions[${index}].vault_paths must be an array.`);
    asArray(question.vault_paths).forEach((path) => {
      if (!isAllowedVaultPath(path)) errors.push(`vault_confirmation_questions[${index}] has invalid Vault field path: ${path}`);
    });
    if (!Array.isArray(question.assembly_paths)) errors.push(`vault_confirmation_questions[${index}].assembly_paths must be an array.`);
    if (!["perfect", "partial", "none"].includes(String(question.vault_match))) errors.push(`vault_confirmation_questions[${index}] has invalid vault_match.`);
    if (!["critical", "high", "medium", "low"].includes(String(question.priority))) errors.push(`vault_confirmation_questions[${index}] has invalid priority.`);
  });
}

function validateFunctionalSections(functionalSections, questions, errors) {
  if (!isObject(functionalSections)) {
    errors.push("functional_sections must be an object.");
    return;
  }
  REQUIRED_FUNCTIONAL_SECTIONS.forEach((sectionKey) => {
    const section = functionalSections[sectionKey];
    if (!isObject(section)) {
      errors.push(`functional_sections missing section: ${sectionKey}`);
      return;
    }
    ["section_key", "section_label", "section_role", "completion_status", "questions"].forEach((field) => {
      if (!(field in section)) errors.push(`functional_sections.${sectionKey} missing ${field}.`);
    });
    if (section.section_key !== sectionKey) errors.push(`functional_sections.${sectionKey}.section_key mismatch.`);
...
function validateFunctionalIntakeVault(vault, errors) {
  if (!isObject(vault)) {
    errors.push("functional_intake_vault must be an object.");
    return;
  }
  ...
  if (!isObject(vault.vault_payload)) errors.push("functional_intake_vault.vault_payload must be an object.");
  if (!isObject(vault.assembly_handoff_intake)) errors.push("functional_intake_vault.assembly_handoff_intake must be an object.");
  if (!isObject(vault.source_trace)) errors.push("functional_intake_vault.source_trace must be an object.");
...
function validateAssemblyHandoff(assemblyHandoff, errors) {
  ...
  if (!Array.isArray(assemblyHandoff.legal_document_status)) errors.push("assembly_handoff.legal_document_status must be an array.");
