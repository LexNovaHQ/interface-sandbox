...
const COMMERCIAL = new Set(["distinct_commercial_outcomes_seen", "mapped_core_feature_ids", "unmapped_outcomes_due_to_insufficient_detail"]);
...
function strip(o, allowed, path, profile, notes) {
  if (!obj(o)) return;
  for (const k of Object.keys(o)) if (!allowed.has(k)) { const old = o[k]; delete o[k]; warn(profile, notes, `${path}/${k}`, "stripped_additional_property", old, null); }
}
...
const sourceOf = (f) => first(f.feature_source_url, (f.data_provenance || []).map((x) => x?.source_url), (f.archetype_provenance || []).map((x) => x?.source_url), (f.surface_provenance || []).map((x) => x?.source_url));
const quoteOf = (f) => first(f.evidence_quote, (f.data_provenance || []).map((x) => x?.evidence_quote), (f.archetype_provenance || []).map((x) => x?.evidence_quote), (f.surface_provenance || []).map((x) => x?.evidence_quote));

function fixData(e, f, profile, notes, path) {
  const old = obj(e) ? { ...e } : e; const d = obj(e) ? { ...e } : {}; strip(d, DATA, path, profile, notes);
  d.data_origin = dataOrigin(d.data_origin); d.data_subject = dataSubject(d.data_subject); d.data_category = dataCategory(d.data_category || f.input_data || f.feature_name);
  d.processing_context = str(d.processing_context, f.system_action || f.commercial_function); d.storage_or_retention_signal = str(d.storage_or_retention_signal); d.training_or_finetuning_signal = str(d.training_or_finetuning_signal);
  d.source_url = str(d.source_url || f.feature_source_url || sourceOf(f), ""); d.evidence_quote = str(d.evidence_quote || f.evidence_quote || quoteOf(f)); d.confidence = conf(d.confidence);
...
function fixArchetypeProvenance(e, f, profile, notes, path) {
  const old = obj(e) ? { ...e } : e; const p = obj(e) ? { ...e } : {}; strip(p, ARCH, path, profile, notes);
  p.archetype_code = fromSet(String(p.archetype_code || "").toUpperCase(), ARCHETYPES, ""); if (!p.archetype_code) return null;
  p.registry_key_detection_logic = str(p.registry_key_detection_logic, "not visible in admitted evidence"); p.matched_feature_behavior = str(p.matched_feature_behavior, f.system_action || f.commercial_function);
  p.evidence_quote = str(p.evidence_quote || f.evidence_quote || quoteOf(f)); p.source_url = str(p.source_url || f.feature_source_url || sourceOf(f), ""); p.confidence = conf(p.confidence);
  warn(profile, notes, path, "canonicalized_archetype_provenance", old, p); return p;
}
function fixSurfaceProvenance(e, f, profile, notes, path) {
  const old = obj(e) ? { ...e } : e; const p = obj(e) ? { ...e } : {}; strip(p, SURF, path, profile, notes);
  p.surface_token = surface(p.surface_token) || ""; if (!p.surface_token) return null;
  p.registry_key_surface_meaning = str(p.registry_key_surface_meaning, "not visible in admitted evidence"); p.matched_data_or_context = str(p.matched_data_or_context, f.input_data?.join(", ") || f.commercial_function);
  p.evidence_quote = str(p.evidence_quote || f.evidence_quote || quoteOf(f)); p.source_url = str(p.source_url || f.feature_source_url || sourceOf(f), ""); p.confidence = conf(p.confidence);
  warn(profile, notes, path, "canonicalized_surface_provenance", old, p); return p;
}
function fixFeature(f, i, profile, notes) {
  const old = obj(f) ? { ...f } : f; const x = obj(f) ? { ...f } : {}; strip(x, FEATURE, `/feature_inventory/${i}`, profile, notes);
  x.feature_id = str(x.feature_id, `F${String(i + 1).padStart(3, "0")}`); x.feature_name = str(x.feature_name || x.business_label_or_product_area || x.commercial_function, `Runtime feature ${i + 1}`);
  x.feature_role = norm(x.feature_role).includes("secondary") ? "SECONDARY" : "CORE"; x.commercial_function = str(x.commercial_function, x.feature_name); x.business_label_or_product_area = str(x.business_label_or_product_area, x.feature_name); x.feature_description = str(x.feature_description, x.commercial_function); x.actor_or_user = str(x.actor_or_user, "unknown"); x.input_data = arr(x.input_data); x.system_action = str(x.system_action, x.commercial_function); x.output_or_result = str(x.output_or_result);
  x.autonomy_level = fromSet(x.autonomy_level, new Set(["none", "draft", "recommend", "execute", "unknown"])); x.human_review_signal = fromSet(x.human_review_signal, new Set(["required", "optional", "not_visible", "unknown"])); x.external_action_signal = tri(x.external_action_signal);
  x.delivery_channels = obj(x.delivery_channels) ? x.delivery_channels : {}; x.delivery_channels = { app: tri(x.delivery_channels.app), api: tri(x.delivery_channels.api), web: tri(x.delivery_channels.web) };
  x.evidence_quote = str(x.evidence_quote || quoteOf(x)); x.feature_source_url = str(x.feature_source_url || sourceOf(x), ""); x.confidence = conf(x.confidence); x.evidence_refs = arr(x.evidence_refs); x.linked_threat_ids = [];
  ...
  warn(profile, notes, `/feature_inventory/${i}`, "canonicalized_feature", old, x); return x;
}
function fixFieldRef(e, i, profile, notes) {
  const old = obj(e) ? { ...e } : e; const r = obj(e) ? { ...e } : {}; strip(r, FIELD_REF, `/evidence/field_evidence_refs/${i}`, profile, notes);
  const refs = arr(r.evidence_refs); for (const c of [e?.evidence_source_id, e?.claim_supported, e?.source_url, e?.url]) if (typeof c === "string" && c.trim() && !refs.includes(c.trim())) refs.push(c.trim());
  r.field_path = str(r.field_path, `unknown_${i + 1}`); r.evidence_refs = refs; r.basis = str(r.basis || e?.claim_supported || e?.evidence_quote, "weak/fallback evidence reference preserved as warning"); r.confidence = conf(r.confidence);
  warn(profile, notes, `/evidence/field_evidence_refs/${i}`, "canonicalized_field_evidence_ref", old, r); return r;
}
...
export function repairTargetFeatureProfileForSchema(profile) {
  ...
  const oldVersion = profile.feature_profile_version; profile.feature_profile_version = "feature_profile_v2"; warn(profile, notes, "/feature_profile_version", "repaired_feature_profile_version", oldVersion, profile.feature_profile_version);
  const ref = obj(profile.target_profile_ref) ? profile.target_profile_ref : {}; strip(ref, new Set(["target_profile_version", "brand_name", "legal_name", "domain"]), "/target_profile_ref", profile, notes);
  ...
  const ids = new Set(); profile.feature_inventory.forEach((f, i) => { const old = f.feature_id; if (ids.has(f.feature_id)) f.feature_id = `${f.feature_id}_${i + 1}`; ids.add(f.feature_id); warn(profile, notes, `/feature_inventory/${i}/feature_id`, "deduplicated_feature_id", old, f.feature_id); });
  const aliasCount = Array.isArray(profile.product_feature_map) ? profile.product_feature_map.length : 0; profile.product_feature_map = []; if (aliasCount) warn(profile, notes, "/product_feature_map", "cleared_legacy_product_feature_map", aliasCount, 0);
  profile.data_provenance_map = []; profile.feature_inventory.forEach((f, i) => f.data_provenance.forEach((d, j) => profile.data_provenance_map.push({ provenance_id: `DP${String(i + 1).padStart(3, "0")}${String(j + 1).padStart(2, "0")}`, feature_id: f.feature_id, ...d })));
  profile.regulated_surface_map = []; profile.feature_inventory.forEach((f, i) => f.surface_provenance.forEach((s, j) => profile.regulated_surface_map.push({ surface_id: `RS${String(i + 1).padStart(3, "0")}${String(j + 1).padStart(2, "0")}`, feature_id: f.feature_id, surface_token: s.surface_token, int_ext_classification: "unknown", basis: s.matched_data_or_context || s.registry_key_surface_meaning, confidence: conf(s.confidence), evidence_refs: arr(f.evidence_refs.length ? f.evidence_refs : f.feature_source_url) })));
  ...
  profile.commercial_scan = obj(profile.commercial_scan) ? profile.commercial_scan : {}; strip(profile.commercial_scan, COMMERCIAL, "/commercial_scan", profile, notes);
  profile.commercial_scan.distinct_commercial_outcomes_seen = arr(profile.commercial_scan.distinct_commercial_outcomes_seen); profile.commercial_scan.mapped_core_feature_ids = arr(profile.commercial_scan.mapped_core_feature_ids).filter((id) => ids.has(id)); profile.commercial_scan.unmapped_outcomes_due_to_insufficient_detail = arr(profile.commercial_scan.unmapped_outcomes_due_to_insufficient_detail);
  profile.vault_feature_candidates = obj(profile.vault_feature_candidates) ? profile.vault_feature_candidates : {}; strip(profile.vault_feature_candidates, VAULT, "/vault_feature_candidates", profile, notes); profile.vault_feature_candidates.baseline = obj(profile.vault_feature_candidates.baseline) ? profile.vault_feature_candidates.baseline : {}; profile.vault_feature_candidates.archetypes = obj(profile.vault_feature_candidates.archetypes) ? profile.vault_feature_candidates.archetypes : {}; profile.vault_feature_candidates.compliance = obj(profile.vault_feature_candidates.compliance) ? profile.vault_feature_candidates.compliance : {};
  profile.evidence = obj(profile.evidence) ? profile.evidence : {}; strip(profile.evidence, EVIDENCE, "/evidence", profile, notes); profile.evidence.field_evidence_refs = (Array.isArray(profile.evidence.field_evidence_refs) ? profile.evidence.field_evidence_refs : []).map((e, i) => fixFieldRef(e, i, profile, notes)); profile.evidence.unresolved_questions = arr(profile.evidence.unresolved_questions);
