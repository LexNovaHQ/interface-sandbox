...
export function buildTargetFeatureProfileInput(args = {}) {
  ...
  if (result?.ok && input && typeof input === "object" && !Array.isArray(input)) {
    ...
    input.stage5_product_family_manifest_v2 = buildStage5ProductFamilyManifestV2(input, {
      max_product_family_packets: Number(args?.budget?.max_product_family_packets || process.env.STAGE5_MAX_PRODUCT_FAMILY_PACKETS || 8)
    });
    input.adapter_policy = {
      ...
      product_family_manifest_v2_required: true,
      product_family_scoped_lossless_classification_available: true,
      deterministic_field_prefill_required: true,
      random_snippet_selection_forbidden: true
    };
    if (input.source_bundle?.source_review && typeof input.source_bundle.source_review === "object") {
      ...
      input.source_bundle.source_review.product_family_manifest_version = input.stage5_product_family_manifest_v2.product_family_manifest_version;
      input.source_bundle.source_review.product_family_count = input.stage5_product_family_manifest_v2.product_family_count;
    }
  }
