...
function evidenceFromRecord(record = {}) {
  ...
  return {
    ...
    evidence_policy: {
      ...
      summarized: false,
      compressed: false,
      truncated_by_stage_5_adapter: false
    }
  };
}

function extractPacket(evidenceJunction = {}) {
  return evidenceJunction?.downstream_packets?.[PRODUCT_STAGE_KEY] || null;
}

export function buildTargetFeatureProfileInput({
  sourceBundle = {},
  evidenceJunction = {},
  companyProfile = null,
  runId = null,
  ...
  for (const record of sortedRecords) {
    ...
    if (!text) {
      excluded.push({ ...exclusionBase, reason: "empty_clean_text" });
      continue;
    }

    if (textLength > maxSingleSourceChars) {
      excluded.push({ ...exclusionBase, reason: "single_source_exceeds_max_single_source_chars" });
      continue;
    }

    if (usedChars + textLength > effectiveCharLimit) {
      excluded.push({ ...exclusionBase, reason: "input_budget_exceeded_by_source_selection" });
      continue;
    }
  ...
  if (excluded.length) {
    limitations.push("Some admitted target-feature sources were excluded from the model input by deterministic source-level budget selection. They remain preserved in the Stage 3 evidence archive.");
  }
  if (hardFailure) {
    limitations.push("No non-empty target-feature source could fit inside the configured Stage 5 input budget without truncation.");
  }
  ...
  const adapterOutput = {
    ...
    adapter_policy: {
      ...
      company_profile_context_only: true,
  ...
  if (hardFailure) {
    return {
      ...
      error: "No target-feature source could fit inside the configured Stage 5 input budget without truncation.",
      input_budget: adapterOutput.input_budget,
      target_feature_profile_input: adapterOutput
    };
  }
