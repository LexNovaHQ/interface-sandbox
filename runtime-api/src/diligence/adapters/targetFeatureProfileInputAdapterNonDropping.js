...
function deterministicCandidateIndex(records = []) {
  ...
  for (const record of records) {
    ...
    if (sourceFamily === "product_profile" || /product|feature|api|developer|docs|model|platform|solution|integration/.test(`${url} ${title}`)) {
      add({ record, candidateKey: "source_product_surface", label: sourceLabel || "Product/source surface", candidateType: "source_surface", reason: "admitted Stage 5 product-family source is available for post-discovery accounting", confidence: sourceFamily === "product_profile" ? "high" : "medium" });
    }
