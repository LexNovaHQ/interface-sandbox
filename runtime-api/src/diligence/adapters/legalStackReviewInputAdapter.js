...
function sourceSpecificity(record = {}) {
  ...
  if (/data processing addendum|data processing agreement|acceptable use|service level agreement|sub-?processors?|annexure|schedule|addendum|prohibited use|support services/.test(text)) score += 25;
  score += Math.min(cleanText(record).length, 30000) / 3000;
  return score;
}
...
function sortSources(records = []) {
  return [...records].sort((a, b) => {
    ...
    return sourceSpecificity(b) - sourceSpecificity(a);
  });
}
...
function evidenceFromRecord(record = {}) {
  ...
  return {
    ...
    evidence_policy: {
      ...
      summarized: false,
      compressed: false,
      truncated_by_stage_6_adapter: false
    }
  };
}

function extractPacket(evidenceJunction = {}) {
  return evidenceJunction?.downstream_packets?.[LEGAL_STAGE_KEY] || null;
}
...
export function buildLegalStackReviewInput({
  sourceBundle = {},
  evidenceJunction = {},
  ...
  const limitations = [];
  if (excluded.length) limitations.push("Some admitted legal/governance sources were excluded from the model input by deterministic source-level budget selection. They remain preserved in the Stage 3 evidence archive.");
  ...
  const adapterOutput = {
    ...
    source_bundle: {
      ...
      source_review: {
        ...
        embedded_artifact_instruction: "Inspect admitted legal/governance text for embedded DPA, AUP, SLA, annexures, schedules, addenda, exhibits, appendices, prohibited-use sections, support terms, subprocessors, trust/security/compliance sections before marking absent."
      },
  ...
  if (hardFailure) {
    return { ok: false, status: 413, error_type: "TOKEN_BUDGET_EXCEEDED", error: "No legal/governance source could fit inside the configured Stage 6 input budget without truncation.", input_budget: adapterOutput.input_budget, legal_stack_review_input: adapterOutput };
  }
