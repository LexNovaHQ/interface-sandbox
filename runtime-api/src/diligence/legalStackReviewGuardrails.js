...
const LEGAL_ADVICE_PHRASES = [
  /is compliant\b/i,
  /is not compliant\b/i,
  /violates\b/i,
  /illegal\b/i,
  /enforceable\b/i,
  /unenforceable\b/i,
  /certif(?:y|ies|ied) compliance/i
];

function push(errors, instancePath, message, params = {}) {
  errors.push({
    keyword: "legal_stack_review_guardrail",
    instancePath,
    schemaPath: "#/legalStackReviewGuardrails",
    message,
    params
  });
}
...
function evidenceUrls(evidenceBuffer = []) {
  const urls = new Set(["N/A", "manual_text"]);
  for (const record of Array.isArray(evidenceBuffer) ? evidenceBuffer : []) {
    for (const value of [record?.source_url, record?.final_url, record?.url]) {
...
export function validateLegalStackReviewGuardrails(review, { evidenceBuffer = [], threatMappingSupplied = false } = {}) {
  ...
  legalStack.forEach((doc, index) => {
    ...
    if (!doc || typeof doc !== "object" || Array.isArray(doc)) {
      push(errors, base, "legal_stack item must be an object");
      return;
    }
    ...
    if (doc.exists === false) {
      if (doc.document_url !== "N/A") push(errors, `${base}/document_url`, "absent/access-failed document_url must be N/A");
      if (doc.covers !== null) push(errors, `${base}/covers`, "absent/access-failed covers must be null");
    }
  ...
  redlines.forEach((item, index) => {
    ...
    if (!item || typeof item !== "object" || Array.isArray(item)) {
      push(errors, base, "document_stack_redline item must be an object");
      return;
    }
    if (!REDLINE_TYPES.has(item.type)) push(errors, `${base}/type`, `invalid redline type: ${item.type}`, { type: item.type });
    for (const field of ["mismatch_id", "quote", "source", "feature_ref", "claim_type", "contradicts"]) {
      if (!nonEmptyString(item[field])) push(errors, `${base}/${field}`, `${field} must be non-empty`);
    }
    if (nonEmptyString(item.source) && !admittedUrls.has(item.source) && !admittedUrls.has(normalizeUrl(item.source)) && item.source !== "N/A") {
      push(errors, `${base}/source`, "redline source must match admitted evidence_buffer source URL or N/A", { source: item.source });
    }
  });
