...
function stringifyLimitation(value) { if (typeof value === "string") return value; if (value === null || value === undefined) return ""; if (Array.isArray(value)) return value.map(stringifyLimitation).filter(Boolean).join("; "); if (typeof value === "object") return asString(value.message || value.reason || value.summary || value.basis || value.value, JSON.stringify(value)); return String(value); }
