...
function familyOf(record) {
  return SOURCE_FAMILIES.includes(record?.source_family) ? record.source_family : "unknown";
}

function idOf(record, index) {
  return record?.evidence_source_id || `SRC_${String(index + 1).padStart(3, "0")}`;
}
...
function dedupeGroups(registry) {
  ...
  for (const source of registry) {
    for (const topic of source.product_topics || []) {
  ...
  for (const [topic, sources] of byTopic.entries()) {
    ...
    groups.push({
      ...
      policy: {
        ...
        repeated_evidence_mapped_not_removed: true,
        no_source_text_rewritten: true,
        no_summary_used_as_evidence: true
      }
    });
  }
  return groups;
...
function routes(registry, groups) {
  ...
  for (const group of groups) {
    groupBySource.set(group.primary_source_id, group);
    for (const sourceId of group.supporting_source_ids || []) groupBySource.set(sourceId, group);
  }
...
function packets(sourceRecords, routed, groups) {
  ...
  for (const stageKey of STAGE_KEYS) {
    ...
    out[stageKey] = {
      ...
      routed_evidence: routed.filter((route) => route.downstream_tags.includes(stageKey)),
...
export function buildEvidenceJunction({ sourceBundle = {}, runId = null, generatedAt = nowIso() } = {}) {
  const sourceRecords = Array.isArray(sourceBundle?.raw_footprint?.source_records)
    ? sourceBundle.raw_footprint.source_records.map(clone)
    : [];
  ...
  return {
    ...
    processing_manifest: {
      ...
      repeated_product_evidence_deduped_by_route_not_source_removal: true,
      downstream_stage_keys: STAGE_KEYS
    }
  };
}
