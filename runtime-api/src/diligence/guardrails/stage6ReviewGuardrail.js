...
export function validateStage6ReviewGuardrail(stage6Review = {}, { input = {}, stageId = null, semanticModelAttempted = null } = {}) {
  ...
  if (component === "stage6a_legal_document_cartography" || component === "stage6_integrated_handoff") {
    ...
    if (hasLegalSources(input) && inventory.length === 0) critical.push(err("STAGE6_EMPTY_6A_WITH_LEGAL_SOURCES", "/legal_document_cartography/legal_document_inventory", "Legal sources exist but 6A inventory is empty."));
    ...
    if (inventory.length && controls.length === 0) critical.push(err("STAGE6_MISSING_REQUIRED_INDEX", "/legal_document_cartography/document_control_signal_map", "Legal docs exist but control signal map is empty."));
    if (stage5FeatureCount(input) && inventory.length && arr(nav.feature_to_legal_unit_index).length === 0) critical.push(err("STAGE6_MISSING_REQUIRED_INDEX", "/stage7_navigation_index/feature_to_legal_unit_index", "Stage 5 features exist but feature/legal-unit index is empty."));
    inventory.forEach((r, i) => { if (!STAGE6_DOCUMENT_TYPES.includes(r?.document_type)) critical.push(err("STAGE6_SCHEMA_INVALID", `/legal_document_cartography/legal_document_inventory/${i}/document_type`, "Invalid document_type.")); });
    units.forEach((r, i) => { if (!STAGE6_LEGAL_UNIT_TYPES.includes(r?.legal_unit_type) || RETIRED_VALUES.has(r?.legal_unit_type)) critical.push(err("STAGE6_MICRO_LEGAL_UNIT_PRESENT", `/legal_document_cartography/legal_document_index/${i}/legal_unit_type`, "Macro legal units only.")); if (!documentIds.has(r?.document_id)) critical.push(err("STAGE6_UNKNOWN_DOCUMENT_REF", `/legal_document_cartography/legal_document_index/${i}/document_id`, "Unknown document_id ref.")); checkEnum(r?.control_families_detected, STAGE6_CONTROL_FAMILIES, `/legal_document_cartography/legal_document_index/${i}/control_families_detected`, critical); checkEnum(r?.basis_codes, STAGE6_BASIS_CODES, `/legal_document_cartography/legal_document_index/${i}/basis_codes`, critical); });
    controls.forEach((r, i) => { if (!documentIds.has(r?.document_id)) critical.push(err("STAGE6_UNKNOWN_DOCUMENT_REF", `/legal_document_cartography/document_control_signal_map/${i}/document_id`, "Unknown document_id ref.")); if (r?.legal_unit_id && !legalUnitIds.has(r.legal_unit_id)) critical.push(err("STAGE6_UNKNOWN_LEGAL_UNIT_REF", `/legal_document_cartography/document_control_signal_map/${i}/legal_unit_id`, "Unknown legal_unit_id ref.")); if (!STAGE6_CONTROL_FAMILIES.includes(r?.control_family)) critical.push(err("STAGE6_SCHEMA_INVALID", `/legal_document_cartography/document_control_signal_map/${i}/control_family`, "Invalid control_family.")); if (!STAGE6_CONTROL_SIGNALS.includes(r?.control_signal)) critical.push(err("STAGE6_SCHEMA_INVALID", `/legal_document_cartography/document_control_signal_map/${i}/control_signal`, "Invalid control_signal.")); checkEnum(r?.basis_codes, STAGE6_BASIS_CODES, `/legal_document_cartography/document_control_signal_map/${i}/basis_codes`, critical); });
...
export const stage6ReviewGuardrailInternals = { RETIRED_KEYS, RETIRED_VALUES, hasLegalSources, stage5ProvenanceCount, stage5FeatureCount };
