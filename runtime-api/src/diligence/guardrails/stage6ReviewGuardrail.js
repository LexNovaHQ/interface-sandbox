...
const RETIRED_KEYS = new Set([
  "legal_stack_review_version", "legal_stack", "document_stack_redline", "document_stack_synthesis", "legal_stack_assessment", "doc_id", "doc_type", "doc_family", "doc_title", "section_id", "section_path", "heading_text", "heading_level", "structural_zone", "document_section", "control_topics_detected", "feature_to_document_section_index", "document_source_locator_index", "flow_id", "profile_summary_signals"
]);
const RETIRED_VALUES = new Set([
  "source_text_classification", "stage6_section_ref", "stage6_legal_section_ref", "document_relationship_signal", "feature_control_alignment", "model_overlay", "heading_classification", "feature_without_control_section", "doc_without_required_section", "conflicting_document_signal", "missing_core_document", "missing_supplemental_document", "control_signal_unclear", "footer_notice", "banner_notice", "modal_notice", "faq", "micro_section", "table_of_contents_row", "page_header", "page_footer"
]);
...
function admissionBySourceRef(input = {}) { const map = new Map(); sourceRecords(input).forEach((record, index) => map.set(stage6ASourceRecordRef(record, index), stage6AAdmissionDecision(record))); return map; }
...
function pushQualityIssues(qualityIssues = [], warnings, repairs, critical) { for (const issue of arr(qualityIssues)) { if (issue?.severity === "critical") critical.push(err(issue.code, issue.path || "/", issue.message || issue.code, issue.params || {})); else if (issue?.severity === "repairable") repairs.push(rep(issue.code, issue.path || "/", issue.message || issue.code, issue.params || {})); else warnings.push(warn(issue.code, issue.path || "/", issue.message || issue.code, issue.params || {})); } }
...
export function validateStage6ReviewGuardrail(stage6Review = {}, { input = {}, stageId = null, semanticModelAttempted = null } = {}) {
  ...
  if (component === "stage6a_legal_document_cartography" || component === "stage6_integrated_handoff") {
    ...
    inventory.forEach((r, i) => { if (!STAGE6_DOCUMENT_TYPES.includes(r?.document_type)) warnings.push(warn("STAGE6_ENUM_DRIFT_WARNING", `/legal_document_cartography/legal_document_inventory/${i}/document_type`, "Invalid document_type passed as warning.", { value: r?.document_type })); const decision = admissionMap.get(r?.source_record_ref); if (decision && decision.admitted !== true) critical.push(err("STAGE6A_NON_LEGAL_SOURCE_ADMITTED", `/legal_document_cartography/legal_document_inventory/${i}/source_record_ref`, "6A inventory contains a source that fails strict legal/governance admission.", { source_record_ref: r?.source_record_ref, source_family: decision.source_family, admission_reason: decision.admission_reason, locator_document_type: decision.locator_document_type })); });
