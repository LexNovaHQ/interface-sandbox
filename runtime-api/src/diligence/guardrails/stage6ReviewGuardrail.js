...
function textOf(record = {}) { return [record.clean_text_lossless, record.clean_text, record.text, record.body, record.extracted_text].filter(Boolean).join(" ").trim(); }
function looksLegal(record = {}) { return /legal|privacy|terms|policy|dpa|aup|sla|security|trust|subprocessor|status|eula/i.test([record?.source_family, record?.source_type, record?.title, record?.url, record?.final_url].filter(Boolean).join(" ")); }
function hasLegalSources(input = {}) { return arr(input?.source_bundle?.raw_footprint?.source_records).concat(arr(input?.source_bundle?.evidence_buffer)).some(looksLegal); }
function hasUsableLegalSources(input = {}) { return arr(input?.source_bundle?.raw_footprint?.source_records).concat(arr(input?.source_bundle?.evidence_buffer)).some((r) => looksLegal(r) && textOf(r).length >= 80); }
...
export function validateStage6ReviewGuardrail(stage6Review = {}, { input = {}, stageId = null, semanticModelAttempted = null } = {}) {
  ...
  if (component === "stage6a_legal_document_cartography" || component === "stage6_integrated_handoff") {
    ...
    if (hasUsableLegalSources(input) && inventory.length === 0) critical.push(err("STAGE6_EMPTY_6A_WITH_USABLE_LEGAL_TEXT", "/legal_document_cartography/legal_document_inventory", "Usable admitted legal source text exists but 6A inventory is empty."));
    else if (hasLegalSources(input) && inventory.length === 0) warnings.push(warn("STAGE6_THIN_LEGAL_SOURCE_WARNING", "/legal_document_cartography/legal_document_inventory", "Legal-looking source URL/title exists but no usable legal text was mapped; passed as warning."));
    ...
    if (inventory.length && controls.length === 0) warnings.push(warn("STAGE6_CONTROL_MAP_EMPTY_WARNING", "/legal_document_cartography/document_control_signal_map", "Legal docs exist but no control signal map was emitted; treat controls as not evidenced."));
    if (stage5FeatureCount(input) && inventory.length && arr(nav.feature_to_legal_unit_index).length === 0) warnings.push(warn("STAGE6_FEATURE_LEGAL_INDEX_EMPTY_WARNING", "/stage7_navigation_index/feature_to_legal_unit_index", "Stage 5 features exist but feature/legal-unit index is empty; passed as warning."));
    inventory.forEach((r, i) => { if (!STAGE6_DOCUMENT_TYPES.includes(r?.document_type)) warnings.push(warn("STAGE6_ENUM_DRIFT_WARNING", `/legal_document_cartography/legal_document_inventory/${i}/document_type`, "Invalid document_type passed as warning.", { value: r?.document_type })); });
