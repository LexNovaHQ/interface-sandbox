...
export async function buildProfiles({ targetInput, sourceBundle, evidenceJunction, mode, logs, runId, runStage }) {
  ...
  const targetFeatureProfile = stage5Result.target_feature_profile;
  logStage(logs, "target_feature_profile", "complete", {
    execution_mode: "stage5_lossless_windowed_runtime",
    feature_count: targetFeatureProfile?.feature_inventory?.length || 0,
    classification_status: targetFeatureProfile?.classification_quality?.status || null,
    stage5_version: stage5Result.stage5_version
  });
