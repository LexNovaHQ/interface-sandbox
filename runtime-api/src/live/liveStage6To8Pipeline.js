...
import { buildStage7ExposureProfile } from "../diligence/stage7ExposureProfileBuilder.js";
...
function throwStage6BError(result = {}) {
  const error = new Error(result.error || result.error_type || "Stage 6B Data Provenance failed.");
...
export async function runStage6Live({ sourceBundle, evidenceJunction, companyProfile, targetFeatureProfile, logs, runId }) {
  ...
  const stage6aStageResult = await runStage("stage6a_legal_document_cartography", stage6Input, {
  ...
  logStage(logs, "stage6a_legal_document_cartography", "complete", {
    ...
    legal_unit_count: legalCartography?.legal_document_index?.length || 0,
  ...
  logStage(logs, "stage6b_data_provenance", "complete", {
    data_flow_profile_count: stage6bStageResult.stage6_review?.data_provenance_profile?.data_flow_profile?.length || 0,
    data_provenance_profile_version: dataProvenanceProfile?.data_provenance_profile_version || null,
    semantic_model_attempted: stage6bStageResult.semantic_model_attempted === true
  });
...
export function buildStage6Cache({
  ...
  return {
    ...
    company_profile: companyProfile,
    target_feature_profile: targetFeatureProfile,
    legal_cartography: legalCartography || stage6aStageResult?.stage6_review?.legal_document_cartography || null,
    data_provenance_profile: dataProvenanceProfile || stage6bStageResult?.data_provenance_profile || null,
...
function fallbackReinvestigationEntry(row, prior, reason) {
  return {
    ...
    entry_number: prior?.entry_number || row._registry_position || 1,
    conditions: asArray(prior?.conditions),
    trigger_if_result: false,
    exclude_if_result: false,
    final_status: "INSUFFICIENT_EVIDENCE",
    feature_refs: asArray(prior?.feature_refs).length ? prior.feature_refs : ["UNKNOWN"],
    evidence_ref: "ROW_REINVESTIGATION_UNRESOLVED",
    reasoning_summary: `STAGE7_ROW_REINVESTIGATION_EXHAUSTED: ${reason || "row still lacked sufficient condition-level provenance after two attempts"}`
  };
}
