...
`M3.S1.C2` Module III controls authority selection, conflict handling, no-local-redefinition rules, Field Canon authority, Registry authority, source authority, and output handoff authority.
...
| `REF_FIELD_DERIVATION_CANON.md` | field meaning, purpose, source route, derivation rule, empty-state rule, evidence requirement, owner Module | object custody, registry row canon, terminal rendering |
...
| `AI_THREAT_REGISTRY` | registry rows evaluated by Module XI | target facts, feature invention, legal advice, report prose |
| `09_OUTPUT_HANDOFF_CONTRACT.md` | final handoff branches, renderer contract, machine handoff, terminal object requirements | new diligence, new evidence, upstream mutation |
...
`M3.S2.C5` If the decision concerns field meaning, derivation logic, empty-state handling, evidence requirement, or owner Module, apply `REF_FIELD_DERIVATION_CANON.md`.
...
`M3.S2.C9` If the decision concerns registry row evaluation, trigger status, exposure status, control status, or registry recovery, apply `MODULE XI` under `REGISTRY_KEY_v3_0.md` and `AI_THREAT_REGISTRY`.

`M3.S2.C10` If the decision concerns final handoff branches, display payload, machine handoff, renderer contract, or terminal JSON shape, apply `MODULE XIII`, `MODULE XIV`, and `09_OUTPUT_HANDOFF_CONTRACT.md`.

---

## M3.S3 — Field, Shape, and Custody Rule

`M3.S3.C1` Field Canon controls what a field means.

`M3.S3.C2` Module IV controls where the field may live.

`M3.S3.C3` A field may enter a state object only if both Field Canon authority and Module IV custody authorize it.

`M3.S3.C4` A complete-looking field with undefined Field Canon meaning is invalid.

`M3.S3.C5` A valid field placed in the wrong object path is invalid.

`M3.S3.C6` If a required field is missing from Field Canon, mark `FIELD_CANON_GAP`.

`M3.S3.C7` If Field Canon and Module IV conflict, mark `FIELD_STATE_CONFLICT` and route to Module XII or the owning Module for repair.
...
`M3.S4.C2` `AI_THREAT_REGISTRY` supplies the registry rows evaluated by Module XI.
...
`M3.S6.C5` Field-meaning conflicts resolve to `REF_FIELD_DERIVATION_CANON.md`.
...
`M3.S6.C9` Registry-evaluation conflicts resolve to `MODULE XI` under registry authority.

`M3.S6.C10` Final-handoff and terminal-schema conflicts resolve to `MODULE XIII`, `MODULE XIV`, and `09_OUTPUT_HANDOFF_CONTRACT.md`.
...
`M4.S1.C4` Field meaning remains controlled by `REF_FIELD_DERIVATION_CANON.md`.
...
`M5.S3.C6` A ledger row must not invent facts outside admitted evidence, documented absence/access records, locked upstream objects, Field Canon rules, Registry authority, or synthetic/demo labels.
...
`M5.S4.C4` A phase-local ledger template may not conflict with Module III authority, Module IV custody, Field Canon, Registry Key, or Output Handoff Contract.
...
`M5.S7.C4` Field-derivation rows must cite Field Canon row IDs where applicable.
...
`M6.S1C.C9` Any violation of `M6.S1C` must be classified under `M2.T5A` and routed through `M6.S13.C0A–C0D`.
...
`M6.S3.C3` Field meanings remain subject to `REF_FIELD_DERIVATION_CANON.md` where applicable, but Module VI controls source extraction, evidence indexing, source status, and route classification.
...
`M6.S13.C0A` Module VI lock defects must be classified under `M2.T5A`.
...
`M7.S3.C5` Field meanings remain subject to `REF_FIELD_DERIVATION_CANON.md`.
...
`M8.S4.C5` Field meanings remain subject to `REF_FIELD_DERIVATION_CANON.md`.
...
`M9.S4.C5` Field meanings remain subject to `REF_FIELD_DERIVATION_CANON.md`.
...
`M10.S6.C4` Field meanings remain subject to `REF_FIELD_DERIVATION_CANON.md` where applicable, but Module X controls visible data/control signal derivation and Anti-Unknown status assignment within its scope.
...
`M11.T0.C8` Substantive registry intelligence for evaluation-routed rows remains Module XI model-led under `REGISTRY_KEY_v3_0`, `AI_THREAT_REGISTRY`, and `REGISTRY_EVALUATION_RULES`.
...
`M11.S1B.C6` MUST consume the locked registry row source `AI_THREAT_REGISTRY`.

`M11.S1B.C7` MUST consume `REGISTRY_KEY_v3_0`.

`M11.S1B.C8` MUST consume `REGISTRY_EVALUATION_RULES`.
...
| Required Input | Required Use |
|---|---|
| `AI_THREAT_REGISTRY` | locked registry row inventory |
| `REGISTRY_KEY_v3_0` | vocabulary, row interpretation, archetypes, surfaces, ID rules |
| `REGISTRY_EVALUATION_RULES` | EXCLUDE_IF, control, insufficiency, and evaluation discipline |
...
```text
1. load every registry row from AI_THREAT_REGISTRY;
...
| Condition | Required Handling |
|---|---|
| `AI_THREAT_REGISTRY` missing | `CONTROLLED_FAILURE` |
| registry row count not 98 | `CONTROLLED_FAILURE` unless registry contract amended |
| `REGISTRY_KEY_v3_0` missing | `CONTROLLED_FAILURE` |
| `REGISTRY_EVALUATION_RULES` missing | `CONTROLLED_FAILURE` |
...
```json id="m11-trigger-review-workspace"
{
  ...
  "registry_row_ref": {
    ...
    "registry_row_source_ref": "AI_THREAT_REGISTRY"
  },
...
field_derivation_records:
  ...
  - table_id: "M11.S5"
    ...
    source_internal_checkpoint_step: "`AI_THREAT_REGISTRY` normalized rows"
...
`M11.S6.C1` Consume `REGISTRY_KEY_v3_0`.

`M11.S6.C2` Consume `AI_THREAT_REGISTRY`.

`M11.S6.C3` Consume `REGISTRY_EVALUATION_RULES`.
...
`M11.S8.C1` Load every row from `AI_THREAT_REGISTRY`.
...
`M11.S12.C4` Consume REGISTRY_EVALUATION_RULES.
...
```json id="m11-output-contract"
{
  "target_exposure_profile": {
    ...
    "registry_manifest": {
      ...
      "registry_key_ref": "REGISTRY_KEY_v3_0",
...
field_derivation_records:
  ...
  - table_id: "M13.S5"
    ...
    mode: "D/H"
    derivation_authority: "upstream profiles"
    value_rule: "Build baseline, architecture, archetypes, compliance groups."
  ...
  - table_id: "M13.S5"
    ...
    mode: "D/H"
    derivation_authority: "upstream profile fields"
    value_rule: "Assemble payload suggestions."
  ...
  - table_id: "M13.S5"
    ...
    mode: "D/H"
    derivation_authority: "upstream state"
    value_rule: "Suggest review-ready prefill fields."
  ...
  - table_id: "M13.S5"
    ...
    mode: "D/H"
    derivation_authority: "limitations + review routes"
    value_rule: "Assemble confirmation questions."
